...
export default function MarketOpportunities({ passwordHash }) {
  ...
  const topFive = useMemo(() => results.filter((row) => row.qualified === true).slice(0, 5), [results]);
        ...
        <section className="topOpportunityCards">
          {topFive.length ? topFive.map((row, index) => (
            <article key={`top-${row.symbol}`}>
              ...
              <p>{row.symbol} - {row.status === "READY TO BUY" ? "Qualified" : row.status}</p>
              <p>Buy only at {formatCurrency(row.recommendedEntry, row.currency)}. Safety Exit {formatCurrency(row.stopLoss, row.currency)}.</p>
              <Link className="prepareTradeLink" href={traderCompanyHref(row.symbol, "from=scanner&prepare=1")}>Open trade plan</Link>
            </article>
