      ...
      <main className="panel">
        ...
        <div className="tableWrap">
          <table>
            <thead>
              <tr>
                <th>Company</th><th>Score</th><th>Capital Flow</th><th>Pressure</th><th>Rel Volume</th><th>Momentum</th><th>Flow Change</th><th>Flow State</th><th>Status</th><th>Current</th>
            ...
            <tbody>
              {results.map((row, index) => (
                <tr key={row.symbol}>
                  ...
                  <td>{formatNumber(row.capitalFlowScore)}</td>
                  <td>{row.buyingSellingPressure || "--"}</td>
                  <td>{Number.isFinite(Number(row.relativeVolume)) ? `${formatNumber(row.relativeVolume)}x` : "--"}</td>
                  <td>{Number.isFinite(Number(row.priceMomentum)) ? `${Number(row.priceMomentum) > 0 ? "+" : ""}${formatNumber(row.priceMomentum)}%` : "--"}</td>
                  <td>{Number.isFinite(Number(row.capitalFlowChange)) ? `${Number(row.capitalFlowChange) > 0 ? "+" : ""}${formatNumber(row.capitalFlowChange)}` : "--"}</td>
                  <td>{row.capitalFlowState || "--"}</td>
