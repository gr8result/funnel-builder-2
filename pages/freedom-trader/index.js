    ...
    <div className="page">
      ...
      <section className="panel reportPanel">
        ...
        <div className="settingsGrid">
          ...
          <label>Max open trades<input type="number" value={reportSettings.maximumSimultaneousOpenTrades} onChange={(event) => updateReportSetting("maximumSimultaneousOpenTrades", event.target.value)} /></label>
        ...
        {report ? <ReportView report={report} showDetails={reportDetailsOpen} alerts={actionAlerts} /> : <div className="emptyState">Click the main button to generate the current action report.</div>}
      </section>
...
function ReportView({ report, showDetails, alerts }) {
      ...
      <section className="reportSubsection">
        ...
        {report.orderInstructions?.approvedTrades?.length ? report.orderInstructions.approvedTrades.map((item) => (
          <article className="instruction" key={item.symbol}>
            ...
            <span>Stop-loss after purchase: {item.stopLossAfterPurchase}</span>
            <span>Profit-taking order: {item.profitTakingOrder}</span>
            <small>Freedom has not placed this order.</small>
          </article>
  ...
  return (
    <article className="recommendation">
      ...
      {item.status === "READY TO BUY" ? (
        <>
          <p>Buy only if it reaches: {formatCurrency(item.buyPrice, item.currency)}</p>
          <p>Sell for profit at: {formatCurrency(item.profitTakingPrice, item.currency)}</p>
          <p>Sell to limit the loss at: {formatCurrency(item.stopLossPrice, item.currency)}</p>
          ...
          <p>Maximum planned loss: {formatCurrency(item.maximumPlannedLoss, item.currency)} plus costs</p>
        </>
      ) : item.status === "WAIT" ? (
        <>
          <p>Do not buy yet.</p>
          <p>Preferred buy price: {formatCurrency(item.buyPrice, item.currency)}</p>
...
function PositionAction({ item }) {
  return (
    <article className="positionAction">
      ...
      <span>Profit target: {formatCurrency(item.target, item.currency)}</span>
      <span>Stop-loss: {formatCurrency(item.stopLoss, item.currency)}</span>
