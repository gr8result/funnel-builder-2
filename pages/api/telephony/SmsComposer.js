...
export default function SmsComposer({
  ...
  useEffect(() => {
    ...
    // do NOT overwrite message when lead changes unless empty:
  ...
  function handleSaveTemplate() {
    ...
    const title = prompt("Template name (e.g. Missed call follow-up):", "New template");
    if (!title) return;
  ...
  function handleDeleteTemplate() {
    ...
    const next = templates.filter((t) => String(t.id) !== String(selectedTemplate.id));
    persist(next);
    setSelectedId("");
  ...
  async function send() {
    ...
    if (!toVal.startsWith("+")) return setErr("Phone must be in +61... format.");
  ...
  function initSmsRecognition() {
    ...
    if (!smsRecognitionRef.current) {
      ...
      recognition.onerror = () => {
        // keep silent; user will see recording stop if needed
      };
    ...
    <div style={styles.wrap}>
      ...
      <div style={{ marginBottom: 8 }}>
        <div style={styles.row}>
          ...
          <button type="button" onClick={handleSaveTemplate} style={styles.smallBtn("#22c55e")}>
            Save new
          </button>
      ...
      <div style={{ marginBottom: 8 }}>
        <div style={styles.row}>
          ...
          <button type="button" onClick={send} style={styles.btn("#22c55e")} disabled={busy}>
      ...
      <div style={styles.hint}>
        Tip: keep templates short. You can say “full stop”, “comma”, “question mark”.
      </div>
    </div>
  );
}
