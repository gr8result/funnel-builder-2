...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.redirect(doneRedirectUrl(req, oauthState.redirect_path, "ok", `YouTube connected: ${channelName}`));
  } catch (err) {
    console.error("[YouTube OAuth callback]", err);
    return res.redirect(doneRedirectUrl(req, "/modules/social_media", "error", err.message || "YouTube connection failed"));
  }
}
