...
Open File is the normal-user action for continuing a selections project. It opens the builder's computer file picker and reads a local `.gr8selections.json` file.
...
- accepted extensions: `.gr8selections.json`, `.json`
- `multiple: false`

Fallback path:

- hidden `<input type="file" accept=".gr8selections.json,.json,application/json">`
