...
4. Compatible Product Library items are shown only when `selection_visibility` is `client_selectable` or an allowed `builder_selectable` value.
...
The modal uses the shared `ProductSelectionCatalogueAdapter` boundary. The development adapter supplies realistic demonstration products, all labelled as demonstration and indicative pricing. It also contains an estimating-only concrete rate regression row that must never appear in the picker.

The picker now has three levels: selection item list, product grid, and product detail. Product detail links use only the stored supplier product URL, open a new tab, and use `rel="noopener noreferrer"`. Missing supplier URLs stay missing; the app does not construct guessed URLs.
