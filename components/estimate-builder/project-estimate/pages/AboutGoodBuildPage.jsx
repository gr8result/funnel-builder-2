...
export default function AboutGoodBuildPage({ resolvedTheme, accent, logo, builderName, whyStats }) {
  const aboutWhyCards = [
  ...
  const aboutCopy = "Building a home is one of the biggest investments you will ever make. At GoodBuild Quality Builders we believe the experience should be just as rewarding as the finished home.";
    ...
    <section className="proposal-builder-page" style={aboutPageStyles.page}>
      ...
      <div style={aboutPageStyles.top}>
        <div style={aboutPageStyles.copy}>
          <h2 style={aboutPageStyles.title}>A quality builder focused on trust, care and a home your family will be proud of.</h2>
          <p style={aboutPageStyles.body}>{aboutCopy}</p>
        </div>
      ...
      <div style={aboutPageStyles.lower}>
        <div>
          <div style={{ ...styles.luxuryEyebrow, color: accent }}>Why build with us</div>
        ...
        <div style={aboutPageStyles.cardGrid}>
          {aboutWhyCards.map(([title, body, Icon]) => (
            <div key={title} style={aboutPageStyles.card}>
              <span style={{ ...aboutPageStyles.icon, background: accent }}>
