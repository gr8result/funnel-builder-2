...
const GROUPS = [
  {
    label: "Select",
    tools: [
      { key: TOOLS.POINTER, icon: "PTR", label: "Pointer", tip: "Select overlays." },
      { key: TOOLS.PAN, icon: "PAN", label: "Pan", tip: "Pan the plan view. Middle mouse or Space+drag works with any tool." },
    ],
  },
  {
    label: "Measure",
    tools: [
      { key: TOOLS.MEASURE, icon: "DIM", label: "Measure", tip: "Click two points to draw a measurement line." },
      { key: TOOLS.AREA, icon: "m2", label: "Area", color: "#0f766e", tip: "Trace a shape to calculate area." },
      { key: TOOLS.POLYLINE, icon: "PL", label: "Polyline", tip: "Draw a multi-point line." },
    ],
  },
  {
    label: "Walls",
    tools: [
      { key: TOOLS.EXTERNAL_WALL, icon: "EXT", label: "Ext Wall", color: "#1d4ed8", tip: "Draw external walls." },
      { key: TOOLS.INTERNAL_WALL, icon: "INT", label: "Int Wall", color: "#ea580c", tip: "Draw internal wall segments." },
    ],
  },
  {
    label: "Areas",
    tools: [
      { key: TOOLS.ROOM, icon: "RM", label: "Room", color: "#0369a1", tip: "Draw room polygon." },
      { key: TOOLS.RECTANGLE, icon: "RECT", label: "Rectangle", color: "#7c3aed", tip: "Click two opposite corners." },
      { key: TOOLS.CIRCLE, icon: "CIR", label: "Circle", color: "#0891b2", tip: "Click centre then radius." },
    ],
  },
  {
    label: "Elements",
    tools: [
      { key: TOOLS.DOOR, icon: "DR", label: "Door", color: "#16a34a", tip: "Place a door marker." },
      { key: TOOLS.WINDOW, icon: "WIN", label: "Window", color: "#7c3aed", tip: "Place a window marker." },
      { key: TOOLS.COLUMN, icon: "COL", label: "Column", color: "#92400e", tip: "Place a column marker." },
    ],
  },
  {
    label: "Edit",
    tools: [
      { key: TOOLS.DELETE, icon: "DEL", label: "Delete", color: "#dc2626", tip: "Click an overlay to delete it." },
    ],
  },
];
  ...
  return (
    <div style={S.bar}>
      ...
      <div style={S.group}>
        ...
        <div style={S.groupTools}>
          <ActionBtn onClick={onUndo} disabled={!canUndo} title="Undo">UNDO</ActionBtn>
          <ActionBtn onClick={onRedo} disabled={!canRedo} title="Redo">REDO</ActionBtn>
        </div>
      </div>
...
function ToolBtn({ icon, label, title, tip, active, color, onClick, disabled = false }) {
  ...
  return (
    ...
    >
      <span style={S.toolIcon}>{icon || label}</span>
...
const S = {
  ...
  toolIcon: { fontSize: 10, lineHeight: 1, fontWeight: 900, minHeight: 12 },
  toolLabel: { fontSize: 10, fontWeight: 700, color: "inherit", whiteSpace: "nowrap" },
  action: { minWidth: 36, height: 36, border: "1.5px solid #e2e8f0", borderRadius: 7, background: "#fff", color: "#334155", fontSize: 9, fontWeight: 900, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: "0 5px" },
