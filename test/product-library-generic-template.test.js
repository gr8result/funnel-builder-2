...
test("Product Library and Inclusions browser expose normal-user flow without admin controls", () => {
  ...
  assert(page.includes("No {category?.label || \"category\"} products have been imported yet."));
  ...
  assert(page.includes("Choose An Interior Area"));
  assert(page.includes("Choose A Category"));
  assert(page.includes("Back to ${selectedArea.label}"));
  assert(page.includes("data-area-key={selectedArea.key}"));
  assert(page.includes("data-category-key={child.kind === \"category\" ? child.key : \"\"}"));
  assert(page.includes("productCategory(\"garage-door\", \"Garage Door\")"));
  assert(page.includes("productCategory(\"oven\", \"Oven\")"));
  assert(!page.includes("No approved selection items have been uploaded for this area."));
  assert(!page.includes("Upload Approved Selections CSV"));
