...
export function InclusionsSelectionsProjectBanner({ currentStage, context, saveStatus, onSave, locked = false }: Props) {
  ...
  const dashboardHref = projectDashboardHref(context);

  useEffect(() => {
    if (!context.organisationId) return;
    if (context.projectId) registerProjectOpen(context as ProjectSelectionContext, currentStage);
    setProjects(loadProjectFileMenu(context.organisationId));
  }, [context.organisationId, context.projectId, currentStage]);
  ...
  async function handleSave() {
    ...
    try {
      ...
      else await saveSelectionsProject(context, currentStage);
      setStatus("saved");
      setMessage("Saved.");
    } catch (error) {
  ...
  async function handleSaveAs() {
    if (!context.organisationId || !context.projectId || !draft.projectName.trim()) return;
    ...
    try {
      const created = await saveSelectionsProjectAs(context, target, copyOptions);
      setSaveAsOpen(false);
  ...
  async function handleImportFile(file: File) {
    ...
    try {
      ...
      setImportPreview({ ok: false, error: "Malformed selections file." });
      return;
    }
    setImportPreview(previewSelectionsProjectImport(parsed, context.organisationId ?? ""));
  }

  async function handleImportAsNew() {
    if (!importPreview?.ok || !context.organisationId) return;
    const target: ProjectSelectionContext = {
  ...
  return (
    <section className="projectBanner" aria-label="Inclusions and Selections project banner">
      ...
      <div className="bannerActions">
        ...
        {menuOpen ? (
          <div className="fileMenu">
            ...
            <button type="button" onClick={() => setImportOpen(true)}>Import Selections File</button>
            <button type="button" disabled={!hasProject} onClick={() => void handleExport()}>Export Selections File</button>
            <button type="button" disabled={locked || !hasProject} onClick={() => void handleSave()}>Save</button>
            <button type="button" disabled={!hasProject} onClick={() => setSaveAsOpen(true)}>Save As</button>
            <button type="button" disabled={!hasProject} onClick={() => void saveSelectionsBuilderTemplate(context).then(() => setMessage("Builder template saved."))}>Save as Builder Template</button>
            <button type="button" onClick={() => void router.push(closeSelectionsProject(context))}>Close Project</button>
          </div>
        ) : null}
      </div>
      {!hasProject ? (
        ...
        <div className="bannerModal" role="dialog" aria-modal="true">
          <div className="modalPanel">
            ...
            <div className="projectRows">
              ...
              {!filteredProjects.length ? <p>No saved jobs match the current search.</p> : null}
            </div>
          </div>
        </div>
      ) : null}
      ...
      {importOpen ? (
        <div className="bannerModal" role="dialog" aria-modal="true">
          <div className="modalPanel narrow">
            <header><h2>Import Selections File</h2><button type="button" onClick={() => setImportOpen(false)}>Close</button></header>
            <input ref={fileInputRef} type="file" accept=".gr8selections.json,application/json" onChange={(event) => { const file = event.target.files?.[0]; if (file) void handleImportFile(file); }} />
            {importPreview ? (
              <p className={importPreview.ok ? "okMessage" : "errorMessage"}>
                {importPreview.ok ? `Ready to import ${importPreview.file.projectSummary.projectName ?? fileNameFromContext(importPreview.file.projectSummary)}. ${importPreview.warnings.join(" ")}` : importPreview.ok === false ? importPreview.error : ""}
              </p>
            ) : null}
