...
- Shared project banner and file-management UI across every stage, backed by `projectFileManagementService` for Open Existing Job, Import Project File, Save, Save As, builder template save, portable export/import and close-project routing.
...
- `services/projectFileManagementService.ts`: shared application service for the project banner, project picker, app project-source adapter, stage save, Save As copy, builder template save and `.gr8selections.json` import/export.
...
- `tests/projectBannerFileManagement.test.ts`: focused coverage for shared banner placement, dashboard/stage routes, project picker index, registered-job source adapter, Save, Save As, builder template save and portable import/export validation.
...
The stage-one through stage-six routes require an existing `organisationId` and `projectId`. If none is present, the shared banner shows Open Existing Job, Import Project File, Create New Project and Back to Project Dashboard actions without rendering an empty workflow beneath it. The module uses browser-scoped repositories until approved database repositories are added; the project picker also reads the existing Estimate Builder registered-job and Projects Hub recent-job sources for the active organisation.
