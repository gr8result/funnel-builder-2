...
- No approvals, locked snapshots, generated documents, procurement export or Estimate Builder export from the workspace or review stage.
...
- `/inclusions-selections/approvals` placeholder only

The stage-one through stage-four routes require an existing `organisationId` and `projectId`. They use in-memory persistence until approved database repositories are added.
