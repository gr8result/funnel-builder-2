...
const effectBody = planViewer.match(/useEffect\(\(\) => \{\s*\n    fitTo\("fit-page"\);[\s\S]*?\n  \}, \[pdfDocument, page\?\.id, page\?\.pageNumber, page\?\.rotation\]\);/)?.[0] || "";
assert.ok(effectBody, "fit-page effect should still depend only on document/page/rotation");
assert.doesNotMatch(effectBody, /detectedWalls|wallDetection|exteriorHighlightedWalls|exteriorWalls/, "detection results must not be fit-page dependencies");
