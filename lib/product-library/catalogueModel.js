...
export const DEMO_BUILDER_ORGANISATION_ID = "846885cd-25b9-4eca-b9f9-3fd02f5882d8";
export const EXPLICIT_BUILDER_DISABLE_REASON = "builder_explicit_disable";

export const LOCKED_PRODUCT_FAMILIES = {
  bricks: {
    status: "locked",
    reason: "Locked - protected from bulk catalogue changes",
    expectedMinimumRecords: 147,
  },
  roofing: {
    status: "locked",
    reason: "Locked - protected from bulk catalogue changes",
    expectedMinimumRecords: 188,
  },
  cladding: {
    status: "locked",
    reason: "Locked - protected from bulk catalogue changes",
    expectedMinimumRecords: 10,
  },
  windows: {
    status: "locked",
    reason: "Locked - protected from bulk catalogue changes",
    expectedMinimumRecords: 6,
  },
  "entry-doors": {
    status: "locked",
    reason: "Locked - protected from bulk catalogue changes",
    expectedMinimumRecords: 4,
  },
  "garage-doors": {
    status: "locked",
    reason: "Locked - protected from bulk catalogue changes",
...
export const GENERIC_IMAGE_URLS = {
  ...
  roofing: "/images/product-library/colorbond-roofing-freshwater.jpg",
  ...
  entryDoors: "/images/product-library/entry-doors/entry-doors-dashboard-contemporary.webp",
  externalDoors: "https://images.unsplash.com/photo-1600607687644-c7171b42498b?auto=format&fit=crop&w=900&q=80",
  garageDoors: "/images/product-library/garage-doors/garage-doors-modern-flatline.webp",
  ...
  externalLighting: "/images/product-library/external-lighting/external-lighting-dashboard-modern-entrance.webp",
  drivewayFinishes: "/images/product-library/driveway/driveway-plain-concrete.jpeg",
  decking: "https://images.unsplash.com/photo-1591825729269-caeb344f6df2?auto=format&fit=crop&w=900&q=80",
  internalDoors: "/images/product-library/internal-areas/category-internal-door.webp",
  stoneBenchtops: "https://images.unsplash.com/photo-1556911220-e15b29be8c8f?auto=format&fit=crop&w=900&q=80",
  cabinetry: "/images/catalogues/product-library/categories/cabinet-doors-panels-standard-base-cupboards.jpg",
  cabinetFinish: "/images/catalogues/product-library/categories/cabinet-doors-panels-standard-base-cupboards.jpg",
  handles: "/images/catalogues/product-library/categories/cabinet-handles-handle-house-c3.jpg",
...
const BRICK_PLACEHOLDER_IMAGE = brickPlaceholderImage();

const PRODUCT_IMAGE_FIELDS = ["primaryImage", "primaryImageUrl", "primary_image_url", "exactImageURL", "exact_image_url", "imageUrl", "image_url"];
const RANGE_IMAGE_FIELDS = ["rangeImage", "rangeImageUrl", "range_image_url", "thumbnail", "thumbnailUrl", "thumbnailURL", "thumbnail_url"];

export const FAMILY_IMAGE_FALLBACKS = {
  bricks: BRICK_PLACEHOLDER_IMAGE,
  ...
  entryDoors: GENERIC_IMAGE_URLS.entryDoors,
  "garage-doors": GENERIC_IMAGE_URLS.garageDoors,
  garageDoors: GENERIC_IMAGE_URLS.garageDoors,
  gutters: GENERIC_IMAGE_URLS.gutters,
  fascia: GENERIC_IMAGE_URLS.fascia,
  "gutters-fascia": GENERIC_IMAGE_URLS.gutters,
  balustrades: GENERIC_IMAGE_URLS.balustrades,
  "external-lighting": GENERIC_IMAGE_URLS.externalLighting,
  exteriorLighting: GENERIC_IMAGE_URLS.externalLighting,
  paint: GENERIC_IMAGE_URLS.paint,
  "exterior-paint": GENERIC_IMAGE_URLS.exteriorPaint,
  driveway: GENERIC_IMAGE_URLS.drivewayFinishes,
  "driveway-finishes": GENERIC_IMAGE_URLS.drivewayFinishes,
  decking: GENERIC_IMAGE_URLS.decking,
  pool: GENERIC_IMAGE_URLS.poolFinish,
  "pool-finishes": GENERIC_IMAGE_URLS.poolFinish,
  "retaining-walls": GENERIC_IMAGE_URLS.outdoor,
  retainingWalls: GENERIC_IMAGE_URLS.outdoor,
  landscaping: GENERIC_IMAGE_URLS.outdoor,
  kitchen: GENERIC_IMAGE_URLS.kitchen,
  cabinetry: GENERIC_IMAGE_URLS.cabinetry,
  "cabinet-finish": GENERIC_IMAGE_URLS.cabinetFinish,
  handles: GENERIC_IMAGE_URLS.handles,
  "stone-benchtops": GENERIC_IMAGE_URLS.stoneBenchtops,
  splashback: GENERIC_IMAGE_URLS.splashback,
  ovens: GENERIC_IMAGE_URLS.ovens,
  oven: GENERIC_IMAGE_URLS.ovens,
  cooktops: GENERIC_IMAGE_URLS.cooktops,
  cooktop: GENERIC_IMAGE_URLS.cooktops,
  rangehoods: GENERIC_IMAGE_URLS.rangehood,
  dishwashers: GENERIC_IMAGE_URLS.dishwasher,
  microwaves: GENERIC_IMAGE_URLS.microwave,
  fridges: GENERIC_IMAGE_URLS.fridge,
  "kitchen-sinks": GENERIC_IMAGE_URLS.sink,
  sink: GENERIC_IMAGE_URLS.sink,
  "kitchen-sink-mixers": GENERIC_IMAGE_URLS.tapware,
  tapware: GENERIC_IMAGE_URLS.tapware,
  lighting: GENERIC_IMAGE_URLS.lighting,
  bathroom: GENERIC_IMAGE_URLS.bathroom,
  bedrooms: GENERIC_IMAGE_URLS.bedrooms,
  "internal-doors": GENERIC_IMAGE_URLS.internalDoors,
  flooring: GENERIC_IMAGE_URLS.flooring,
};

const REMOVED_CLADDING_PRODUCT_NAMES = new Set([
  "colorbond wall sheeting",
]);

export function isRemovedDuplicateCladdingProduct(product = {}) {
  const entity = product.metadata?.productEntity || product;
  const familyKey = normaliseText(entity.familyKey || entity.family_key || product.familyKey || product.family_key);
  if (familyKey !== "cladding") return false;
  const productCode = entity.productCode || entity.product_code || product.productCode || product.product_code;
  if (["CLADDING-JAMES-HARDIE-LINEA-150", "CLADDING-JAMES-HARDIE-LINEA-180"].includes(productCode)) return false;
  const productName = normaliseText(entity.productName || entity.product_name || product.productName || product.product_name);
  if (REMOVED_CLADDING_PRODUCT_NAMES.has(productName)) return true;
  if (["150mm linea board", "180mm linea board"].includes(productName)) return true;
  return false;
}
...
export const TAXONOMY_CATEGORY_DEFINITIONS = [
  ...
  category("exterior", "Roofing", "Roofing Materials", ["ROOFING", "ROOF TILES", "ROOFING IRON", "ROOF SHEETING", "ROOF COLOUR", "ROOF COLOR", "GUTTER", "GUTTERS", "FASCIA", "DOWNPIPE", "DOWNPIPES"]),
  ...
  category("exterior", "Pool", "Pool Finishes", ["POOL", "POOL FINISH", "POOL TILE", "COPING"]),
  category("exterior", "Retaining Walls", "Retaining Wall Finishes", ["RETAINING WALL", "SLEEPER WALL", "BLOCK WALL"]),
  category("exterior", "Landscaping", "Landscape Finishes", ["LANDSCAPING", "TURF", "MULCH", "GARDEN EDGING", "FEATURE GRAVEL"]),
...
const BASE_PRODUCT_FAMILIES = [
  ...
  family({
    familyKey: "cabinetry",
    displayName: "Cabinetry",
    topLevelArea: "kitchen",
    category: "Cabinetry",
    subcategory: "Kitchen Cabinetry Systems",
    sourceMatchers: ["CABINETRY", "KITCHEN CABINET", "CABINETMAKER"],
    unit: "ITEM",
    quantityRule: "Selected as a builder/cabinetmaker system; cabinet quantities remain plan and joinery-schedule driven.",
    requiredAttributes: ["supplier", "range", "construction", "configuration"],
    optionalAttributes: ["doorStyle", "hardware", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["supplier", "system", "doorStyle", "configuration"],
    imageRequirement: "Kitchen cabinetry or cabinet system image required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.cabinetry,
  }),
  family({
    familyKey: "cabinet-finish",
    displayName: "Cabinet Finish",
    topLevelArea: "kitchen",
    category: "Cabinet Finish",
    subcategory: "Doors and Panels",
    sourceMatchers: ["CABINET FINISH", "LAMINATE", "TWO PAC", "POLYTEC", "LAMINEX"],
    unit: "ITEM",
    quantityRule: "Selected by brand, range, colour, finish and texture; quantities remain joinery-schedule driven.",
    requiredAttributes: ["manufacturer", "brand", "range", "colour", "finish"],
    optionalAttributes: ["texture", "substrate", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["brand", "range", "colour", "finish", "texture"],
    imageRequirement: "Exact colour/finish swatch or reviewed range image required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.cabinetFinish,
  }),
  family({
    familyKey: "handles",
    displayName: "Handles",
    ...
    sourceMatchers: ["HANDLE", "CABINET HANDLE", "PULL"],
    unit: "EACH",
    quantityRule: "Per cabinet handle or pull from joinery schedule.",
    requiredAttributes: ["supplier", "brand", "range", "model", "finish", "size"],
    optionalAttributes: ["material", "projection", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["range", "model", "finish", "size", "material"],
    imageRequirement: "Exact cabinet handle image required where a model is named.",
    pricingMode: "per-item-upgrade",
    image: GENERIC_IMAGE_URLS.handles,
  }),
  family({
    familyKey: "bricks",
    displayName: "Bricks",
    ...
    image: FAMILY_IMAGE_FALLBACKS.bricks,
  }),
  family({
    familyKey: "cladding",
    displayName: "Cladding",
    topLevelArea: "exterior",
    category: "Cladding",
    subcategory: "External Cladding",
    sourceMatchers: ["CLADDING", "WEATHERBOARD", "WEATHER BOARD", "FACADE BOARD", "EXTERNAL WALL FINISH"],
    unit: "ITEM",
    quantityRule: "Selected as a facade system by manufacturer, range, material, profile, colour and finish; takeoff remains wall-area driven.",
    requiredAttributes: ["manufacturer", "range", "material", "profile", "colour", "finish"],
    optionalAttributes: ["boardWidth", "orientation", "texture", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["range", "material", "profile", "colour", "finish", "orientation"],
    imageRequirement: "Cladding product/profile or facade application image required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.cladding,
  }),
  family({
    familyKey: "roofing",
    displayName: "Roofing",
    ...
    sourceMatchers: ["ROOFING", "ROOF TILES", "ROOFING IRON", "METAL ROOF", "ROOF SHEETING", "ROOFING MATERIALS", "ROOF COLOUR", "GUTTER", "GUTTERS", "FASCIA", "DOWNPIPE", "DOWNPIPES"],
    unit: "ITEM",
    quantityRule: "Configured as one roof package: roof type/system/profile/colour/finish, then fascia, gutters and downpipes; quantities remain quote-linked to measured roof area and roofline takeoff.",
    requiredAttributes: ["roofType", "manufacturer", "brand", "material", "profile", "colour", "finish"],
    optionalAttributes: ["materialManufacturer", "gauge", "minimumRoofSlope", "coverWidth", "ribHeight", "roofPackageStep", "gutterType", "gutterProfile", "fasciaType", "downpipeType", "matchRoofColour", "sharedColourCatalogueKey", "colourSelectionMode", "compatibleRoofTypes", "image", "price", "supplierURL", "supplierUrl", "specificationURL"],
    supportedVariantTypes: ["roofType", "material", "profile", "colour", "finish", "gauge", "fascia", "gutters", "downpipes"],
  ...
  family({
    ...
    topLevelArea: "exterior",
    category: "Windows",
    subcategory: "Window Systems",
    sourceMatchers: ["WINDOW", "WINDOWS", "SLIDING WINDOW", "AWNING WINDOW", "CASEMENT WINDOW", "DOUBLE HUNG WINDOW", "FIXED WINDOW", "LOUVRE WINDOW"],
    unit: "ITEM",
    quantityRule: "Window system, range, frame, colour, glazing and configuration selected once; opening quantities and sizes remain quote/window-schedule driven.",
    requiredAttributes: ["manufacturer", "supplier", "range", "frameMaterial", "frameColour", "glassType", "configuration"],
    optionalAttributes: ["windowTypes", "screening", "energyRating", "hardware", "image", "price", "supplierURL", "supplierUrl", "specificationURL"],
    supportedVariantTypes: ["range", "windowType", "frameMaterial", "frameColour", "glassType", "configuration"],
    imageRequirement: "Exact window type or official range image required; do not use unrelated room imagery as primary catalogue art.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.windows,
  }),
  family({
  ...
  family({
    familyKey: "entry-door-furniture",
    displayName: "Entry Door Furniture",
    topLevelArea: "exterior",
    category: "External Door Furniture",
    subcategory: "Entry Door Furniture",
    sourceMatchers: ["ENTRY DOOR FURNITURE"],
    unit: "EACH",
    quantityRule: "One primary furniture selection per scheduled exterior entry door.",
    requiredAttributes: ["brand", "model"],
    optionalAttributes: ["finish", "material", "dimensions", "lockingType"],
    supportedVariantTypes: ["finish"],
    imageRequirement: "Local licensed entrance hardware category image; product records retain their exact images.",
    pricingMode: "per-item-upgrade",
    image: "/images/product-library/entrance-door-lockset.jpg",
  }),
  family({
  ...
  family({
    familyKey: "gutters-fascia",
    displayName: "Gutters & Fascia",
    topLevelArea: "exterior",
    category: "Gutters & Fascia",
    subcategory: "Guttering / Fascia",
    sourceMatchers: ["GUTTER", "GUTTERS", "FASCIA", "DOWNPIPE", "EAVES"],
    unit: "ITEM",
    quantityRule: "Selected as one coherent system by gutter profile, fascia type, material and colour; quantities remain roofline/takeoff driven.",
    requiredAttributes: ["manufacturer", "gutterType", "gutterProfile", "fasciaType", "material", "colour"],
    optionalAttributes: ["matchRoofColour", "finish", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["gutterType", "gutterProfile", "fasciaType", "colour", "finish", "matchRoofColour"],
    imageRequirement: "Gutter, fascia or eaves system imagery required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.gutters,
  }),
  family({
    familyKey: "balustrades",
    displayName: "Balustrades & Handrails",
    topLevelArea: "exterior",
    category: "Balustrades",
    subcategory: "Balustrades / Handrails",
    sourceMatchers: ["BALUSTRADE", "BALUSTRADES", "HANDRAIL", "HANDRAILS", "BALCONY RAIL"],
    unit: "ITEM",
    quantityRule: "Selected by system/material/finish; quantities remain project-specific.",
    requiredAttributes: ["manufacturer", "range", "material", "finish", "configuration"],
    optionalAttributes: ["glazing", "height", "postType", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["system", "material", "finish", "configuration", "height"],
    imageRequirement: "Balustrade or handrail system image required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.balustrades,
  }),
  family({
    familyKey: "external-lighting",
    displayName: "External Lighting",
    topLevelArea: "exterior",
    category: "External Lighting",
    subcategory: "Outdoor Light Fittings",
    sourceMatchers: ["EXTERNAL LIGHT", "OUTDOOR LIGHT", "WALL LIGHT", "BOLLARD", "ENTRY LIGHT", "DECK LIGHT"],
    unit: "EACH",
    quantityRule: "Per selected fitting; quantities follow electrical schedule.",
    requiredAttributes: ["supplier", "brand", "model", "finish", "ipRating"],
    optionalAttributes: ["lampType", "wattage", "dimensions", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["type", "brand", "model", "finish", "ipRating"],
    imageRequirement: "Exact exterior light fitting image required for branded products.",
    pricingMode: "per-item-upgrade",
    image: GENERIC_IMAGE_URLS.externalLighting,
  }),
  family({
    ...
    subcategory: "Exterior Paint Systems",
    sourceMatchers: ["EXTERIOR PAINT", "EXTERNAL PAINT", "PAINT COLOUR", "FACADE PAINT"],
    unit: "ITEM",
    quantityRule: "Selected by brand/range/colour/finish; paint quantities remain project-scope driven.",
    requiredAttributes: ["manufacturer", "range", "colour", "finish"],
    optionalAttributes: ["substrate", "system", "sheen", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["range", "colour", "finish", "sheen", "substrate"],
    imageRequirement: "Paint product image, facade image or visual colour swatch required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.exteriorPaint,
  }),
  family({
    familyKey: "driveway",
    displayName: "Driveway",
    topLevelArea: "exterior",
    category: "Driveway Finishes",
    subcategory: "Driveway",
    sourceMatchers: ["DRIVEWAY", "CONCRETE FINISH", "EXPOSED AGGREGATE", "PAVERS"],
    unit: "ITEM",
    quantityRule: "Selected by finish/system; measured area remains project/takeoff driven.",
    requiredAttributes: ["finishType", "material", "colour", "finish"],
    optionalAttributes: ["aggregate", "sealer", "supplier", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["finishType", "material", "colour", "finish"],
    imageRequirement: "Driveway finish or system image required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.drivewayFinishes,
  }),
  family({
    familyKey: "decking",
    displayName: "Decking",
    ...
    category: "Decking",
    subcategory: "Decking",
    sourceMatchers: ["DECKING", "DECK", "COMPOSITE DECK", "TIMBER DECK"],
    unit: "ITEM",
    quantityRule: "Selected by material, manufacturer, range, board/profile, colour and finish; measured area remains project/takeoff driven.",
    requiredAttributes: ["manufacturer", "range", "material", "profile", "colour", "finish"],
    optionalAttributes: ["boardWidth", "fixing", "slipRating", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["material", "range", "profile", "colour", "finish"],
    imageRequirement: "Decking board/material or deck application image required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.decking,
  }),
  family({
    familyKey: "pool",
    displayName: "Pool",
    topLevelArea: "exterior",
    category: "Pool",
    subcategory: "Pool Finishes",
    sourceMatchers: ["POOL", "POOL FINISH", "POOL TILE", "COPING", "WATERLINE"],
    unit: "ITEM",
    quantityRule: "Project-conditional pool finish selection by finish type, colour and location.",
    requiredAttributes: ["finishType", "material", "colour", "finish"],
    optionalAttributes: ["location", "coping", "waterlineTile", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["finishType", "material", "colour", "finish", "location"],
    imageRequirement: "Pool finish, tile, coping or surround image required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.poolFinish,
  }),
  family({
    familyKey: "retaining-walls",
    displayName: "Retaining Walls",
    topLevelArea: "exterior",
    category: "Retaining Walls",
    subcategory: "Retaining Wall Finishes",
    sourceMatchers: ["RETAINING WALL", "SLEEPER WALL", "BLOCK WALL", "STONE WALL"],
    unit: "ITEM",
    quantityRule: "Project-conditional retaining wall system selection by material/system/finish.",
    requiredAttributes: ["systemType", "material", "finish", "colour"],
    optionalAttributes: ["height", "engineering", "supplier", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["systemType", "material", "finish", "colour"],
    imageRequirement: "Retaining wall system or relevant generic retaining-wall imagery required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.outdoor,
  }),
  family({
    familyKey: "landscaping",
    displayName: "Landscaping",
    topLevelArea: "exterior",
    category: "Landscaping",
    subcategory: "Landscape Finishes",
    sourceMatchers: ["LANDSCAPING", "TURF", "MULCH", "GARDEN EDGING", "FEATURE GRAVEL"],
    unit: "ITEM",
    quantityRule: "Project-conditional landscape finish/product selection.",
    requiredAttributes: ["finishType", "material", "colour", "finish"],
    optionalAttributes: ["location", "supplier", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["finishType", "material", "colour", "finish", "location"],
    imageRequirement: "Residential landscaping product or relevant generic landscape finish image required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.outdoor,
  }),
  family({
  ...
  family({
    familyKey: "door-hardware", displayName: "Internal Door Furniture", topLevelArea: "interior",
    category: "Fix Out", subcategory: "Internal Door Furniture", sourceMatchers: ["INTERNAL DOOR FURNITURE", "PASSAGE LEVER", "PRIVACY LEVER"],
    unit: "EACH", quantityRule: "Per selected door furniture set.", requiredAttributes: ["brand", "model", "function"],
    optionalAttributes: ["finish", "material"], supportedVariantTypes: ["function", "finish"],
    imageRequirement: "Official residential door furniture image.", pricingMode: "quote-required-or-allowance-only",
    image: "/images/product-library/internal-areas/category-internal-handle.webp",
  }),
  family({
    familyKey: "skirting", displayName: "Skirting & Architraves", topLevelArea: "interior",
    category: "Fix Out", subcategory: "Skirting & Architraves", sourceMatchers: ["SKIRTING", "ARCHITRAVE"],
    unit: "LENGTH", quantityRule: "Number of the published stock lengths.", requiredAttributes: ["brand", "profile", "size", "material"],
    optionalAttributes: ["finish", "length"], supportedVariantTypes: ["profile", "size", "length", "material"],
    imageRequirement: "Official trim product or installed trim image.", pricingMode: "quote-required-or-allowance-only",
    image: "/images/product-library/internal-areas/category-skirting-architraves.webp",
  }),
  family({
    familyKey: "ovens",
    displayName: "Ovens",
  ...
  family({
    ...
    category: "Fridge",
    subcategory: "Fridge",
    sourceMatchers: ["FRIDGE", "REFRIGERATOR"],
    unit: "EACH",
    quantityRule: "Per appliance selection.",
    requiredAttributes: ["supplier", "brand", "range", "model", "size"],
    optionalAttributes: ["finish", "width", "capacity", "configuration", "image", "price", "supplierURL", "supplierUrl", "specificationURL"],
    supportedVariantTypes: ["brand", "range", "model", "size", "finish", "configuration"],
    imageRequirement: "Fridge product image required.",
    pricingMode: "per-item-upgrade",
    image: GENERIC_IMAGE_URLS.fridge,
  }),
  family({
  ...
  family({
    familyKey: "splashback",
    displayName: "Splashback",
    topLevelArea: "kitchen",
    category: "Splashback",
    subcategory: "Kitchen Splashback",
    sourceMatchers: ["SPLASHBACK", "TILED SPLASHBACK", "STONE SPLASHBACK", "GLASS SPLASHBACK"],
    unit: "M2",
    quantityRule: "Measured by kitchen splashback area and selected as tile, stone, glass or builder-configured system.",
    requiredAttributes: ["supplier", "material", "colour", "finish"],
    optionalAttributes: ["brand", "range", "format", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["material", "range", "colour", "finish", "format"],
    imageRequirement: "Splashback product, swatch or relevant finish image required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.splashback,
  }),
  family({
    familyKey: "lighting",
    ...
    subcategory: "Client-selected Light Fittings",
    sourceMatchers: ["LIGHT FITTING", "LIGHTING", "PENDANT", "DOWNLIGHT", "TASK LIGHT"],
    unit: "ITEM",
    quantityRule: "Selected as client-nominated light fittings; quantities follow the electrical schedule.",
    requiredAttributes: ["supplier", "brand", "model", "finish"],
    optionalAttributes: ["lampType", "colourTemperature", "ipRating", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["type", "brand", "model", "finish", "colourTemperature"],
    imageRequirement: "Exact light fitting image or reviewed family image required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.lighting,
  }),
  family({
    familyKey: "paint",
    displayName: "Paint",
    topLevelArea: "interior",
    category: "Painting",
    subcategory: "Paint Products / Colours",
    sourceMatchers: ["WALL PAINT", "INTERIOR PAINT", "PAINT COLOUR", "PAINT"],
    unit: "ITEM",
    quantityRule: "Selected by brand, range, colour and finish; quantities remain paint-schedule driven.",
    requiredAttributes: ["manufacturer", "range", "colour", "finish"],
    optionalAttributes: ["substrate", "sheen", "image", "price", "supplierURL", "specificationURL"],
    supportedVariantTypes: ["range", "colour", "finish", "sheen"],
    imageRequirement: "Paint product image, colour swatch or reviewed paint-family image required.",
    pricingMode: "quote-required-or-allowance-only",
    image: GENERIC_IMAGE_URLS.paint,
  }),
  family({
    familyKey: "tapware",
    displayName: "Tapware",
...
export function normalizeMasterProductRecord(input = {}) {
  ...
  const sku = cleanString(input.sku || input.manufacturer_sku || input.manufacturerSku);
  ...
  const colourSwatches = Array.isArray(input.colourSwatches)
    ? input.colourSwatches
    : (Array.isArray(input.colour_swatches)
      ? input.colour_swatches
      : (Array.isArray(attributes.colourSwatches) ? attributes.colourSwatches : []));
  ...
  return {
    ...
    ...(input.masterGroup ? { masterGroup: cleanString(input.masterGroup), category: cleanString(input.category || input.categoryKey) } : {}),
    ...
    country,
    regions,
    displayOrder: moneyOrNull(input.displayOrder ?? input.display_order ?? input.sortOrder ?? input.sort_order),
    sortOrder: moneyOrNull(input.sortOrder ?? input.sort_order ?? input.displayOrder ?? input.display_order),
...
export function mergeMasterCatalogueProducts(baselineProducts = [], storedProducts = [], protectionOptions = {}) {
  const normalizedBaseline = baselineProducts.map((product) => normalizeMasterProductRecord(product));
  const normalizedStored = Array.isArray(storedProducts)
    ? storedProducts.map((product) => normalizeMasterProductRecord(product))
    : [];
  const currentCladdingCodes = new Set(
    normalizedBaseline
      .filter((product) => product.familyKey === "cladding")
      .map((product) => product.productCode),
  );
  const byProductCode = new Map();
  normalizedBaseline.forEach((product) => {
  ...
  normalizedStored.forEach((product) => {
    ...
    if (product.familyKey === "cladding" && !currentCladdingCodes.has(product.productCode)) return;
    const baseline = byProductCode.get(product.productCode);
    byProductCode.set(product.productCode, baseline ? mergeStoredMasterProduct(baseline, product) : product);
  });
  return protectCatalogueFamilyCounts(
    normalizedBaseline,
    Array.from(byProductCode.values()).filter((product) => !isRemovedDuplicateCladdingProduct(product)),
    protectionOptions,
  );
}

function mergeStoredMasterProduct(baseline = {}, stored = {}) {
  const merged = { ...baseline, ...stored };
  if (!familyIsLocked(baseline.familyKey)) return merged;
  return {
    ...merged,
    productCode: baseline.productCode,
    productId: baseline.productId,
    familyKey: baseline.familyKey,
    requirementKeys: baseline.requirementKeys,
    topLevelArea: baseline.topLevelArea,
    categoryKey: baseline.categoryKey,
    manufacturer: baseline.manufacturer,
    configuration: baseline.configuration,
    regions: baseline.regions,
    country: baseline.country,
    active: baseline.active,
    discontinued: baseline.discontinued,
    archived: baseline.archived,
    reviewRejected: baseline.reviewRejected,
    attributes: { ...(stored.attributes || {}), ...(baseline.attributes || {}) },
  };
}

export function familyCatalogueStatus(familyKey = "") {
  return LOCKED_PRODUCT_FAMILIES[familyKey]?.status || "draft";
}

export function familyIsLocked(familyKey = "") {
  return familyCatalogueStatus(familyKey) === "locked";
}

export function protectCatalogueFamilyCounts(existingProducts = [], proposedProducts = [], { explicitFamilyKey = "", allowLockedFamilyReplacement = false } = {}) {
  const existing = existingProducts.map((product) => normalizeMasterProductRecord(product));
  const proposed = proposedProducts.map((product) => normalizeMasterProductRecord(product));
  const proposedByCode = new Map(proposed.filter((product) => product.productCode).map((product) => [product.productCode, product]));
  const proposedFamilies = new Set(proposed.map((product) => product.familyKey).filter(Boolean));
  const protectedByCode = new Map();
  const familyKeys = new Set([
    ...existing.map((product) => product.familyKey).filter(Boolean),
    ...proposedFamilies,
    ...Object.keys(LOCKED_PRODUCT_FAMILIES),
  ]);

  familyKeys.forEach((familyKey) => {
    const existingFamily = existing.filter((product) => product.familyKey === familyKey);
    const proposedFamily = proposed.filter((product) => product.familyKey === familyKey);
    const locked = familyIsLocked(familyKey);
    const explicitlyTargeted = explicitFamilyKey && explicitFamilyKey === familyKey;
    const existingCount = existingFamily.length;
    const proposedCount = proposedFamily.length;

    if (existingCount > 0 && proposedCount === 0) {
      existingFamily.forEach((product) => protectedByCode.set(product.productCode, product));
      return;
    }

    if (existingCount > 0 && proposedCount < existingCount / 2 && !(explicitlyTargeted && (!locked || allowLockedFamilyReplacement))) {
      throw new Error(`Catalogue protection prevented removal of ${existingCount - proposedCount} ${familyDisplayName(familyKey)} products.`);
    }

    if (locked && explicitlyTargeted && proposedCount < existingCount && !allowLockedFamilyReplacement) {
      throw new Error(`Catalogue protection prevented replacement of locked ${familyDisplayName(familyKey)} products.`);
    }

    const familyProducts = existingCount > 0
      ? existingFamily.map((product) => proposedByCode.get(product.productCode) || product)
      : proposedFamily;
    familyProducts.forEach((product) => {
      if (product.productCode) protectedByCode.set(product.productCode, product);
    });
  });

  proposed.forEach((product) => {
    if (!product.productCode) return;
    if (familyIsLocked(product.familyKey) && !explicitFamilyKey) return;
    protectedByCode.set(product.productCode, protectedByCode.get(product.productCode) || product);
  });

  return Array.from(protectedByCode.values()).filter((product) => !isRemovedDuplicateCladdingProduct(product));
}

function familyDisplayName(familyKey = "") {
  return familyByKey(familyKey)?.displayName || familyKey || "catalogue family";
}
...
export function createBuilderProductReference(masterProduct, { organisationId, enabled = true, tier = "Classic", selectionMode = "available_upgrade", allowance = null, clientPrice = null, supplier = "", preferredSupplier = "", availability = "", notes = "", active = true, disableReason = "", disabledReason = "" } = {}) {
  if (!masterProduct?.productCode) throw new Error("Master product reference requires a productCode.");
  const isEnabled = Boolean(enabled);
  const isActive = Boolean(active);
  const resolvedDisableReason = !isEnabled || !isActive ? cleanString(disableReason || disabledReason) : "";
  return {
    ...
    notes,
    active: isActive,
    disableReason: resolvedDisableReason,
    overrides: {},
    sourceType: "master_catalogue_reference",
  };
}

export function activeQldBrickMasterProducts(products = []) {
  return products.filter((product) => (
    product?.familyKey === "bricks" &&
    product.active !== false &&
    product.discontinued !== true &&
    product.archived !== true &&
    !product.reviewRejected &&
    regionCompatible(product.regions, "QLD") &&
    ["PGH Bricks", "Austral Bricks"].includes(product.manufacturer)
  ));
}

export function isExplicitlyDisabledBuilderReference(reference = {}) {
  if (!reference || (reference.enabled !== false && reference.active !== false)) return false;
  return cleanString(reference.disableReason || reference.disabledReason || reference.reason) === EXPLICIT_BUILDER_DISABLE_REASON;
}

export function builderEnablementState(reference = {}, enabled = true) {
  const nextEnabled = Boolean(enabled);
  return {
    ...reference,
    enabled: nextEnabled,
    active: nextEnabled,
    disableReason: nextEnabled ? "" : EXPLICIT_BUILDER_DISABLE_REASON,
  };
}

function ensureBuilderEnablementsForCandidates(products = [], builderProducts = [], organisationId = "", candidates = [], options = {}) {
  if (!organisationId) return builderProducts;
  const repairFullyDisabled = options.repairFullyDisabled === true;
  const candidateCodes = new Set(candidates.map((product) => product.productCode));
  const existingRefs = new Map(
    builderProducts
      .filter((item) => item.organisationId === organisationId && candidateCodes.has(item.masterProductCode))
      .map((item) => [item.masterProductCode, item]),
  );
  const existingCandidateRefs = Array.from(existingRefs.values());
  const usableRefs = existingCandidateRefs.filter((item) => item.enabled !== false && item.active !== false);
  const staleDisabledRefs = existingCandidateRefs.filter((item) => {
    const disabled = item.enabled === false || item.active === false;
    return disabled && !isExplicitlyDisabledBuilderReference(item);
  });
  const familyFullyDisabled = candidates.length > 0 && existingCandidateRefs.length > 0 && usableRefs.length === 0;
  const repairStaleDisabledRefs = candidates.length > 0 && usableRefs.length === 0 && staleDisabledRefs.length > 0;
  const repairAllDisabledRefs = repairFullyDisabled && familyFullyDisabled;
  const repairedBuilderProducts = builderProducts.map((item) => {
    if (item.organisationId !== organisationId || !candidateCodes.has(item.masterProductCode)) return item;
    if (!repairStaleDisabledRefs && !repairAllDisabledRefs) return item;
    if (!repairAllDisabledRefs && isExplicitlyDisabledBuilderReference(item)) return item;
    return {
      ...item,
      enabled: true,
      active: true,
  ...
  if (existingRefs.size === candidates.length && candidates.length > 0) return repairedBuilderProducts;
  return [
    ...repairedBuilderProducts,
    ...candidates
      .filter((product) => !existingRefs.has(product.productCode))
      .map((product) => createBuilderProductReference(product, {
        organisationId,
        enabled: true,
        active: true,
        disableReason: "",
...
export function ensureDemoBuilderBrickEnablements(products = [], builderProducts = [], organisationId = DEMO_BUILDER_ORGANISATION_ID) {
  if (organisationId !== DEMO_BUILDER_ORGANISATION_ID) return builderProducts;
  return ensureBuilderEnablementsForCandidates(products, builderProducts, organisationId, activeQldBrickMasterProducts(products), { repairFullyDisabled: true });
}

export function ensureBuilderBrickEnablements(products = [], builderProducts = [], organisationId = "") {
  return ensureBuilderEnablementsForCandidates(products, builderProducts, organisationId, activeQldBrickMasterProducts(products), { repairFullyDisabled: true });
}

export function activeAuMetalRoofingMasterProducts(products = []) {
  return products.filter((product) => (
    product?.familyKey === "roofing" &&
    (product.roofType === "metal_roofing" || product.configuration === "metal_roofing" || product.attributes?.roofType === "metal_roofing") &&
    product.active !== false &&
    product.discontinued !== true &&
    product.archived !== true &&
    !product.reviewRejected &&
...
export function activeQldRoofingMasterProducts(products = []) {
  return products.filter((product) => (
    product?.familyKey === "roofing" &&
    product.active !== false &&
    product.discontinued !== true &&
...
export function ensureDemoBuilderRoofingEnablements(products = [], builderProducts = [], organisationId = DEMO_BUILDER_ORGANISATION_ID) {
  if (organisationId !== DEMO_BUILDER_ORGANISATION_ID) return builderProducts;
  return ensureBuilderEnablementsForCandidates(products, builderProducts, organisationId, activeQldRoofingMasterProducts(products), { repairFullyDisabled: true });
}

export function ensureBuilderRoofingEnablements(products = [], builderProducts = [], organisationId = "") {
  return ensureBuilderEnablementsForCandidates(products, builderProducts, organisationId, activeQldRoofingMasterProducts(products), { repairFullyDisabled: true });
}

export function activeExteriorOpeningMasterProducts(products = []) {
  const familyKeys = new Set(["windows", "entry-doors", "garage-doors"]);
  return products.filter((product) => (
    familyKeys.has(product?.familyKey) &&
    product.active !== false &&
    product.discontinued !== true &&
    product.archived !== true &&
    !product.reviewRejected &&
...
export function activeExteriorFinishMasterProducts(products = []) {
  const familyKeys = new Set([
    "cladding",
    "gutters-fascia",
    "balustrades",
...
export function activeCladdingMasterProducts(products = []) {
  return activeExteriorFinishMasterProducts(products).filter((product) => product.familyKey === "cladding");
}

export function activeKitchenMasterProducts(products = []) {
  const familyKeys = new Set([
    "cabinetry",
    "cabinet-finish",
    "handles",
...
export function ensureDemoBuilderExteriorOpeningEnablements(products = [], builderProducts = [], organisationId = DEMO_BUILDER_ORGANISATION_ID) {
  if (organisationId !== DEMO_BUILDER_ORGANISATION_ID) return builderProducts;
  return ensureBuilderEnablementsForCandidates(products, builderProducts, organisationId, activeExteriorOpeningMasterProducts(products));
}

export function ensureBuilderExteriorOpeningEnablements(products = [], builderProducts = [], organisationId = "") {
  return ensureBuilderEnablementsForCandidates(products, builderProducts, organisationId, activeExteriorOpeningMasterProducts(products));
}

export function ensureDemoBuilderExteriorFinishEnablements(products = [], builderProducts = [], organisationId = DEMO_BUILDER_ORGANISATION_ID) {
  if (organisationId !== DEMO_BUILDER_ORGANISATION_ID) return builderProducts;
  return ensureBuilderEnablementsForCandidates(products, builderProducts, organisationId, activeExteriorFinishMasterProducts(products));
}

export function ensureBuilderExteriorFinishEnablements(products = [], builderProducts = [], organisationId = "") {
  return ensureBuilderEnablementsForCandidates(products, builderProducts, organisationId, activeExteriorFinishMasterProducts(products));
}

export function ensureBuilderCladdingEnablements(products = [], builderProducts = [], organisationId = "") {
  return ensureBuilderEnablementsForCandidates(products, builderProducts, organisationId, activeCladdingMasterProducts(products), { repairFullyDisabled: true });
}

export function ensureDemoBuilderCladdingEnablements(products = [], builderProducts = [], organisationId = DEMO_BUILDER_ORGANISATION_ID) {
  if (organisationId !== DEMO_BUILDER_ORGANISATION_ID) return builderProducts;
  return ensureBuilderCladdingEnablements(products, builderProducts, organisationId);
}

export function ensureDemoBuilderKitchenEnablements(products = [], builderProducts = [], organisationId = DEMO_BUILDER_ORGANISATION_ID) {
  if (organisationId !== DEMO_BUILDER_ORGANISATION_ID) return builderProducts;
  return ensureBuilderEnablementsForCandidates(products, builderProducts, organisationId, activeKitchenMasterProducts(products));
}

export function ensureBuilderKitchenEnablements(products = [], builderProducts = [], organisationId = "") {
  return ensureBuilderEnablementsForCandidates(products, builderProducts, organisationId, activeKitchenMasterProducts(products));
}

export function ensureDemoBuilderCatalogueEnablements(products = [], builderProducts = [], organisationId = DEMO_BUILDER_ORGANISATION_ID) {
  return ensureDemoBuilderKitchenEnablements(
    products,
    ensureDemoBuilderCladdingEnablements(
      products,
      ensureDemoBuilderExteriorFinishEnablements(
        products,
        ensureDemoBuilderExteriorOpeningEnablements(
          products,
          ensureDemoBuilderRoofingEnablements(
            products,
            ensureDemoBuilderBrickEnablements(products, builderProducts, organisationId),
            organisationId,
          ),
          organisationId,
        ),
...
export function ensureBuilderCompletedFamilyEnablements(products = [], builderProducts = [], organisationId = "") {
  return ensureBuilderKitchenEnablements(
    products,
    ensureBuilderCladdingEnablements(
      products,
      ensureBuilderExteriorFinishEnablements(
        products,
        ensureBuilderExteriorOpeningEnablements(
          products,
          ensureBuilderRoofingEnablements(
            products,
            ensureBuilderBrickEnablements(products, builderProducts, organisationId),
            organisationId,
          ),
          organisationId,
        ),
...
export function queryClientSelectableProducts({ organisationId, projectId = "", familyKey, region = "", requirementKey = "", masterProducts = [], builderProducts = [], organisationProducts = [], savedSelections = [], includeDiscontinuedSaved = false } = {}) {
  ...
  return [...enabledMaster, ...scopedProducts].filter((product) => {
    ...
    return true;
  }).sort(compareSelectableProductOrder);
}

function compareSelectableProductOrder(a = {}, b = {}) {
  const aOrder = moneyOrNull(a.displayOrder ?? a.sortOrder ?? a.builder?.displayOrder ?? a.builder?.sortOrder);
  const bOrder = moneyOrNull(b.displayOrder ?? b.sortOrder ?? b.builder?.displayOrder ?? b.builder?.sortOrder);
...
export function masterProductToClientSelectionProduct(product = {}, { organisationId = "", requirement = null } = {}) {
  ...
  const entity = {
    ...
    colourSwatches: Array.isArray(product.colourSwatches) ? product.colourSwatches : [],
  ...
  return {
    ...
    categoryKey: product.categoryKey || entity.category,
    configuration: product.configuration || entity.attributes?.configuration || "",
...
function regionCompatible(productRegions = [], region = "AU") {
  ...
  const requestedRegion = String(region || "AU").toUpperCase();
  if (!REGION_CODES.has(requestedRegion)) return true;
  const regions = normaliseRegions(productRegions);
  return regions.includes("AU") || regions.includes(requestedRegion);
}
...
export function imageForFamilyKey(familyKey, topLevelArea = "") {
  const familyItem = familyByKey(familyKey);
  return familyItem?.image || FAMILY_IMAGE_FALLBACKS[familyKey] || GENERIC_IMAGE_URLS[familyKey] || TOP_LEVEL_AREAS.find((areaItem) => areaItem.key === topLevelArea)?.image || "";
}

export function resolveProductLibraryImage({ product = null, range = null, family = null, familyKey = "", categoryKey = "", areaKey = "", exactProductImage = "", exactRangeImage = "" } = {}) {
  const entity = product?.metadata?.productEntity || product || {};
  const familyItem = family || familyByKey(familyKey || entity.familyKey || product?.familyKey);
  const resolvedFamilyKey = familyKey || familyItem?.familyKey || entity.familyKey || product?.familyKey || "";
  const productImage = exactProductImage || firstPresent(entity, PRODUCT_IMAGE_FIELDS) || firstPresent(product || {}, PRODUCT_IMAGE_FIELDS);
  if (productImage) return productImage;
  const rangeImage = exactRangeImage || range?.image || range?.imageUrl || firstPresent(entity, RANGE_IMAGE_FIELDS) || firstPresent(product || {}, RANGE_IMAGE_FIELDS);
  if (rangeImage) return rangeImage;
  const familyImage = familyItem?.image || FAMILY_IMAGE_FALLBACKS[resolvedFamilyKey];
  if (familyImage) return familyImage;
  if (categoryKey && FAMILY_IMAGE_FALLBACKS[categoryKey]) return FAMILY_IMAGE_FALLBACKS[categoryKey];
  if (areaKey && FAMILY_IMAGE_FALLBACKS[areaKey]) return FAMILY_IMAGE_FALLBACKS[areaKey];
  return "";
}
...
function imageForCategory(topLevelArea, categoryName) {
  ...
  const byCategory = {
    ...
    fridge: GENERIC_IMAGE_URLS.fridge,
    fridges: GENERIC_IMAGE_URLS.fridge,
    lighting: GENERIC_IMAGE_URLS.lighting,
