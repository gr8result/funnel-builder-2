...
export function ensureBuilderCladdingEnablements(products = [], builderProducts = [], organisationId = "") {
  ...
  const candidateCodes = new Set(candidates.map((product) => product.productCode));
  const existingCladdingRefs = builderProducts.filter((item) => item.organisationId === organisationId && candidateCodes.has(item.masterProductCode));
  if (existingCladdingRefs.length === candidates.length && candidates.length > 0) return builderProducts;
  const existingCodes = new Set(existingCladdingRefs.map((item) => item.masterProductCode));
  return [
    ...builderProducts,
    ...candidates
      .filter((product) => !existingCodes.has(product.productCode))
