...
export function mergeMasterCatalogueProducts(baselineProducts = [], storedProducts = []) {
  ...
  return Array.from(byProductCode.values()).filter((product) => !isRemovedDuplicateCladdingProduct(product));
}
...
export function ensureDemoBuilderBrickEnablements(products = [], builderProducts = [], organisationId = DEMO_BUILDER_ORGANISATION_ID) {
  ...
  const candidates = activeQldBrickMasterProducts(products);
  const candidateCodes = new Set(candidates.map((product) => product.productCode));
  const existingBrickRefs = builderProducts.filter((item) => item.organisationId === organisationId && candidateCodes.has(item.masterProductCode));
  if (existingBrickRefs.length) return builderProducts;
  return [
    ...builderProducts,
    ...candidates.map((product) => createBuilderProductReference(product, {
      organisationId,
      enabled: true,
      active: true,
      tier: "",
...
export function ensureDemoBuilderRoofingEnablements(products = [], builderProducts = [], organisationId = DEMO_BUILDER_ORGANISATION_ID) {
  if (organisationId !== DEMO_BUILDER_ORGANISATION_ID) return builderProducts;
  const candidates = activeAuMetalRoofingMasterProducts(products);
  const candidateCodes = new Set(candidates.map((product) => product.productCode));
  const existingRoofingRefs = builderProducts.filter((item) => item.organisationId === organisationId && candidateCodes.has(item.masterProductCode));
  if (existingRoofingRefs.length === candidates.length && candidates.length > 0) return builderProducts;
  const existingCodes = new Set(existingRoofingRefs.map((item) => item.masterProductCode));
  return [
    ...builderProducts,
    ...candidates
      .filter((product) => !existingCodes.has(product.productCode))
...
export function ensureDemoBuilderExteriorOpeningEnablements(products = [], builderProducts = [], organisationId = DEMO_BUILDER_ORGANISATION_ID) {
  if (organisationId !== DEMO_BUILDER_ORGANISATION_ID) return builderProducts;
  const candidates = activeExteriorOpeningMasterProducts(products);
  const candidateCodes = new Set(candidates.map((product) => product.productCode));
  const existingOpeningRefs = builderProducts.filter((item) => item.organisationId === organisationId && candidateCodes.has(item.masterProductCode));
  if (existingOpeningRefs.length === candidates.length && candidates.length > 0) return builderProducts;
  const existingCodes = new Set(existingOpeningRefs.map((item) => item.masterProductCode));
  return [
    ...builderProducts,
    ...candidates
      .filter((product) => !existingCodes.has(product.productCode))
...
export function ensureDemoBuilderExteriorFinishEnablements(products = [], builderProducts = [], organisationId = DEMO_BUILDER_ORGANISATION_ID) {
  if (organisationId !== DEMO_BUILDER_ORGANISATION_ID) return builderProducts;
  const candidates = activeExteriorFinishMasterProducts(products);
  const candidateCodes = new Set(candidates.map((product) => product.productCode));
  const existingFinishRefs = builderProducts.filter((item) => item.organisationId === organisationId && candidateCodes.has(item.masterProductCode));
  if (existingFinishRefs.length === candidates.length && candidates.length > 0) return builderProducts;
  const existingCodes = new Set(existingFinishRefs.map((item) => item.masterProductCode));
  return [
    ...builderProducts,
    ...candidates
      .filter((product) => !existingCodes.has(product.productCode))
...
export function ensureBuilderCladdingEnablements(products = [], builderProducts = [], organisationId = "") {
  if (!organisationId) return builderProducts;
  const candidates = activeCladdingMasterProducts(products);
  const candidateCodes = new Set(candidates.map((product) => product.productCode));
  const existingCladdingRefs = new Map(
    builderProducts
  ...
  if (existingCladdingRefs.size === candidates.length && candidates.length > 0) return repairedBuilderProducts;
  return [
    ...repairedBuilderProducts,
    ...candidates
      .filter((product) => !existingCladdingRefs.has(product.productCode))
      .map((product) => createBuilderProductReference(product, {
        organisationId,
        enabled: true,
        active: true,
...
export function ensureDemoBuilderKitchenEnablements(products = [], builderProducts = [], organisationId = DEMO_BUILDER_ORGANISATION_ID) {
  if (organisationId !== DEMO_BUILDER_ORGANISATION_ID) return builderProducts;
  const candidates = activeKitchenMasterProducts(products);
  const candidateCodes = new Set(candidates.map((product) => product.productCode));
  const existingKitchenRefs = builderProducts.filter((item) => item.organisationId === organisationId && candidateCodes.has(item.masterProductCode));
  if (existingKitchenRefs.length === candidates.length && candidates.length > 0) return builderProducts;
  const existingCodes = new Set(existingKitchenRefs.map((item) => item.masterProductCode));
  return [
    ...builderProducts,
    ...candidates
      .filter((product) => !existingCodes.has(product.productCode))
