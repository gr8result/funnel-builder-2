...
const PRICE_STATUSES = new Set(["current", "allowance_only", "quote_required", "price_pending", "expired"]);
...
function createImportModelRow(row) {
  ...
  const familyItem = familyByKey(familyKey);
  const productSpecific = isIdentifiableProductRow(row);
...
function createCatalogueProductFromImportRow(row, organisationId) {
  ...
  return {
    ...
    linkedQuoteItemCode: row.quoteItemCode || `approved-family:${row.familyKey}`,
    requirementKey: row.requirementKey,
    familyKey: row.familyKey,
    sourceRow: row.sourceRow,
