...
const FAMILY_ATTRIBUTE_RULES = {
  bricks: { relevant: ["range", "colour", "texture", "finish", "dimensions"], aliases: { brickName: "productName" } },
  ovens: { relevant: ["model", "size", "finish", "configuration"], aliases: { width: "size" } },
  "metal-roofing": { relevant: ["material", "profile", "range", "colour", "finish"] },
...
export function normalizeMasterProductRecord(input = {}) {
  const familyKey = input.familyKey || input.family_key || input.product_family || "";
...
function knownFamilySuggestion(familyKey) {
  ...
  if (text === "roofing") return "metal-roofing";
...
function fallbackFamilyKey(row) {
  ...
  if (category.includes("roof")) return "metal-roofing";
