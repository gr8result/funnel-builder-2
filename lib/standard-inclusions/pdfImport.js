    ...
    if (!pageResult || !pageResult.objects.length) {
      ...
      if (fallback.imageRef) {
        objects.push(createObject("image", {
          ...
          data: { imageRef: fallback.imageRef, alt: `PDF page ${pageNumber}`, fixedVisual: true, source: "original-pdf-page-render", importConfidence: "preserved" },
        }));
      }
      ...
      if (ocrResult.textObjects.length) {
        ...
        warnings.push(`Page ${pageNumber} preserved as original artwork. Text could not be extracted reliably.`);
      }
      pageReports.push({
        pageNumber,
        ...
        warnings: ocrResult.textObjects.length
          ...
          : ["Page preserved as original artwork. Text could not be extracted reliably."],
      });
      continue;
    }
...
// pdf.js's real operator-list args for setFillRGBColor/setFillGray/etc. are
// already 0-255 integers (confirmed against production pdf.js 3.11.174 —
// e.g. a real fill of [240, 237, 236] is the colour #f0edec, not near-black),
// not the 0-1 floats an earlier version of this file assumed. That mismatch
// silently posterised almost every non-black fill toward white/cyan/magenta
// (any channel > 1 was multiplied by 255 and clamped straight to 255) —
// the root cause of the reported "large dark block" / wrong page colours.
// Detect the scale per call so this is correct regardless of exactly which
// convention a given pdf.js build uses, rather than hard-coding one.
function clamp255(value) {
  const number = Number(value) || 0;
  const scaled = Math.abs(number) > 1 ? number : number * 255;
...
// CMYK operator args are conventionally 0-1 in PDF content streams; normalise
// defensively in case a given pdf.js build also pre-scales these to 0-255
// (as it does for RGB — see clamp255 above), rather than assuming.
function normaliseUnitComponent(value) {
  const number = Number(value) || 0;
  return Math.abs(number) > 1 ? number / 255 : number;
}

function cmykToHex(c, m, y, k) {
  const cc = normaliseUnitComponent(c);
  const mm = normaliseUnitComponent(m);
  const yy = normaliseUnitComponent(y);
  const kk = normaliseUnitComponent(k);
  return rgbToHex((1 - cc) * (1 - kk) * 255, (1 - mm) * (1 - kk) * 255, (1 - yy) * (1 - kk) * 255);
}
...
async function walkOperatorList({ lib, page, viewport, upload, imageCache }) {
  ...
  // shadingFill (gradient/pattern fills across the current clip region) has
  // no faithful native representation here — its colours come from a shading
  // function this walk doesn't resolve. Rather than silently drop that
  // artwork (the earlier cause of "missing design content"), pages that use
  // it are routed to hybrid import so the real pdf.js canvas renderer (which
  // does support shading) supplies a faithful background. See
  // maybeCreateHybridPage.
  let shadingFillCount = 0;
  ...
  return { showTextColours, shapes, images, droppedComplexPaths, operatorCounts, shadingFillCount };
}
  ...
  walk.shapes.forEach((shape) => {
    ...
    if (shape.kind === "line") {
      ...
      objects.push({
        ...createObject("divider", {
          ...
          data: { orientation, source: "pdf-vector", importConfidence: "editable" },
        }),
    ...
    objects.push({
      ...createObject("shape", {
        ...
        data: { shapeKind: shape.kind, source: "pdf-vector", importConfidence: "editable" },
      }),
      _sequence: shape.sequence,
    });
  });
  ...
  walk.images.forEach((image) => {
    objects.push({
      ...createObject("image", {
        ...
        style: { objectFit: "cover" },
        data: { imageRef: image.imageRef, alt: "Imported PDF image", source: "pdf-image-xobject", importConfidence: "needs-review" },
      }),
...
function validateImportedPage({ objects, width, height, rawTextItems, walk }) {
  ...
  const shadingFillCount = walk.shadingFillCount || 0;
  if (shadingFillCount > 0) warnings.push(`${shadingFillCount} gradient/shading fill(s) cannot be reconstructed natively and require hybrid import.`);

  const failed = Boolean(pageCoveringDarkShapes.length || overflowObjects.length || tinyTextObjects.length || unsupportedImageOps > 0 || walk.droppedComplexPaths > 16 || shadingFillCount > 0);
  const scorePenalty = warnings.length * 9 + Math.min(35, walk.droppedComplexPaths * 1.5) + (unsupportedImageOps ? 22 : 0) + (shadingFillCount ? 18 : 0);
  return {
    failed,
    warnings,
    sourceTextCount,
...
function createHybridObjectsFromPageResult({ pageResult, fallback, pageNumber }) {
  ...
  if (fallback.imageRef) {
    objects.push(createObject("image", {
      ...
      locked: true,
      style: { objectFit: "cover" },
      data: { imageRef: fallback.imageRef, alt: `PDF page ${pageNumber}`, fixedVisual: true, importMode: "hybrid-background", source: "original-pdf-page-render", importConfidence: "preserved" },
      layer: 0,
    }));
  }
  const textObjects = pageResult.objects
    ...
    .map((object, index) => createObject("text", {
      ...
      data: {
        ...
        edited: false,
        hybridOverlay: true,
        source: object.data?.source || "pdf-text-layer",
        importConfidence: "editable",
      },
      style: {
        ...(object.style || {}),
...
async function attemptOcrFallback({ fallback, page, ocr }) {
  if (!ocr) return { textObjects: [], averageConfidence: 0 };
