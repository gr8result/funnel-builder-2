...
export async function importPdfAsStandardDocumentPreview(file, {
  ...
  for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
    ...
    const shapeCount = pageResult.objects.filter((object) => object.type === "shape" || object.type === "line").length;
...
async function walkOperatorList({ lib, page, viewport, upload, imageCache }) {
  ...
  for (let i = 0; i < opList.fnArray.length; i += 1) {
    ...
    try {
      if (opName === "save") {
        ...
        imagePlacements.push({ sequence: sequence += 1, objId, box });
  ...
  for (const placement of imagePlacements) {
    ...
    try {
      const imageRef = await resolveAndUploadImage({ page, objId: placement.objId, upload, imageCache });
  ...
  return { showTextColours, shapes, images, droppedComplexPaths };
}
...
async function extractPdfPage({ lib, page, upload, imageCache, fontSubstitutions }) {
    ...
    if (shape.kind === "line") {
      ...
      objects.push({
        ...createObject("divider", {
          ...
          style: { color: shape.stroke !== "transparent" ? shape.stroke : shape.fill, thickness: Math.max(1, Math.round(shape.box.height)) },
          data: {},
        }),
  ...
  return { width, height, objects, droppedComplexPaths: walk.droppedComplexPaths };
}

async function renderPageAsFallbackImage({ page, upload }) {
  const rotation = page.rotate || 0;
  const cssViewport = page.getViewport({ scale: CSS_PIXELS_PER_POINT, rotation });
  const renderViewport = page.getViewport({ scale: FALLBACK_RENDER_SCALE, rotation });
  const canvas = document.createElement("canvas");
  ...
  return { width: Math.round(cssViewport.width), height: Math.round(cssViewport.height), imageRef, canvas, renderScale: FALLBACK_RENDER_SCALE };
}
