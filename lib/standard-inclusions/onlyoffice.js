...
export const STANDARD_INCLUSIONS_TEMPLATE_SCOPES = {
...
/**
 * @typedef {"system-base" | "organisation" | "user"} TemplateScope
 */
...
export function onlyOfficeAppInternalUrl(req = null) {
  const configured = String(process.env.ONLYOFFICE_APP_INTERNAL_URL || "").trim().replace(/\/$/, "");
  if (configured) return configured;
  return appBaseUrl(req);
}

export function appPublicUrl(req = null) {
...
export async function createStandardInclusionsOnlyOfficeDocument({
  ...
  fileType,
  editorMode,
  templateScope = STANDARD_INCLUSIONS_TEMPLATE_SCOPES.ORGANISATION,
  sourceType = "pptx-upload",
  allowedEditorUserIds = [],
}) {
  const now = new Date().toISOString();
  const documentId = id || createOnlyOfficeId("std-inclusions");
  const resolvedStoragePath = officeStoragePath || pptxStoragePath;
  const resolvedFileType = normaliseOnlyOfficeFileType(fileType || sourceFileName || resolvedStoragePath);
  const resolvedEditorMode = editorMode || editorModeForOnlyOfficeFileType(resolvedFileType);
  const record = {
    id: documentId,
    ...
    version: 1,
    source_file_name: sourceFileName || `Standard Inclusions.${resolvedFileType}`,
    source_type: sourceType,
    current_pptx_asset_id: resolvedStoragePath,
    ...
    revision_history: [{
      version: 1,
      action: sourceType,
      pptxAssetId: resolvedStoragePath,
      officeAssetId: resolvedStoragePath,
    ...
    metadata: {
      ...
      fileType: resolvedFileType,
      templateScope,
      currentOfficeAssetId: resolvedStoragePath,
    },
  };
  const { data, error } = await supabaseAdmin
    .from(STANDARD_INCLUSIONS_TABLE)
...
export async function appendStandardInclusionsRevision({ document, sourceStoragePath, action, actorUserId, extraMetadata = {} }) {
  ...
  await uploadStandardInclusionsAsset(
    storagePath,
    sourceBytes,
    onlyOfficeContentType(fileType),
    false,
  );
  const now = new Date().toISOString();
  const revision = {
    ...
    sourceOfficeAssetId: sourceStoragePath,
    fileType,
    editorMode: editorModeForOnlyOfficeFileType(fileType),
    userId: actorUserId,
    createdAt: now,
    ...extraMetadata,
  };
...
export function buildOnlyOfficeEditorConfig({ document, user, req }) {
  ...
  const baseUrl = onlyOfficeAppInternalUrl(req);
...
export function standardInclusionsDocumentFileType(document) {
  const metadataType = normaliseOnlyOfficeFileType(document?.metadata?.fileType || "");
  if (metadataType) return metadataType;
  return normaliseOnlyOfficeFileType(document?.source_file_name || document?.current_pptx_asset_id || "pptx") || "pptx";
}

export function normaliseOnlyOfficeFileType(value = "") {
  const text = String(value || "").toLowerCase();
  if (text === "docx" || text.endsWith(".docx") || text.includes("wordprocessingml")) return "docx";
  if (text === "pptx" || text.endsWith(".pptx") || text.includes("presentationml")) return "pptx";
  if (text === "pdf" || text.endsWith(".pdf") || text.includes("application/pdf")) return "pdf";
  return "";
}

export function onlyOfficeDocumentType(fileType = "pptx") {
  return fileType === "docx" ? "word" : "slide";
}

export function editorModeForOnlyOfficeFileType(fileType = "pptx") {
  if (fileType === "docx") return STANDARD_INCLUSIONS_EDITOR_MODES.ONLYOFFICE_DOCX;
  if (fileType === "pdf") return STANDARD_INCLUSIONS_EDITOR_MODES.FINISHED_PDF;
  return STANDARD_INCLUSIONS_EDITOR_MODES.ONLYOFFICE_PPTX;
}

export function onlyOfficeContentType(fileType = "pptx") {
