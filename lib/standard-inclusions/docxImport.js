...
async function extractDocumentFlow({ zip, documentXml, styles, numbering, relationships, headers, footers, upload, fontSubstitutions, warnings }) {
  ...
  for (const child of Array.from(body?.childNodes || [])) {
    ...
    if (child.localName === "p") {
      const paragraphBlocks = await paragraphToBlocks({ zip, paragraph: child, styles, numbering, relationships, upload, fontSubstitutions, warnings });
...
async function paragraphToBlocks({ zip, paragraph, styles, numbering, relationships, upload, fontSubstitutions, warnings }) {
  ...
  for (const run of childrenByLocalName(paragraph, "r")) {
    ...
    for (const blip of images) {
      ...
      const image = await imageBlockFromRelationship({ zip, rel, run, upload, warnings });
...
async function imageBlockFromRelationship({ zip, rel, run, upload, warnings }) {
...
async function defaultUploadAsset(dataUrl) {
  const response = await fetch("/api/standard-inclusions/docx-import/upload-asset", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ dataUrl }),
  });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok || !payload?.ok) throw new Error(payload?.error || "Failed to upload an extracted DOCX image.");
  return payload.url;
}
