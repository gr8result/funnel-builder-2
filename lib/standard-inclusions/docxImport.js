...
const FRAGMENT_TEXT_LENGTH_WARNING = 8;
const FRAGMENTED_TEXT_WARNING_RATIO = 0.35;
...
export async function importDocxAsStandardDocumentPreview(file, {
  ...
  const validation = validateImportedComposition({ structure, pages, layoutMode, warnings });
...
function validateImportedComposition({ structure = {}, pages = [], layoutMode = "flow", warnings = [] }) {
  const pageValidation = validateImportedPageCount(structure.sourcePageCount, pages.length, layoutMode, warnings);
  const textObjects = pages.flatMap((page) => page.objects || []).filter((object) => object.type === "text" && !object.data?.docxRepeated);
  const tinyTextObjects = textObjects.filter((object) => String(object.data?.text || "").trim().length > 0 && String(object.data?.text || "").trim().length <= FRAGMENT_TEXT_LENGTH_WARNING);
  const tinyRatio = textObjects.length ? tinyTextObjects.length / textObjects.length : 0;
  if (textObjects.length >= 40 && tinyRatio > FRAGMENTED_TEXT_WARNING_RATIO) {
    warnings.push(`DOCX import appears fragmented: ${tinyTextObjects.length} of ${textObjects.length} text objects are tiny fragments.`);
  }
  const offPageElements = pages.flatMap((page, pageIndex) => (page.objects || []).filter((object) => {
    const right = Number(object.x || 0) + Number(object.width || 0);
    const bottom = Number(object.y || 0) + Number(object.height || 0);
    return right < -16 || bottom < -16 || Number(object.x || 0) > Number(page.width || A4_WIDTH) + 16 || Number(object.y || 0) > Number(page.height || A4_HEIGHT) + 16;
  }).map((object) => ({ pageIndex, objectId: object.id })));
  if (offPageElements.length) warnings.push(`${offPageElements.length} DOCX positioned element${offPageElements.length === 1 ? "" : "s"} landed outside the page bounds.`);
  return {
    ...pageValidation,
    tinyTextObjectCount: tinyTextObjects.length,
    textObjectCount: textObjects.length,
    offPageElementCount: offPageElements.length,
    fragmentedTextWarning: textObjects.length >= 40 && tinyRatio > FRAGMENTED_TEXT_WARNING_RATIO,
  };
}
...
async function extractDocumentFlow({ zip, documentXml, styles, numbering, relationships, headers, footers, upload, onProgress, fontSubstitutions, warnings }) {
  ...
  for (const child of Array.from(body?.childNodes || [])) {
    ...
    if (child.localName === "p") {
      ...
      const paragraphBlocksRaw = await paragraphToBlocks({ zip, paragraph: child, styles, numbering, relationships, upload, onProgress, fontSubstitutions, warnings });
      const paragraphBlocks = paragraphFixedElements.length && paragraphBlocksRaw.length === 1 && paragraphBlocksRaw[0].type === "spacer"
        ? []
        : paragraphBlocksRaw;
      paragraphBlocks.forEach((block) => {
        block.sourceOrder = currentOrder;
        block.sourcePageIndex = pageIndex;
      });
      blocks.push(...paragraphBlocks);
      const pageBreaks = paragraphBlocks.filter((block) => block.type === "pageBreak").length;
      if (pageBreaks) hasExplicitPageBoundary = true;
      pageIndex += pageBreaks;
    } else if (child.localName === "tbl") {
      const currentOrder = sourceOrder++;
      const tableBlock = tableToFlowBlock(child, styles, numbering, fontSubstitutions);
      tableBlock.sourceOrder = currentOrder;
      tableBlock.sourcePageIndex = pageIndex;
      blocks.push(tableBlock);
    } else if (child.localName === "sectPr") {
      pageSettings = sectionSettings(child);
    }
  }
  return { blocks, fixedElements, pageSettings, header, footer, hasExplicitPageBoundary, sourceOrderCount: sourceOrder };
}

async function fixedElementsFromParagraph({ zip, paragraph, relationships, upload, onProgress, warnings, pageIndex, styles, numbering, fontSubstitutions, sourceOrder = 0 }) {
  const elements = [];
...
function layoutDocxFixedPages({ flow, structure, warnings }) {
  ...
  const useSourceOrderPages = sourcePageCount > 1 && !flow.hasExplicitPageBoundary && (flow.fixedElements?.length || 0) >= sourcePageCount;
  let pageIndex = 0;
  let y = margin.top;
  let flowIndex = 0;
  ...
  for (const block of flow.blocks || []) {
    ...
    const targetPageIndex = useSourceOrderPages
      ? sourceOrderToPageIndex(block.sourceOrder, flow.sourceOrderCount, sourcePageCount)
      : Math.max(0, Math.min(sourcePageCount - 1, Number(block.sourcePageIndex ?? pageIndex) || 0));
    if (targetPageIndex !== pageIndex) {
      pageIndex = targetPageIndex;
      y = nextFlowYForPage(pageObjects[pageIndex], margin.top);
    }
  ...
  const fixedElements = flow.fixedElements || [];
  for (let fixedIndex = 0; fixedIndex < fixedElements.length; fixedIndex += 1) {
    const fixed = fixedElements[fixedIndex];
    const targetPage = useSourceOrderPages
      ? sourceOrderToPageIndex(fixedIndex, fixedElements.length, sourcePageCount)
      : Math.max(0, Math.min(sourcePageCount - 1, Number(fixed.pageIndex || 0)));
    const object = fixedElementToObject({ ...fixed, pageIndex: targetPage }, settings);
...
function sourceOrderToPageIndex(sourceOrder = 0, sourceOrderCount = 1, sourcePageCount = 1) {
  const total = Math.max(1, Number(sourceOrderCount || 1));
  const pages = Math.max(1, Number(sourcePageCount || 1));
  const index = Math.floor((Math.max(0, Number(sourceOrder || 0)) / total) * pages);
  return Math.max(0, Math.min(pages - 1, index));
}

function nextFlowYForPage(objects = [], fallbackY = DEFAULT_MARGIN.top) {
  const flowObjects = objects.filter((object) => object.data?.docxFixedFlowBlock);
  if (!flowObjects.length) return fallbackY;
  return Math.max(fallbackY, ...flowObjects.map((object) => Number(object.y || 0) + Number(object.height || 0) + 8));
}
