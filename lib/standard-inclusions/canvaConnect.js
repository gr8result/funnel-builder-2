...
export async function storeCanvaExportedPdf({ workspaceId, userId, documentId, designId, exportUrl, versionNumber }) {
  ...
  if (!buffer.toString("latin1", 0, 5).startsWith("%PDF-")) {
    const error = new Error("Canva export download was not a valid PDF.");
    error.code = "CANVA_EXPORT_INVALID_PDF";
    error.statusCode = 502;
    throw error;
  }
  const pageCount = countPdfPages(buffer);
  if (!pageCount) {
    const error = new Error("Canva exported PDF has zero pages.");
    error.code = "CANVA_EXPORT_EMPTY_PDF";
    error.statusCode = 502;
    throw error;
  }
