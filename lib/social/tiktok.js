...
function ensureTikTokOk(res, data, fallbackMessage) {
  if (data?.error?.code === 'scope_not_authorized' || /required scope/i.test(String(data?.error?.message || ''))) {
    const error = new Error('TikTok connection is missing publish permission. Reconnect TikTok and approve posting access.');
    error.code = data?.error?.code || 'scope_not_authorized';
    throw error;
  }
...
export async function refreshTikTokAccountAccess({ admin, userId, socialAccountId }) {
  ...
  if (tokenErr || !tokenRow?.encrypted_refresh_token) {
