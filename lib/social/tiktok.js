...
function ensureTikTokOk(res, data, fallbackMessage) {
  ...
  if (data?.error?.code === 'unaudited_client_can_only_post_to_private_accounts') {
    const error = new Error('TikTok direct publishing is blocked until the TikTok app passes Content Posting API audit, or the connected TikTok account allows private-only posting during audit.');
