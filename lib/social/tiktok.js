...
function ensureTikTokOk(res, data, fallbackMessage) {
  ...
  if (data?.error?.code === 'unaudited_client_can_only_post_to_private_accounts') {
    throw new Error('TikTok direct publishing is blocked for this account until the TikTok app passes Content Posting API audit or the connected TikTok account is private.');
  }
