...
const TIKTOK_POSTING_SCOPES = ['video.publish', 'video.upload'];
...
export function hasTikTokPostingScope(scopeValue) {
  ...
  return TIKTOK_POSTING_SCOPES.some((scope) => granted.has(scope));
}

export function assertTikTokPostingScope(scopeValue) {
  if (!scopeValue || hasTikTokPostingScope(scopeValue)) return;

  const error = new Error('TikTok did not grant posting access. Reconnect TikTok and approve video publishing/upload access.');
...
function ensureTikTokOk(res, data, fallbackMessage) {
  if (data?.error?.code === 'scope_not_authorized' || /required scope/i.test(String(data?.error?.message || ''))) {
    const error = new Error('TikTok connection is missing posting permission. Reconnect TikTok and approve video publishing/upload access.');
    error.code = 'scope_not_authorized';
    throw error;
  }

  if (data?.error?.code === 'unaudited_client_can_only_post_to_private_accounts') {
    const error = new Error('TikTok direct publishing is blocked for this account until the TikTok app passes Content Posting API audit or the connected TikTok account is private.');
