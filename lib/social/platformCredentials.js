...
async function selectGlobalCredentialRow(admin, platform) {
  try {
    const { data, error } = await admin
      .from("social_app_credentials")
      .select("app_id, encrypted_secret, secret_iv, secret_tag, config_id, updated_at")
      .eq("platform", platform)
      .not("app_id", "is", null)
    ...
    const { data, error: fallbackError } = await admin
      .from("social_app_credentials")
      .select("app_id, encrypted_secret, secret_iv, secret_tag, updated_at")
      .eq("platform", platform)
      .not("app_id", "is", null)
      .order("updated_at", { ascending: false })
      .limit(1)
...
function buildResolvedCredentials(data, hasConfigId, fallbackAppId, fallbackAppSecret, fallbackConfigId) {
...
export async function getPlatformCredentials(admin, userId, platform) {
  ...
  if (userId && admin) {
    try {
      ...
      const resolved = buildResolvedCredentials(data, hasConfigId, fallbackAppId, fallbackAppSecret, fallbackConfigId);
