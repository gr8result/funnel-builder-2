...
export async function addToList({ list_id, subscriber_id }) {
  ...
  // ✅ Check 500-subscriber per-list limit
  const { count: listSize, error: countErr } = await supabaseAdmin
    .from("list_subscribers")
    .select("*", { count: "exact" })
    .eq("list_id", list_id);

  if (!countErr && listSize >= 500) {
    const err = new Error("List is full (500 subscriber limit reached)");
    err.code = "LIST_LIMIT_EXCEEDED";
    err.details = { limit: 500, current: listSize };
    throw err;
  }
