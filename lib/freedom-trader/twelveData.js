...
export function classifyTwelveDataError(message) {
...
export async function fetchTwelveDataHistoryBatch({ symbols = [], range = "1y", interval = null }) {
  ...
  if (!result.ok) {
    entries.forEach(({ original, mapped }) => {
      output.set(original, emptyTwelveDataHistory({ symbol: mapped, range: normalizedRange, interval: normalizedInterval, error: result.error }));
    });
    return output;
  }
