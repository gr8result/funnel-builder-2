...
function emptyTwelveDataHistory({ symbol, range, interval, error }) {
  const errorCode = classifyTwelveDataError(error);
  return {
    ok: false,
    symbol,
    range,
  ...
  try {
    ...
    if (!payload) {
      return {
        ...
        payload: null,
        error: "Twelve Data returned a malformed response.",
        errorCode: "malformed-provider-response",
        retryAfterMs,
      };
    }
    if (!response.ok || payload?.status === "error") {
      return {
        ...
        errorCode: classifyTwelveDataError(payload?.message || payload?.code || "", response.status),
        retryAfterMs,
      };
    }
    return { ok: true, status: response.status, payload, error: null, errorCode: null, retryAfterMs: null };
  } catch (error) {
    ...
    return {
      ...
      errorCode: classifyTwelveDataError(error.message || "", 0),
      retryAfterMs: null,
    };
  }
}
...
function mapTimeSeriesPayload({ symbol, range, interval, payload }) {
  ...
  const candles = values
    .map((item) => {
      const timestamp = Math.floor(Date.parse(String(item.datetime || "").replace(" ", "T")) / 1000);
...
export async function fetchTwelveDataHistory({ symbol, range = "1y", interval = null, exchange = "" }) {
  ...
  if (!result.ok) {
    return {
      ...
      errorCode: result.errorCode || classifyTwelveDataError(result.error, result.status),
...
function isPlanRestrictionError(message) {
  return /available (starting with|only) (the )?(pro|venture|grow|ultra)/i.test(String(message || ""))
    || /upgrad(e|ing)/i.test(String(message || ""));
}

export function classifyTwelveDataError(message, status = null) {
  if (Number(status) === 401) return "auth-required";
  if (Number(status) === 403) return "permission-denied";
  if (Number(status) === 429) return "rate-limited";
  if (isPlanRestrictionError(message)) return "plan-restricted";
  if (/current limit being 8|api credits for the current minute|per-minute/i.test(String(message || ""))) return "rate-limited";
  if (/timeout|timed out|abort/i.test(String(message || ""))) return "timeout";
  if (/malformed|invalid json|could not parse/i.test(String(message || ""))) return "malformed-provider-response";
  return "error";
}
...
export async function fetchTwelveDataHistoryBatch({ symbols = [], range = "1y", interval = null }) {
  ...
  const entries = symbols
    .map((original) => ({ original, mapped: mapTwelveDataSymbol(original) }))
  ...
  entries.forEach(({ original, mapped }) => {
    const sub = result.payload?.[mapped];
    if (!sub || sub.status === "error") {
      output.set(original, emptyTwelveDataHistory({
        symbol: mapped,
        range: normalizedRange,
        interval: normalizedInterval,
        error: sub?.message || "Twelve Data did not return data for this symbol in the batch.",
      }));
      return;
    }
    output.set(original, trimHistoryToRange(mapTimeSeriesPayload({
