...
const requestLog = globalThis.__freedomTwelveDataRequestLog || [];
globalThis.__freedomTwelveDataRequestLog = requestLog;
...
export function resetTwelveDataRequestLog() {
  requestLog.length = 0;
}

export function getTwelveDataRequestLog() {
...
async function fetchTwelveData(path, params) {
  const key = apiKey();
  const endpoint = `${TWELVE_DATA_BASE_URL}/${path}`;
  if (!key) {
    return {
      ok: false,
      status: 0,
  ...
  const symbols = String(params?.symbol || "").split(",").map((item) => item.trim()).filter(Boolean);
  const estimatedCredits = Math.max(1, symbols.length || 1);
  ...
  try {
    ...
    requestLog.push({
      provider: "Twelve Data",
      path,
      endpoint: safeUrl.toString(),
      symbols,
      estimatedCredits,
      creditsUsed: creditsUsed === null ? null : Number(creditsUsed),
      creditsLeft: creditsLeft === null ? null : Number(creditsLeft),
      httpStatus: response.status,
      providerStatus: payload?.status || (response.ok ? "ok" : "error"),
      providerCode: payload?.code || null,
      providerMessage: payload?.message || payload?.code || null,
      retryAfterMs,
      startedAt,
    ...
    if (!response.ok || payload?.status === "error") {
      return {
        ...
        endpoint,
        keySupplied: true,
        providerCode: payload?.code || response.status,
    ...
    return { ok: true, status: response.status, payload, error: null, endpoint, keySupplied: true, retryAfterMs: null, creditsUsed, creditsLeft };
  } catch (error) {
...
function mapTimeSeriesPayload({ symbol, range, interval, payload }) {
  ...
  const dataLabel = classifyDataLabel(meta, latest?.date);
  const enrichedCandles = candles.map((candle) => ({
    ...candle,
    provider: "Twelve Data",
    delayed: dataLabel !== "Live",
    freshness: dataLabel,
  }));
  ...
  return {
    ...
    meta,
    firstTimestamp: enrichedCandles[0]?.date || null,
    latestTimestamp: enrichedCandles[enrichedCandles.length - 1]?.date || null,
    currentPrice: enrichedCandles[enrichedCandles.length - 1]?.close ?? null,
    error: null,
  };
}
...
export async function fetchTwelveDataHistory({ symbol, range = "1y", interval = null, exchange = "" }) {
  ...
  if (!result.ok) {
    return {
      ...
      keySupplied: result.keySupplied,
...
export async function fetchTwelveDataQuoteBatch({ symbols = [] }) {
  ...
  if (!result.ok) {
    entries.forEach(({ original, mapped }) => {
      output.set(original, { ok: false, symbol: mapped, provider: "Twelve Data", error: result.error, providerStatus: result.status, providerCode: result.providerCode, endpoint: result.endpoint, keySupplied: result.keySupplied, retryAfterMs: result.retryAfterMs || null, creditsUsed: result.creditsUsed, creditsLeft: result.creditsLeft });
    });
    return output;
  }
...
export async function fetchTwelveDataStocks(params = {}) {
  ...
  if (!result.ok) return { ok: false, symbols: [], error: result.error, status: result.status, providerCode: result.providerCode, endpoint: result.endpoint, keySupplied: result.keySupplied, creditsUsed: result.creditsUsed, creditsLeft: result.creditsLeft };
  return {
    ok: true,
