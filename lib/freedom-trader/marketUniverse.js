...
import { fetchSharedHistoryBatch } from "./marketDataService.js";
...
const DAILY_PRE_SCREEN_TTL_MS = Math.max(60 * 60 * 1000, Number(process.env.FREEDOM_DAILY_PRE_SCREEN_CACHE_TTL_MS) || 20 * 60 * 60 * 1000);
const DEFAULT_SCAN_TIMEOUT_MS = Math.max(30_000, Number(process.env.FREEDOM_SCAN_TIMEOUT_MS) || 120_000);

const referenceCache = globalThis.__freedomMarketUniverseReferenceCache || { fetchedAt: 0, rows: [], error: null };
const alpacaAssetCache = globalThis.__freedomAlpacaAssetCache || { fetchedAt: 0, assetsBySymbol: new Map(), error: null };
const dailyPreScreenCache = globalThis.__freedomDailyPreScreenCache || new Map();
globalThis.__freedomMarketUniverseReferenceCache = referenceCache;
globalThis.__freedomAlpacaAssetCache = alpacaAssetCache;
globalThis.__freedomDailyPreScreenCache = dailyPreScreenCache;
...
function broadQuoteLimit(settings = {}) {
  const configured = Number(settings.broadScreenLimit) || Number(process.env.FREEDOM_BROAD_SCREEN_LIMIT);
  if (Number.isFinite(configured) && configured > 0) return Math.max(1, configured);
  if (Array.isArray(settings.markets) && settings.markets.length === 1 && settings.markets[0] === "ASX") return 300;
  return selectMarketDataProvider("preScreenQuote").id === AlpacaProvider.id ? Number.POSITIVE_INFINITY : 64;
}

function detailedAnalysisLimit(settings = {}) {
  return Math.max(1, Number(settings.detailedAnalysisLimit) || Number(process.env.FREEDOM_DETAILED_ANALYSIS_LIMIT) || 100);
}
...
function providerSymbolFor(row) {
  const symbol = normalizeSymbol(row.symbol);
  const exchange = normalizeText(row.exchange).toUpperCase();
  if (!symbol) return "";
  if (exchange === "ASX" || exchange === "XASX") return `${symbol}:ASX`;
  return symbol;
}

function twelveDataConfigured() {
  return Boolean(process.env.TWELVE_DATA_API_KEY?.trim() || process.env.TWELVEDATA_API_KEY?.trim());
}
...
function mapReferenceRow(row) {
  ...
  return {
    ...
    market,
    providerSymbol: providerSymbolFor(row),
    active,
...
function symbolIsPlainUsTicker(symbol) {
  return /^[A-Z][A-Z0-9.-]{0,9}$/.test(normalizeSymbol(symbol));
}

export function namesCompatible(finnhubName = "", alpacaName = "") {
  const a = normalizeText(finnhubName).toLowerCase().replace(/\b(inc|corp|corporation|company|co|ltd|limited|common stock|class a|class b|ordinary shares)\b/g, "").replace(/[^a-z0-9]/g, "");
  const b = normalizeText(alpacaName).toLowerCase().replace(/\b(inc|corp|corporation|company|co|ltd|limited|common stock|class a|class b|ordinary shares)\b/g, "").replace(/[^a-z0-9]/g, "");
  if (!a || !b) return true;
  return a.includes(b.slice(0, Math.min(8, b.length))) || b.includes(a.slice(0, Math.min(8, a.length)));
}

async function loadAlpacaAssets() {
  if (alpacaAssetCache.assetsBySymbol.size && Date.now() - alpacaAssetCache.fetchedAt < REFERENCE_TTL_MS) {
    return { ok: true, cacheHit: true, assetsBySymbol: alpacaAssetCache.assetsBySymbol, error: alpacaAssetCache.error };
  }
  const result = await AlpacaProvider.assetUniverse().catch((error) => ({ ok: false, assetsBySymbol: new Map(), error: error.message }));
  if (!result.ok) {
    alpacaAssetCache.error = result.error || "Alpaca assets unavailable.";
    return { ok: false, cacheHit: false, assetsBySymbol: alpacaAssetCache.assetsBySymbol, error: alpacaAssetCache.error };
  }
  alpacaAssetCache.assetsBySymbol = result.assetsBySymbol || new Map();
  alpacaAssetCache.fetchedAt = Date.now();
  alpacaAssetCache.error = null;
  return { ok: true, cacheHit: false, assetsBySymbol: alpacaAssetCache.assetsBySymbol, error: null };
}

export function identityGate(rows = [], assetsBySymbol = new Map()) {
  const invalid = [];
  const valid = rows.map((row) => {
    if (row.market !== "US") return row;
    const asset = assetsBySymbol.get(row.symbol);
    const failures = [];
    if (!symbolIsPlainUsTicker(row.symbol)) failures.push("invalid symbol format");
    if (!asset) failures.push("missing Alpaca asset metadata");
    if (asset && asset.providerSymbol !== row.symbol) failures.push("provider symbol mismatch");
    if (asset && asset.assetClass !== "us_equity") failures.push("not a US equity asset");
    if (asset && asset.active !== true) failures.push("inactive Alpaca asset");
    if (asset && asset.tradable !== true) failures.push("not tradable in Alpaca");
    if (asset && !["NASDAQ", "NYSE", "AMEX", "ARCA", "BATS", "OTC"].includes(String(asset.exchange || "").toUpperCase())) failures.push("unsupported Alpaca exchange");
    if (asset && !namesCompatible(row.companyName, asset.name)) failures.push("company name mismatch");
    if (failures.length) {
      invalid.push({ symbol: row.symbol, companyName: row.companyName, reason: failures.join("; "), asset });
      return { ...row, identityValid: false, identityInvalidReason: failures.join("; "), alpacaAsset: asset || null };
    }
    return {
      ...row,
      identityValid: true,
      ...
      alpacaAssetName: asset.name,
      exchange: asset.exchange || row.exchange,
      active: asset.active,
...
export function resolveStockQueryFromRows(rows = [], query, options = {}) {
  ...
  const exactUs = exact.filter((row) => row.market === "US");
  const selected = exactUs.length === 1 ? exactUs : exact.length ? exact : candidates;
...
function marketCoverage(rows, usage, settings) {
  ...
  const usEligible = usSupported.filter((row) => row.identityValid !== false && row.tradable && !excludedBySettings(row, settings));
  const asxEligible = asxSupported.filter((row) => row.identityValid !== false && row.tradable && !excludedBySettings(row, settings));
  const asxUnavailable = null;
  return {
    US: {
...
function candidateUniverse(rows, settings, coverage) {
  ...
  const selected = String(settings.universeSelection || "").toUpperCase();
  const asxCap = selected === "ASX_200" ? 200 : selected === "ASX_300" ? 300 : null;
  const filtered = rows.filter((row) => {
    if (!markets.includes(row.market)) return false;
    if (row.market === "ASX" && coverage.ASX.unavailableReason) return false;
    if (row.identityValid === false) return false;
    return row.tradable && row.active !== false && !excludedBySettings(row, settings);
  }).sort((a, b) => {
  ...
  if (markets.length === 1 && markets[0] === "ASX" && asxCap) return filtered.slice(0, asxCap);
  return filtered;
}

function preScreenCacheKey(symbol, range, interval) {
  const day = new Date().toISOString().slice(0, 10);
  return `${day}:${normalizeSymbol(symbol)}:${range}:${interval}`;
}

function cachedPreScreenHistory(symbol, range, interval) {
  const cached = dailyPreScreenCache.get(preScreenCacheKey(symbol, range, interval));
  if (!cached || Date.now() > cached.expiresAt) return null;
  return { ...cached.history, cache: { ...(cached.history.cache || {}), preScreenHit: true, fetchedAt: cached.fetchedAt } };
}

function storePreScreenHistory(symbol, range, interval, history) {
  if (!history) return;
  dailyPreScreenCache.set(preScreenCacheKey(symbol, range, interval), {
    fetchedAt: new Date().toISOString(),
    expiresAt: Date.now() + DAILY_PRE_SCREEN_TTL_MS,
    history,
  });
}

export function priceSanity(history) {
  const candles = Array.isArray(history?.candles) ? history.candles : [];
  if (candles.length < 20) return { ok: false, statusCode: "DATA_UNAVAILABLE", reason: "Insufficient history for price sanity." };
  ...
  if (![latestClose, median, low, high].every(Number.isFinite) || latestClose <= 0) {
    return { ok: false, statusCode: "DATA_UNAVAILABLE", reason: "Invalid current price." };
  }
  const medianDiscontinuity = median > 0 ? Math.abs(latestClose - median) / median : 0;
  const outsideRange = latestClose > high * 1.75 || latestClose < low * 0.35;
  if (medianDiscontinuity > 1.5 || outsideRange) {
    return {
      ok: false,
      statusCode: "DATA_INVALID",
      reason: "Unexplained price discontinuity versus recent historical range.",
      latestClose,
      recentMedian: median,
      recentLow: low,
      recentHigh: high,
  ...
  return { ok: true, latestClose, recentMedian: median, recentLow: low, recentHigh: high };
}
...
function broadHistoryScore(row, history, settings = {}) {
  ...
  const minimumHistory = Math.max(20, Number(settings.broadMinimumHistoryBars) || 50);
  ...
  if (candles.length < minimumHistory) return { scored: null, unavailable: { symbol: row.symbol, statusCode: "INSUFFICIENT_HISTORY", reason: `Only ${candles.length} OHLCV bars returned.` } };
  const sanity = priceSanity(history);
  if (!sanity.ok) return { scored: null, unavailable: { symbol: row.symbol, statusCode: sanity.statusCode || "DATA_INVALID", reason: sanity.reason } };
  const latest = candles[candles.length - 1];
  ...
  if (!Number.isFinite(price) || price < minimumPrice) return { scored: null, unavailable: { symbol: row.symbol, statusCode: "PRICE_REJECTED", reason: "Price is below the configured threshold." } };
  if (!Number.isFinite(volume) || !Number.isFinite(avgVolume) || avgVolume < minimumVolume) return { scored: null, unavailable: { symbol: row.symbol, statusCode: "LIQUIDITY_REJECTED", reason: "Average volume is below the configured liquidity threshold." } };
  if (!Number.isFinite(volatility) || volatility > maximumVolatility) return { scored: null, unavailable: { symbol: row.symbol, statusCode: "VOLATILITY_REJECTED", reason: "Volatility is outside the configured scanner range." } };
  ...
  return {
    scored: {
      ...
      identity: {
        alpacaAssetId: row.alpacaAssetId || null,
        providerSymbol: row.providerSymbol || row.symbol,
        assetName: row.alpacaAssetName || null,
...
function scanDeadline(settings = {}) {
  const configured = Number(settings.scanTimeoutMs) || DEFAULT_SCAN_TIMEOUT_MS;
  return Number(settings.scanDeadlineMs) || Date.now() + configured;
}

function scanTimedOut(deadline) {
  return Number.isFinite(deadline) && Date.now() >= deadline;
}

function scanWasCancelled(settings = {}) {
  return Boolean(settings.abortSignal?.aborted || settings.cancelled?.());
}

function reportProgress(settings = {}, patch = {}) {
  if (typeof settings.onProgress === "function") {
    settings.onProgress({ elapsedMs: Date.now() - (Number(settings.scanStartedAtMs) || Date.now()), ...patch });
  }
}
...
async function broadQuoteScreen(candidates, settings, usage) {
  if (candidates.length && candidates.every((row) => row.market === "ASX")) {
    return broadAsxHistoryScreen(candidates, settings);
  }
  if (selectMarketDataProvider("preScreenQuote").id === AlpacaProvider.id && candidates.every((row) => row.market === "US")) {
    return broadAlpacaScreen(candidates, settings);
  }
  if (selectMarketDataProvider("preScreenQuote").id === AlpacaProvider.id && candidates.some((row) => row.market === "US")) {
    const us = await broadAlpacaScreen(candidates.filter((row) => row.market === "US"), settings);
    const asx = await broadQuoteScreen(candidates.filter((row) => row.market === "ASX"), settings, usage);
    return {
      provider: `Alpaca + ${asx.provider || "Twelve Data"}`,
      providerCalls: (us.providerCalls || 0) + (asx.providerCalls || 0),
      providerWaits: (us.providerWaits || 0) + (asx.providerWaits || 0),
      providerWaitMs: (us.providerWaitMs || 0) + (asx.providerWaitMs || 0),
      retries: (us.retries || 0) + (asx.retries || 0),
      quoteLimit: (us.quoteLimit || 0) + (asx.quoteLimit || 0),
      screenedCount: (us.screenedCount || 0) + (asx.screenedCount || 0),
      eligible: [...(us.eligible || []), ...(asx.eligible || [])],
      unavailable: [...(us.unavailable || []), ...(asx.unavailable || [])],
      budgetExhausted: Boolean(us.budgetExhausted || asx.budgetExhausted),
      budgetExhaustedReason: us.budgetExhaustedReason || asx.budgetExhaustedReason || null,
      pausedAtOffset: us.pausedAtOffset ?? asx.pausedAtOffset ?? null,
      symbolsRequested: (us.symbolsRequested || 0) + (asx.symbolsRequested || 0),
      barsReturned: (us.barsReturned || 0) + (asx.barsReturned || 0),
      cacheHits: (us.cacheHits || 0) + (asx.cacheHits || 0),
      freshFetches: (us.freshFetches || 0) + (asx.freshFetches || 0),
      batches: (us.batches || 0) + (asx.batches || 0),
      missingSymbols: (us.missingSymbols || 0) + (asx.missingSymbols || 0),
      elapsedMs: (us.elapsedMs || 0) + (asx.elapsedMs || 0),
    };
  }
  const minimumVolume = Number(settings.minimumDailyVolume) || 1_000_000;
  ...
  const deadline = scanDeadline(settings);
  ...
  reportProgress(settings, {
    ...
    validated: 0,
    rejected: 0,
    message: `Scanning ${settings.markets?.join("+") || "market"}: 0 of ${quoteLimit} symbols checked.`,
  });
  for (let offset = 0; offset < pool.length; offset += batchSize) {
    if (scanWasCancelled(settings)) {
      budgetExhausted = true;
      budgetExhaustedReason = "Scan cancelled by user.";
      pausedAtOffset = offset;
      break;
    }
    if (scanTimedOut(deadline)) {
      budgetExhausted = true;
      budgetExhaustedReason = "Scan timed out before the provider returned enough data.";
      pausedAtOffset = offset;
      break;
    }
    ...
    providerCalls += 1;
    const requestSymbols = batch.map((row) => row.providerSymbol || row.symbol);
    let quotes = await fetchTwelveDataQuoteBatch({ symbols: requestSymbols });
    if (Array.from(quotes.values()).some(isRateLimit)) {
      ...
      retries += 1;
      providerCalls += 1;
      quotes = await fetchTwelveDataQuoteBatch({ symbols: requestSymbols });
    ...
    batch.forEach((row) => {
      const requestSymbol = row.providerSymbol || row.symbol;
      const quote = quotes.get(requestSymbol) || quotes.get(row.symbol);
      if (!quote?.ok) {
        unavailable.push({ symbol: row.symbol, statusCode: quote?.statusCode || "DATA_UNAVAILABLE", reason: quote?.error || "Quote did not return a usable price." });
        return;
      }
      if (row.market === "ASX" && (String(quote?.currency || "").toUpperCase() !== "AUD" || !["ASX", "XASX"].includes(String(quote?.exchange || "").toUpperCase()))) {
        unavailable.push({ symbol: row.symbol, statusCode: "DATA_INVALID", reason: `Quote exchange/currency mismatch: ${quote?.exchange || "unknown"} ${quote?.currency || "unknown"}.` });
        return;
      }
    ...
    reportProgress(settings, {
      ...
      total: quoteLimit,
      validated: screened.length,
      rejected: unavailable.length,
      message: `Scanning ${settings.markets?.join("+") || "market"}: ${processedCount} of ${quoteLimit} symbols checked - ${screened.length} valid - ${unavailable.length} rejected.`,
    });
  }
  ...
  return {
    ...
    symbolsRequested: processedCount,
    barsReturned: 0,
    cacheHits: 0,
    freshFetches: 0,
    batches: 0,
    missingSymbols: unavailable.filter((item) => item.statusCode === "SYMBOL_NOT_FOUND").length,
    elapsedMs: 0,
    timedOut: budgetExhaustedReason?.toLowerCase().includes("timed out") || false,
    cancelled: budgetExhaustedReason?.toLowerCase().includes("cancelled") || false,
  };
}

async function broadAsxHistoryScreen(candidates, settings) {
  const quoteLimit = Math.min(broadQuoteLimit(settings), candidates.length);
  const pool = candidates.slice(0, quoteLimit);
  const started = Date.now();
  const range = settings.broadScreenRange || "1y";
  const interval = "1day";
  const deadline = scanDeadline(settings);

  if (scanWasCancelled(settings) || scanTimedOut(deadline)) {
    return {
      ...
      unavailable: [],
      budgetExhausted: true,
      budgetExhaustedReason: scanWasCancelled(settings) ? "Scan cancelled by user." : "Scan timed out before the provider returned enough data.",
      pausedAtOffset: 0,
      symbolsRequested: 0,
      barsReturned: 0,
      cacheHits: 0,
      ...
      timedOut: scanTimedOut(deadline),
      cancelled: scanWasCancelled(settings),
    };
  }
  ...
  reportProgress(settings, {
    ...
    validated: 0,
    rejected: 0,
    message: `Scanning ASX: 0 of ${quoteLimit} symbols checked.`,
  });

  const histories = await fetchSharedHistoryBatch(pool.map((row) => ({
    symbol: row.symbol,
    providerSymbol: row.providerSymbol || `${row.symbol}:ASX`,
    exchange: "ASX",
    currency: "AUD",
  })), range, interval);
  const screened = [];
  ...
  pool.forEach((row) => {
    ...
    const { scored, unavailable: failed } = broadHistoryScore(row, history, settings);
  ...
  reportProgress(settings, {
    ...
    validated: screened.length,
    rejected: unavailable.length,
    message: `Scanning ASX: ${pool.length} of ${quoteLimit} symbols checked - ${screened.length} valid - ${unavailable.length} rejected.`,
  });

  screened.sort((a, b) => (b.broadScore - a.broadScore) || (b.volume - a.volume) || a.symbol.localeCompare(b.symbol));
  return {
    provider: screened.some((row) => row.provider === "Yahoo Finance") ? "Yahoo Finance" : "Twelve Data",
    providerCalls: 1,
    providerWaits: 0,
    providerWaitMs: 0,
    retries: 0,
    ...
    pausedAtOffset: null,
    symbolsRequested: pool.length,
    barsReturned: screened.reduce((total, row) => total + (Number(row.candleCount) || 0), 0),
    cacheHits: screened.filter((row) => row.cache?.hit || row.cache?.preScreenHit).length,
    freshFetches: pool.length,
    batches: 1,
    symbolsPerBatch: pool.length,
    missingSymbols: unavailable.filter((item) => item.statusCode === "SYMBOL_NOT_FOUND").length,
    elapsedMs: Date.now() - started,
  };
}
...
async function broadAlpacaScreen(candidates, settings) {
  ...
  const pool = candidates.slice(0, quoteLimit);
  const started = Date.now();
  const range = settings.broadScreenRange || "3mo";
  const interval = "1day";
  ...
  if (scanWasCancelled(settings)) {
    return {
      ...
      freshFetches: 0,
      batches: 0,
      symbolsPerBatch: broadAlpacaBatchSize(),
      missingSymbols: 0,
      elapsedMs: Date.now() - started,
      cancelled: true,
    };
  ...
  pool.forEach((row) => {
    const hit = cachedPreScreenHistory(row.symbol, range, interval);
    if (hit) cached.set(row.symbol, hit);
    else toFetch.push(row.symbol);
  });
  const batch = toFetch.length
    ? await AlpacaProvider.historyBatch(toFetch, {
      range,
      interval,
      limit: Math.max(60, Number(settings.broadScreenBars) || 80),
    }, { chunkSize: broadAlpacaBatchSize() })
    : new Map();
  toFetch.forEach((symbol) => storePreScreenHistory(symbol, range, interval, batch.get(symbol)));
  cached.forEach((history, symbol) => batch.set(symbol, history));
  const diagnostics = batch.diagnostics || { apiCalls: 0, symbolsRequested: toFetch.length, barsReturned: 0, failures: 0 };
  const screened = [];
  const unavailable = [];
  pool.forEach((row) => {
  ...
  reportProgress(settings, {
    ...
    validated: screened.length,
    rejected: unavailable.length,
    message: `Scanning ${settings.markets?.join("+") || "market"}: ${pool.length} of ${quoteLimit} symbols checked - ${screened.length} valid - ${unavailable.length} rejected.`,
  });
  screened.sort((a, b) => (b.broadScore - a.broadScore) || (b.volume - a.volume) || a.symbol.localeCompare(b.symbol));
  return {
    provider: "Alpaca",
    ...
    freshFetches: toFetch.length,
    batches: Math.ceil(toFetch.length / broadAlpacaBatchSize()),
    symbolsPerBatch: broadAlpacaBatchSize(),
    missingSymbols: unavailable.filter((item) => item.statusCode === "SYMBOL_NOT_FOUND").length,
    elapsedMs: Date.now() - started,
  };
...
export async function buildMarketDiscovery(settings = {}) {
  const usesTwelvePreScreen = selectMarketDataProvider("preScreenQuote").id !== AlpacaProvider.id;
  const usage = usesTwelvePreScreen
    ? await fetchTwelveDataApiUsage().catch((error) => ({ ok: false, error: error.message }))
    : { ok: true, provider: "Alpaca", plan_category: "alpaca", plan_limit: null, plan_daily_limit: null };
  const usageBudgetExhausted = !usage?.ok && isDailyLimit(usage);
  let reference = null;
  let alpacaAssets = null;
  if (usageBudgetExhausted) {
    reference = { ok: false, rows: referenceCache.rows || [], cacheHit: Boolean(referenceCache.rows?.length), fetchedAt: referenceCache.fetchedAt, error: usage.error || "Provider daily credit limit reached." };
    alpacaAssets = { ok: true, cacheHit: false, assetsBySymbol: new Map(), error: null };
  } else {
    [reference, alpacaAssets] = await Promise.all([
      loadReferenceRows(),
      selectMarketDataProvider("preScreenQuote").id === AlpacaProvider.id
        ? loadAlpacaAssets()
        : Promise.resolve({ ok: true, cacheHit: false, assetsBySymbol: new Map(), error: null }),
    ]);
  }
  ...
  const shouldGateIdentity = selectMarketDataProvider("preScreenQuote").id === AlpacaProvider.id;
  const identity = shouldGateIdentity
    ? identityGate(reference.rows || [], alpacaAssets.assetsBySymbol || new Map())
    : { rows: reference.rows || [], invalid: [] };
  ...
  const selectedUniverse = settings.symbols?.length
    ? (settings.universeSelection === "CMC_IMPORTED" ? "CMC imported candidates" : "Custom watchlist")
    : settings.universeSelection === "ASX_200"
    ? "ASX 200"
    : settings.universeSelection === "ASX_300"
    ? "ASX 300"
    : settings.universeSelection === "ASX_LIQUID"
    ? "ASX liquidity-filter candidates"
    : "Provider-supported securities";
  ...
  return {
    ...
    identity: {
      ok: alpacaAssets.ok,
      cacheHit: alpacaAssets.cacheHit,
      valid: rows.filter((row) => row.market === "US" && row.identityValid !== false).length,
      invalid: identity.invalid.length,
      error: alpacaAssets.error || null,
    },
    coverage,
    supportedUniverseCount: rows.length,
    candidateUniverseCount: candidates.length,
    expectedUniverseSize: settings.symbols?.length || candidates.length,
    validatedLiquidUniverseSize: broad.eligible.length,
    selectedUniverse,
    universeOptions: [
      { id: "ASX_200", label: "ASX 200", expectedSize: Math.min(200, rows.filter((row) => row.market === "ASX").length), market: "ASX" },
      { id: "ASX_300", label: "ASX 300", expectedSize: Math.min(300, rows.filter((row) => row.market === "ASX").length), market: "ASX" },
      { id: "ASX_LIQUID", label: "ASX liquidity-filter candidates", expectedSize: candidates.length, validatedSize: broad.eligible.length, market: "ASX" },
      { id: "CMC_IMPORTED", label: "CMC imported candidates", expectedSize: settings.symbols?.length || 0, market: "ASX" },
      { id: "CUSTOM", label: "Custom watchlist", expectedSize: settings.symbols?.length || 0, market: "ASX/US" },
    ],
    broadScreen: {
      ...
      cacheHits: broad.cacheHits || 0,
      freshFetches: broad.freshFetches || 0,
      batches: broad.batches || 0,
      symbolsPerBatch: broad.symbolsPerBatch || null,
      missingSymbols: broad.missingSymbols || 0,
