...
function normalizeSettings(input = {}) {
  ...
  const sizing = normalizePositionSizingSettings({
    ...settings,
    availableCash: settings.availableCash,
    defaultMaximumLoss: input.defaultMaximumLoss ?? input.maximumLoss ?? input.maximumPlannedLossPerTrade ?? settings.maximumPlannedLossPerTrade,
    maximumCapitalPerTrade: input.maximumCapitalPerTrade ?? input.maximumPositionValue ?? settings.maximumPositionValue,
    currentTradeBudget: input.currentTradeBudget ?? input.tradeAllocation ?? input.tradeBudget ?? input.maximumPositionValue ?? settings.maximumPositionValue,
  });
  settings.tradingBalance = sizing.tradingAccountBalance;
  settings.currentTradeBudget = sizing.currentTradeBudget;
  settings.defaultMaximumLoss = sizing.defaultMaximumLoss;
  settings.minimumAcceptableExpectedProfit = sizing.minimumAcceptableExpectedProfit;
  settings.maximumCapitalPerTrade = sizing.maximumCapitalPerTrade;
  settings.currencyPreference = sizing.currencyPreference;
  settings.brokerageEstimate = sizing.brokerageEstimate;
  settings.allowFractionalShares = sizing.allowFractionalShares;
...
function conversionRate(fromCurrency, settings) {
  return conversionRateToAccount(fromCurrency, settings);
}
...
function validateCandidate(row, settings, context, now) {
  ...
  const sizing = calculatePositionSize({
    ...
    maximumLoss: settings.defaultMaximumLoss,
  }, {
    ...settings,
    availableCash: remainingCash,
    currentTradeBudget: Math.min(settings.currentTradeBudget, remainingCommitted),
    maximumCapitalPerTrade: Math.min(settings.maximumCapitalPerTrade, settings.maximumPositionValue, remainingCommitted),
  });
  if (!sizing.ok) {
    const reason = sizing.blockers?.[0] || "position sizing rejected this trade";
    return { ok: false, status: /conversion|missing/i.test(reason) ? "DATA UNAVAILABLE" : "WAIT", reason, quality, sizing };
  }
  if (context.usedPlannedLoss + sizing.maximumLossAccount > settings.maximumTotalPlannedLoss) return { ok: false, status: "WAIT", reason: "maximum planned loss exceeded", quality, sizing };
  ...
  return {
    ...
    purchaseValue: sizing.capitalRequired,
    accountPurchaseValue: sizing.capitalRequiredAccount,
    plannedLoss: sizing.maximumLossAccount,
...
function tradeFitScore(row, settings, now = new Date()) {
  const quality = classifyMarketData(row, now);
  if (quality === "stale" || quality === "unavailable") return -100000;
  const values = tradeValues(row, settings);
  const currencyCode = String(row.currency || row.opportunity?.currency || "USD").toUpperCase();
  const sizing = calculatePositionSize({
    currency: currencyCode,
  ...
  const technical = Number(row.tradingScore ?? row.opportunity?.score) || 0;
  if (!sizing.ok) return technical - 1000;
  return (sizing.expectedProfitAccount || 0) + ((sizing.rewardRisk || 0) * 25) - ((sizing.accountPercent || 0) * 2) + (technical / 10);
}
...
export function calculatePartialExit({ quantity, percent = 50 } = {}) {
  ...
  if (held === 1 && pct < 100) return { sellQuantity: 0, remainingQuantity: 1, percent: pct, unevenSplit: true, oneShareOnly: true };
...
export function buildRecommendation(row, settings, context, now = new Date()) {
  ...
  const recommendation = {
    ...
    recommendedPosition: validation.quantity || 0,
    ...
    capitalRequired: validation.accountPurchaseValue || null,
    expectedProfit: validation.sizing?.expectedProfitAccount ?? null,
    expectedTakeProfit: validation.sizing?.expectedTakeProfitAccount ?? null,
    rewardRisk: validation.sizing?.rewardRisk ?? validation.riskReward ?? null,
    percentageOfAccount: validation.sizing?.accountPercent ?? null,
    positionSizing: validation.sizing || null,
    currency: currencyCode,
    accountCurrency,
  ...
  if (validation.ok) {
    recommendation.cmcOrder = {
      ...
      instructions: [
        "Open CMC Invest.",
        `Search for ${recommendation.symbol}.`,
        "Select Buy.",
        "Choose a Limit or conditional order.",
        `Enter ${recommendation.suggestedQuantity} whole shares.`,
        `Enter the buy-trigger price of ${formatMoney(recommendation.buyPrice, currencyCode)}.`,
        "Review and submit the order.",
        "Add the Safety Exit and profit instructions supported by the CMC order workflow.",
        "Return to Freedom and record the actual filled price only after CMC confirms execution.",
      ],
      afterPurchase: [
        ...
        `Maximum loss about ${formatMoney(recommendation.maximumPlannedLoss, accountCurrency)}`,
        `Expected Final Exit profit about ${formatMoney(recommendation.expectedProfit, accountCurrency)}`,
      ],
...
export function buildPositionAction(position = {}, settings = DEFAULT_REPORT_SETTINGS, now = new Date()) {
  ...
  if (Number.isFinite(currentPrice) && Number.isFinite(safetyExit) && currentPrice <= safetyExit) {
    ...
    instruction = partial.oneShareOnly
      ? "This position contains one share. Choose either: sell the full share at Take Some Profit, or hold the full share for Final Exit."
      : `Sell ${partial.sellQuantity} share${partial.sellQuantity === 1 ? "" : "s"} now and keep ${partial.remainingQuantity} open for the Final Exit.${settings.moveSafetyExitToEntryAfterTakeProfit && Number.isFinite(entryPrice) ? " Move the Safety Exit on the remaining shares to the original buy price." : ""}`;
...
export function generateFreedomTraderReport({
  ...
  const trustworthyScannerRows = scanSummary && scanSummary.status !== "complete" ? [] : scannerRows;
  const sorted = [...trustworthyScannerRows].sort((a, b) => tradeFitScore(b, cleanSettings, now) - tradeFitScore(a, cleanSettings, now));
  ...
  const marketRows = [...trustworthyScannerRows, ...openPositions.map((position) => position.priceData || position)];
