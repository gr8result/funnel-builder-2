...
export const MARKET_DATA_STATUSES = ["READY", "DELAYED", "STALE", "UNAVAILABLE", "UNSUPPORTED"];
...
function normalizeSettings(input = {}) {
  return {
    ...DEFAULT_CONFIG,
    ...input,
    markets: Array.isArray(input.markets) && input.markets.length ? input.markets : ["US"],
    ...
    chunkSize: Math.max(1, Math.min(80, Number(input.chunkSize) || 30)),
...
export function validateMarketData(row = {}, universeItem = {}, { now = new Date(), staleDailyHours = DEFAULT_CONFIG.staleDailyHours } = {}) {
  ...
  if (row.status === "DATA UNAVAILABLE" || row.dataQuality === "unavailable" || row.dataStatus?.apiError || row.error) {
    return { status: "UNAVAILABLE", couldAnalyse: false, reason: row.error || row.dataStatus?.apiError || row.dataStatus?.status || "Market data unavailable" };
  }
  ...
  if (row.dataStatus?.permissionFailure || /permission|access|plan/i.test(String(row.dataStatus?.status || ""))) {
    return { status: "UNAVAILABLE", couldAnalyse: false, reason: row.dataStatus.status || "Market-data permission failure" };
  }
...
export function buildOpportunityDecision(row = {}, { settings = {}, universeItem = {}, now = new Date() } = {}) {
  ...
  return {
    ...
    couldAnalyse: quality.couldAnalyse,
    couldNotAnalyseReason: quality.couldAnalyse ? null : quality.reason,
    currentPrice: values.currentPrice,
    priceTimestamp: row.priceTimestamp || row.dataStatus?.latestTimestamp || opportunity.priceTimestamp || null,
    entry: values.entry,
    recommendedEntry: values.entry,
    entryZoneHigh: values.entryHigh,
    safetyExit: values.safetyExit,
    stopLoss: values.safetyExit,
    takeSomeProfit: values.takeSomeProfit,
    target: values.takeSomeProfit,
    ...
    tradingScore: cleanNumber(opportunity.score ?? row.tradingScore),
    reason,
...
export function rankOpportunityDecisions(decisions = [], top = 5) {
  const rankable = decisions.filter((item) => item.dataQualityStatus !== "STALE");
  const weight = (item) => {
...
export async function runOpportunityEngine({
  ...
  const snapshots = typeof marketSnapshotBatch === "function"
    ? await marketSnapshotBatch(chunk.map((item) => item.symbol), { range: "1y", interval: "1day" })
    : new Map();
