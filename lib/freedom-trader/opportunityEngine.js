...
export const SUPPORTED_UNIVERSES = {
  US_LIQUID: [
    ...
    ["PANW", "Palo Alto Networks", "Cybersecurity", "US"], ["CRWD", "CrowdStrike", "Cybersecurity", "US"], ["SNOW", "Snowflake", "Data Cloud", "US"],
    ["DDOG", "Datadog", "Software", "US"], ["NET", "Cloudflare", "Internet Infrastructure", "US"], ["SHOP", "Shopify", "E-commerce Software", "US"],
    ["UBER", "Uber", "Mobility", "US"], ["ABNB", "Airbnb", "Travel", "US"], ["BKNG", "Booking Holdings", "Travel", "US"],
    ["DIS", "Disney", "Media", "US"], ["CMCSA", "Comcast", "Media", "US"], ["T", "AT&T", "Telecom", "US"],
    ["VZ", "Verizon", "Telecom", "US"], ["PFE", "Pfizer", "Healthcare", "US"], ["JNJ", "Johnson & Johnson", "Healthcare", "US"],
    ["TMO", "Thermo Fisher Scientific", "Healthcare", "US"], ["ISRG", "Intuitive Surgical", "Healthcare", "US"], ["DHR", "Danaher", "Healthcare", "US"],
    ["PG", "Procter & Gamble", "Consumer Defensive", "US"], ["KO", "Coca-Cola", "Consumer Defensive", "US"], ["PEP", "PepsiCo", "Consumer Defensive", "US"],
    ["PM", "Philip Morris", "Consumer Defensive", "US"], ["MO", "Altria", "Consumer Defensive", "US"], ["TGT", "Target", "Retail", "US"],
    ["LULU", "Lululemon", "Retail", "US"], ["ROST", "Ross Stores", "Retail", "US"], ["TJX", "TJX Companies", "Retail", "US"],
    ["AXP", "American Express", "Financials", "US"], ["C", "Citigroup", "Financials", "US"], ["WFC", "Wells Fargo", "Financials", "US"],
    ["BLK", "BlackRock", "Financials", "US"], ["SCHW", "Charles Schwab", "Financials", "US"], ["SPGI", "S&P Global", "Financials", "US"],
    ["RTX", "RTX", "Industrials", "US"], ["LMT", "Lockheed Martin", "Industrials", "US"], ["NOC", "Northrop Grumman", "Industrials", "US"],
    ["HON", "Honeywell", "Industrials", "US"], ["UPS", "UPS", "Logistics", "US"], ["FDX", "FedEx", "Logistics", "US"],
    ["LIN", "Linde", "Materials", "US"], ["APD", "Air Products", "Materials", "US"], ["NEM", "Newmont", "Materials", "US"],
    ["COP", "ConocoPhillips", "Energy", "US"], ["SLB", "Schlumberger", "Energy", "US"], ["EOG", "EOG Resources", "Energy", "US"],
    ["MAR", "Marriott", "Travel", "US"], ["HLT", "Hilton", "Travel", "US"], ["GM", "General Motors", "Autos", "US"],
    ["F", "Ford", "Autos", "US"], ["ELF", "e.l.f. Beauty", "Consumer", "US"], ["CELH", "Celsius", "Consumer", "US"],
...
export function asxMarketDataConfigured() {
  return /^(1|true|yes)$/i.test(String(process.env.FREEDOM_TRADER_ASX_MARKET_DATA_ENABLED || "").trim());
}

function configuredMarkets() {
  return asxMarketDataConfigured() ? ["US", "ASX"] : FREEDOM_TRADER_V1_MARKETS;
}

function marketScopeMessage(markets = ["US"]) {
  return markets.includes("ASX")
    ? "Freedom Trader V1.0 analyses the configured US universe plus ASX symbols because ASX market data is enabled on the server."
    : FREEDOM_TRADER_V1_MARKET_SCOPE_MESSAGE;
}

export function parseConfiguredUniverse(value = "", market = "US") {
  return String(value || "")
    .split(/[\n,]+/)
    .map((entry) => entry.trim())
    .filter(Boolean)
    .map((entry, index) => {
      const [rawSymbol, companyName = rawSymbol, sector = "Configured universe"] = entry.split("|").map((part) => part.trim());
      const symbol = String(rawSymbol || "").toUpperCase();
      if (!/^[A-Z0-9.-]{1,12}$/.test(symbol)) return null;
      return {
        symbol,
        companyName: companyName || symbol,
        sector: sector || "Configured universe",
        market,
        exchange: market === "ASX" ? "ASX" : "NASDAQ/NYSE",
        universe: `Configured ${market} shares`,
        enabled: true,
        minimumLiquidity: market === "ASX" ? 500000 : 1000000,
...
function normalizeSettings(input = {}) {
  ...
  const enabledMarkets = configuredMarkets();
  const markets = requestedMarkets.filter((market) => enabledMarkets.includes(market));
  const selectedMarkets = markets.length ? markets : ["US"];
  const ignoredMarkets = requestedMarkets.filter((market) => !selectedMarkets.includes(market));
  return {
    ...DEFAULT_CONFIG,
    ...input,
    ...
    requestedMarkets,
    ignoredMarkets,
    marketScope: selectedMarkets.includes("ASX") ? "US_ASX_CONFIGURED" : "US_ONLY",
    marketScopeMessage: marketScopeMessage(selectedMarkets),
    ...
    chunkSize: Math.max(1, Math.min(250, Number(input.chunkSize) || 80)),
...
export function supportedUniverseForMarkets(markets = ["US"]) {
  ...
  if (requestedMarkets.has("US")) {
    rows.push(...SUPPORTED_UNIVERSES.US_LIQUID);
    rows.push(...parseConfiguredUniverse(process.env.FREEDOM_TRADER_US_UNIVERSE, "US"));
  }
  if (requestedMarkets.has("ASX")) {
    const asxEnabled = asxMarketDataConfigured();
    rows.push(...SUPPORTED_UNIVERSES.ASX_SUPPORTED.map((item) => ({
      ...item,
      enabled: asxEnabled,
      disabledReason: asxEnabled ? null : item.disabledReason,
    })));
    if (asxEnabled) rows.push(...parseConfiguredUniverse(process.env.FREEDOM_TRADER_ASX_UNIVERSE, "ASX"));
  }
  const seen = new Set();
