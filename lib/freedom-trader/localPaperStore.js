...
function storePath() {
  return process.env.FREEDOM_PAPER_STORE_PATH || path.join(process.cwd(), "tmp", "freedom-paper-local.json");
}
...
async function writeStore(store) {
  const writeTask = localWriteQueue.then(async () => {
    const targetPath = storePath();
    await mkdir(path.dirname(targetPath), { recursive: true });
    let current = null;
    try {
      current = JSON.parse(await readFile(targetPath, "utf8"));
    } catch {}
    const merged = current?.account ? mergeStore(current, store) : store;
    await writeFile(targetPath, JSON.stringify(merged, null, 2));
    return merged;
  });
  localWriteQueue = writeTask.catch(() => {});
  return writeTask;
}

function mergeRows(existing = [], incoming = []) {
  const byId = new Map();
  existing.forEach((item) => item?.id && byId.set(item.id, item));
...
function mergeStore(existing, incoming) {
  return {
    ...
    orders: mergeRows(existing.orders, incoming.orders),
    positions: mergeRows(existing.positions, incoming.positions),
    trades: mergeRows(existing.trades, incoming.trades),
    watchlist: mergeRows(existing.watchlist, incoming.watchlist),
    marketWatch: mergeRows(existing.marketWatch, incoming.marketWatch),
    lastGoodScan: incoming.lastGoodScan || existing.lastGoodScan || null,
    journal: mergeRows(existing.journal, incoming.journal),
    notifications: mergeRows(existing.notifications, incoming.notifications),
    capitalFlowHistory: { ...(existing.capitalFlowHistory || {}), ...(incoming.capitalFlowHistory || {}) },
    notificationSettings: incoming.notificationSettings || existing.notificationSettings,
    events: mergeRows(existing.events, incoming.events),
  };
}
...
export async function loadLocalMarketWatch() {
  const store = await readStore();
  return {
    marketWatch: Array.isArray(store.marketWatch) ? store.marketWatch.map((item) => ({ ...item, state: canonicalMarketWatchState(item.state) })) : [],
    journal: Array.isArray(store.journal) ? store.journal : [],
    capitalFlowHistory: store.capitalFlowHistory || {},
    notifications: Array.isArray(store.notifications) ? store.notifications : [],
    notificationSettings: store.notificationSettings || emptyStore().notificationSettings,
    storageMode: "local",
    localStorePath: storePath(),
  };
}

export async function loadLocalLastGoodScan() {
  const store = await readStore();
  return store.lastGoodScan || null;
}

export async function saveLocalLastGoodScan(scan = null) {
  if (!scan?.ok || !scan?.scanSummary || scan.scanSummary.status === "failed") return null;
  const store = await readStore();
  store.lastGoodScan = {
    ...scan,
  ...
  await writeStore(store);
  return store.lastGoodScan;
}

function canonicalMarketWatchState(state) {
  const value = String(state || "").toUpperCase();
  if (value === "POSITION_ACTIVE") return "ACTIVE";
  if (value === "TRADE_COMPLETED") return "COMPLETED";
  if (value === "SETUP_CANCELLED") return "CANCELLED";
  return value || "WAITING_FOR_ENTRY";
}

function marketWatchStateIs(state, expected) {
  return canonicalMarketWatchState(state) === expected;
}

function normalizeMarketWatchItem(item = {}) {
  return { ...item, state: canonicalMarketWatchState(item.state) };
}

export async function loadLocalTraderWatchlist() {
  const store = await readStore();
  return Array.isArray(store.watchlist) ? store.watchlist : [];
}

export async function addLocalTraderWatchlistItem(input = {}) {
  const store = await readStore();
  ...
  const item = {
    id: existing?.id || `watchlist:${symbol}`,
    symbol,
    companyName: input.companyName || input.name || symbol,
    exchange: input.exchange || "NASDAQ",
    sector: input.sector || input.assetType || input.market || "Trading Watchlist",
    currency: input.currency || (symbol.endsWith(".AX") ? "AUD" : "USD"),
    source: "analyse-stock",
    addedAt: existing?.addedAt || now,
    updatedAt: now,
  };
  store.watchlist = [item, ...(store.watchlist || []).filter((row) => row.symbol !== symbol)];
  await writeStore(store);
  return item;
}

export async function loadLocalCapitalFlowHistory() {
  const store = await readStore();
  return store.capitalFlowHistory || {};
}

export async function saveLocalCapitalFlowSnapshot(rows = [], timestamp = new Date().toISOString()) {
  const store = await readStore();
  const history = { ...(store.capitalFlowHistory || {}) };
  rows.forEach((row) => {
    if (!row?.symbol || !row.capitalFlow) return;
    history[row.symbol] = {
      symbol: row.symbol,
      capitalFlowScore: row.capitalFlow.capitalFlowScore,
      state: row.capitalFlow.state,
      pressure: row.capitalFlow.pressure,
      relativeVolume: row.capitalFlow.relativeVolume,
      priceChangePercent: row.capitalFlow.priceChangePercent,
      updatedAt: timestamp,
    };
  });
  store.capitalFlowHistory = history;
  await writeStore(store);
  return history;
}

function notificationKey(input = {}) {
  return [input.planId || input.marketWatchId || "", input.symbol || "", input.alertType || "", input.triggerState || ""].join(":").toUpperCase();
}

export async function loadLocalFreedomNotifications() {
  const store = await readStore();
  return {
    notifications: Array.isArray(store.notifications) ? store.notifications : [],
    settings: store.notificationSettings || emptyStore().notificationSettings,
    storageMode: "local",
    localStorePath: storePath(),
  };
}

export async function updateLocalFreedomNotificationSettings(input = {}) {
  const store = await readStore();
  const current = store.notificationSettings || emptyStore().notificationSettings;
  store.notificationSettings = {
    ...current,
    ...input,
    sendSMSFor: { ...(current.sendSMSFor || {}), ...(input.sendSMSFor || {}) },
  };
  await writeStore(store);
  return store.notificationSettings;
}

export async function recordLocalFreedomNotification(input = {}) {
  const store = await readStore();
  const key = notificationKey(input);
  const existing = (store.notifications || []).find((item) => item.key === key && item.status !== "cleared");
  if (existing && !input.force) return { notification: existing, duplicate: true };
  const now = new Date().toISOString();
  const notification = {
    id: id("notice"),
    key,
    planId: input.planId || input.marketWatchId || null,
    ...
    smsError: input.smsError || null,
    createdAt: now,
    updatedAt: now,
  };
  store.notifications = [notification, ...(store.notifications || [])].slice(0, 500);
  await writeStore(store);
  return { notification, duplicate: false };
}

export async function updateLocalFreedomNotification(idValue, patch = {}) {
  const store = await readStore();
  const item = (store.notifications || []).find((row) => row.id === idValue);
...
export async function registerLocalMarketWatchPlan(input = {}) {
  ...
  const existing = (store.marketWatch || []).find((item) => (item.symbol || item.plan?.symbol) === plan.symbol && ["WAITING_FOR_ENTRY", "ACTIVE"].includes(canonicalMarketWatchState(item.state)));
  if (existing) {
    existing.plan = plan;
    ...
    await writeStore(store);
    return normalizeMarketWatchItem(existing);
  }
  const item = {
    id: id("watch"),
    broker: "CMC",
  ...
  await writeStore(store);
  return normalizeMarketWatchItem(item);
}
...
export async function recordLocalMarketWatchFill(input = {}) {
  ...
  const symbol = String(input.symbol || "").trim().toUpperCase();
  const item = (store.marketWatch || []).find((row) => (idValue && row.id === idValue) || (symbol && row.plan?.symbol === symbol && marketWatchStateIs(row.state, "WAITING_FOR_ENTRY")));
...
export function evaluateMarketWatchItem(item = {}, quote = {}, options = {}) {
  ...
  const base = {
    ...
    timestamp,
    state: canonicalMarketWatchState(item.state),
    trigger: null,
    action: "HOLD",
  ...
  if (marketWatchStateIs(item.state, "WAITING_FOR_ENTRY")) {
  ...
  if (!marketWatchStateIs(item.state, "ACTIVE")) return { ...base, action: marketWatchStateIs(item.state, "COMPLETED") ? "TRADE_COMPLETE" : "HOLD" };
...
export async function checkLocalMarketWatch(input = {}) {
  ...
  store.marketWatch = (store.marketWatch || []).map((item) => {
    ...
    if (report.action === "CANCEL_SETUP") item.state = "CANCELLED";
    item.updatedAt = new Date().toISOString();
    return item;
  });
  await writeStore(store);
  return { reports, marketWatch: store.marketWatch.map(normalizeMarketWatchItem) };
}
...
export async function recordLocalMarketWatchSale(input = {}) {
  ...
  const idValue = input.id;
  const symbol = String(input.symbol || "").trim().toUpperCase();
  const item = (store.marketWatch || []).find((row) => (idValue && row.id === idValue) || (symbol && row.plan?.symbol === symbol && marketWatchStateIs(row.state, "ACTIVE")));
  ...
  item.state = item.remainingQuantity > 0 ? "ACTIVE" : "COMPLETED";
  item.updatedAt = new Date().toISOString();
