...
const TWELVE_DATA_REQUESTS_PER_MINUTE = Math.max(
  1,
...
export const MARKET_DATA_PROVIDERS = [
  {
    ...
    label: "Twelve Data",
    role: "daily history",
    batchSize: TWELVE_DATA_BATCH_SIZE,
...
export function creditCapacityRemaining() {
  ...
  return Math.max(0, TWELVE_DATA_REQUESTS_PER_MINUTE - creditWindow.credits);
}
      ...
      const requestPromise = (async () => {
        ...
        try {
          ...
          if (batchNeedsRetry(batchMap)) {
            ...
            await wait(rateLimited ? msUntilNextMinute() : RETRY_BACKOFF_MS[0]);
            await waitForProviderSlot(1);
            batchMap = await fetchTwelveDataHistoryBatch({ symbols: toFetch, range, interval });
          }
        } catch (error) {
