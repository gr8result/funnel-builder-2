...
function classifyDataQuality({ historyOk, planRestricted, liveQuote, latestCandleDate }) {
  ...
  if (Number.isFinite(ageHours) && ageHours > STALE_DAILY_HOURS) return "unavailable";
  return "daily-only";
}

function unavailableReason(historyResult, symbol) {
  const classification = classifyTwelveDataError(historyResult?.error);
...
async function buildSnapshot(symbol, historyResult, { allowFinnhubEnhancement = true } = {}) {
  ...
  return {
    ...
    error: dataQuality === "unavailable" ? unavailableReason(historyResult, normalized) : null,
  };
}
