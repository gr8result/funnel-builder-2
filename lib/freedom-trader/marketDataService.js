...
export const MARKET_DATA_PROVIDERS = [
  { id: "twelve-data", label: "Twelve Data", history: true, quotes: true, batchHistory: true, creditsPerMinute: TWELVE_DATA_CREDITS_PER_MINUTE },
  { id: "finnhub", label: "Finnhub", history: false, quotes: true, batchHistory: false },
];
...
function storeHistory(symbol, range, interval, data) {
  ...
  historyCache.set(historyCacheKey(symbol, range, interval), {
    fetchedAt,
    expiresAt: Date.now() + DAILY_HISTORY_TTL_MS,
    quality: "daily-history",
    data: { ...data, fetchedAt },
  });
}
