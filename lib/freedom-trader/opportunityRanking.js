...
import { computeCapitalFlow } from "./capitalFlow.js";
import { volatilitySuitabilityScore } from "./volatility.js";
...
function scoreComponents(row) {
  ...
  const capitalFlow = row.capitalFlow || computeCapitalFlow(row, row.previousCapitalFlow);
  ...
  const volatilityScore = numberValue(row.scoreExplanation?.volatilitySuitability?.score)
    ?? volatilitySuitabilityScore(row.volatility || { averageDailyMovementPercent: row.indicators?.volatility20, rating: row.indicators?.volatilityRating });
  ...
  const opportunityQuality = round(
    ...
    Number(volumeScore) * 0.07 +
    (Number(capitalFlow.capitalFlowScore) || 0) * 0.03 +
  ...
  const currentTradeRank = round(
    ...
    Number(rewardScore) * 0.08 +
    (Number(capitalFlow.capitalFlowScore) || 0) * 0.02
  );
  return {
    ...
    capitalFlow: capitalFlow.capitalFlowScore,
...
function eligibility(row, settings = {}, now = new Date()) {
  ...
  const volatilityRating = String(row.volatility?.rating || row.indicators?.volatilityRating || "").toUpperCase();
  ...
  const exchange = String(row.exchange || (row.market === "ASX" ? "ASX" : "NASDAQ")).toUpperCase();
  const currency = String(row.currency || (row.market === "ASX" ? "AUD" : "USD")).toUpperCase();
  const supportedExchange = ["NASDAQ", "NYSE", "AMEX", "US", "ASX", "XASX"].includes(exchange);
  const supportedCurrency = ["USD", "AUD"].includes(currency);
  if (!supportedExchange) reasons.push("The exchange is not supported for this scan.");
  if (!supportedCurrency) reasons.push("The currency is not supported for this scan.");
  if (!Number.isFinite(volume) || volume < minimumVolume) reasons.push("Trading activity is too low.");
  if (!Number.isFinite(volatility)) reasons.push("Volatility could not be calculated from validated history.");
  else if (volatility > maximumVolatility || volatilityRating === "EXTREME") reasons.push("The price is too volatile for the current rules.");
...
function strategyStatus(classification, baseEligible, components, reasons = []) {
  ...
  if ((classification.setupType === "MOMENTUM_CONTINUATION" || classification.setupType === "BREAKOUT") && baseEligible && Number(components.capitalFlow) >= 78) return "READY";
...
export function rankMarketOpportunities(analysisRows = [], settings = {}, options = {}) {
  ...
  const ranked = analysisRows.map((row) => {
    ...
    const item = {
      ...
      dataSource: row.dataSource || row.marketData?.historySource || null,
      ...
      components,
      capitalFlow,
      capitalFlowScore: capitalFlow.capitalFlowScore,
      capitalFlowState: capitalFlow.state,
      buyingSellingPressure: capitalFlow.pressure,
      relativeVolume: capitalFlow.relativeVolume,
      volatility: row.volatility || null,
      priceMomentum: capitalFlow.priceChangePercent,
      capitalFlowChange: capitalFlow.acceleration,
      catalyst: capitalFlow.catalyst,
      reason: primaryReason(status, { ...components, riskReward: row.setup?.riskRewardRatio }, classification),
      plainEnglish: [...plainSetup(classification, status), row.volatility?.assessment || null, capitalFlow.explanation || plainVolume(row), Number(row.setup?.riskRewardRatio) >= 2 ? `The possible profit is approximately ${round(row.setup.riskRewardRatio, 1)} times the planned loss.` : "The possible profit is not yet large enough compared with the planned loss."].filter(Boolean),
  ...
  ranked.sort((a, b) => {
    ...
    const flowDelta = (Number(b.capitalFlowScore) || 0) - (Number(a.capitalFlowScore) || 0);
    if (flowDelta && ["REVIEW NOW", "TRADE READY"].includes(b.capitalFlowState)) return flowDelta;
  ...
  const displayableRows = options.includeDevelopingTopFive
    ? ranked.filter((row) => ["READY", "REVERSAL DEVELOPING", "WAIT FOR REVERSAL", "WAIT FOR PULLBACK"].includes(row.status))
    : eligibleRows;
  const topFive = displayableRows.slice(0, 5);
  const bestCurrentTrade = ranked.find((row) => row.status === "READY") || null;
  const bestSetupToWatch = ranked.find((row) => ["REVERSAL DEVELOPING", "WAIT FOR REVERSAL", "WAIT FOR PULLBACK"].includes(row.status)) || null;
