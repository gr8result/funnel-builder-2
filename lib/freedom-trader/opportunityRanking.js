...
export function rankMarketOpportunities(analysisRows = [], settings = {}, options = {}) {
  ...
  const ranked = analysisRows.map((row) => {
    ...
    const item = {
      ...
      plainEnglish: [...plainSetup(classification, status), capitalFlow.explanation || plainVolume(row), Number(row.setup?.riskRewardRatio) >= 2 ? `The possible profit is approximately ${round(row.setup.riskRewardRatio, 1)} times the planned loss.` : "The possible profit is not yet large enough compared with the planned loss."],
