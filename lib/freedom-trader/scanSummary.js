...
export const DEFAULT_SCAN_COVERAGE_THRESHOLD = 0.8;
...
function statusFor({ analysedCount, unavailableCount, requestedCount, totalsValid = true, coverageThreshold = DEFAULT_SCAN_COVERAGE_THRESHOLD }) {
  if (!totalsValid || requestedCount <= 0 || analysedCount <= 0) return "failed";
  const coverage = requestedCount > 0 ? analysedCount / requestedCount : 0;
  if (unavailableCount > 0 || analysedCount < requestedCount || coverage < coverageThreshold) return "partial";
  return "complete";
}

function plainEnglish({ requestedCount, qualifiedCount }) {
  const companies = requestedCount === 1 ? "company" : "companies";
  const opportunities = qualifiedCount === 1 ? "valid opportunity" : "valid opportunities";
  return `Checked ${requestedCount} ${companies}. Found ${qualifiedCount} ${opportunities}.`;
}
...
export function buildFreedomScanSummaryFromEngine(result = {}) {
  ...
  const totalsValid = requestedCount === analysedCount + unavailableCount
    && analysedCount === qualifiedCount + notQualifiedCount
    && requestedCount >= 0
    && analysedCount >= 0
    && unavailableCount >= 0;
  const coverageThreshold = cleanNumber(result.settings?.minimumScanCoverage, DEFAULT_SCAN_COVERAGE_THRESHOLD);
  const status = statusFor({ analysedCount, unavailableCount, requestedCount, totalsValid, coverageThreshold });
  ...
  return {
    ...
    coverage: requestedCount > 0 ? analysedCount / requestedCount : 0,
    checkedCount: requestedCount,
    validOpportunityCount: qualifiedCount,
    plainEnglish: plainEnglish({ requestedCount, qualifiedCount }),
    dataLabel: "Delayed by 15 minutes",
    results: decisions,
    rankedResults: status === "complete" && Array.isArray(result.results) ? result.results : [],
...
export function buildFailedFreedomScanSummary({ startedAt = new Date().toISOString(), requestedCount = 0, universeCount = 0, error = "" } = {}) {
  return {
    ...
    status: "failed",
    totalsValid: true,
    minimumCoverage: DEFAULT_SCAN_COVERAGE_THRESHOLD,
    coverage: 0,
    checkedCount: requestedCount,
    validOpportunityCount: 0,
    plainEnglish: plainEnglish({ requestedCount, qualifiedCount: 0 }),
