...
export const BROKER_WORKFLOW_STATES = ["PLAN PREPARED", "ORDER ENTERED IN CMC", "ORDER FILLED", "POSITION ACTIVE"];
...
export function normalizeTradePlan(input = {}, settings = DEFAULT_MARKET_WATCH_SETTINGS, now = new Date()) {
  ...
  const expiresAt = input.expiresAt || input.expires_at || (Number.isFinite(expiryMs)
    ...
    : new Date(dateMs(createdAt) + (cleanSettings.setupExpiryHours * 3600000)).toISOString());
  return {
    ...
    brokerState: BROKER_WORKFLOW_STATES.includes(input.brokerState) ? input.brokerState : "PLAN PREPARED",
    orderEnteredAt: input.orderEnteredAt || null,
    orderFilledAt: input.orderFilledAt || null,
    actualEntryPrice: cleanNumber(input.actualEntryPrice ?? input.confirmedEntryPrice ?? input.fillPrice),
...
function notificationBody(action, plan) {
  if (action === "BUY NOW") return `${plan.symbol} entry condition reached. Review CMC.`;
...
export function evaluateWatchPlan(planInput = {}, quote = {}, settingsInput = {}, now = new Date()) {
  ...
  const purchased = plan.state === "ACTIVE" || plan.state === "PARTIAL_PROFIT" || plan.brokerState === "ORDER FILLED" || plan.brokerState === "POSITION ACTIVE";

  if (purchased && Number.isFinite(plan.safetyExit) && currentPrice <= plan.safetyExit) {
    return { plan, action: "SAFETY EXIT", currentPrice, triggerPrice: plan.safetyExit, nextState: "STOPPED", reason: "Safety Exit reached." };
  }
  if (purchased && Number.isFinite(plan.finalExit) && currentPrice >= plan.finalExit) {
    return { plan, action: "FINAL EXIT", currentPrice, triggerPrice: plan.finalExit, nextState: "COMPLETED", reason: "Final Exit reached." };
  }
  if ((plan.state === "ACTIVE" || plan.brokerState === "ORDER FILLED" || plan.brokerState === "POSITION ACTIVE") && Number.isFinite(plan.takeSomeProfit) && currentPrice >= plan.takeSomeProfit) {
  ...
  if (plan.state === "WAITING_FOR_ENTRY" && Number.isFinite(plan.entryPrice) && currentPrice <= plan.entryPrice) {
    return { plan, action: "BUY NOW", currentPrice, triggerPrice: plan.entryPrice, nextState: "WAITING_FOR_ENTRY", reason: "Entry condition reached. Confirm any fill manually in CMC before Freedom monitors this as an active position." };
  }
...
export async function runMarketWatchCycle({ plans = [], alerts = [], settings = {}, fetchQuote, now = new Date() } = {}) {
  ...
  for (const rawPlan of plans) {
    ...
    const updatedPlan = {
      ...
      brokerState: evaluation.nextState === "ACTIVE" && plan.brokerState === "ORDER FILLED" ? "POSITION ACTIVE" : plan.brokerState,
      ...
      entryFilledAt: plan.entryFilledAt,
