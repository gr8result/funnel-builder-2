...
export function getFunnelTemplateLibraryAssets() {
  ...
  Object.keys(SERVICE_IMAGE_TERMS_BY_SLUG).forEach((slug) => {
    buildTradeSpecificImagePool(slug).forEach((src, index) => {
      const normalized = String(src || '').trim();
      if (!normalized || seen.has(normalized)) return;
      seen.add(normalized);
      assets.push({
        id: `funnel-template-${slug}-${index + 1}`,
        name: `Funnel ${slug.replace(/-/g, ' ')} ${index + 1}`,
        type: 'image/jpeg',
        src: normalized,
      });
    });
  ...
  Object.entries(SERVICE_IMAGE_POOLS).forEach(([groupKey, urls]) => {
    urls.forEach((src, index) => {
      const normalized = String(src || '').trim();
      if (!normalized || seen.has(normalized)) return;
      seen.add(normalized);
      assets.push({
        id: `funnel-template-${groupKey}-${index + 1}`,
        name: `Funnel ${groupKey.replace(/-/g, ' ')} ${index + 1}`,
        type: 'image/jpeg',
        src: normalized,
      });
    });
