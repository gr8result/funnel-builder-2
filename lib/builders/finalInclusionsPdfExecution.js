...
export async function generateAndStoreFinalInclusionsPdf({
  ...
  const documentDraft = createFinalInclusionsDocumentVersion({
    snapshot: {
      ...snapshot,
      summary: {
        ...snapshot.summary,
  ...
  return {
    ...
    async registerDocument(document) {
      ...
      previousLatest.forEach((item) => {
        if (item.id !== document.id && item.status === "generated") item.status = "outdated";
      });
    ...
    latestDocument() {
      return [...documents].reverse().find((document) => document.latest && document.status === "generated") || null;
    },
  };
}
