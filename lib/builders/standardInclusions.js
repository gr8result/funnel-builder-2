import { createPremierInclusionsWorkingCopy } from "../standard-inclusions/masterTemplate.js";
...
export const STANDARD_INCLUSIONS_EDITOR_MODES = {
  ONLYOFFICE_DOCX: "onlyoffice-docx",
  ONLYOFFICE_PPTX: "onlyoffice-pptx",
  DOCUMENT_ENGINE: "document-engine",
  FINISHED_PDF: "finished-pdf",
  CANVA: "canva",
};

/**
 * @typedef {"onlyoffice-docx" | "onlyoffice-pptx" | "document-engine" | "finished-pdf"} StandardInclusionsEditorMode
 */

export function normaliseStandardInclusionsEditorMode(value = "") {
  const mode = String(value || "").trim();
  if (mode === "onlyoffice-docx" || mode === "onlyoffice-pptx" || mode === "document-engine" || mode === "finished-pdf" || mode === "canva") return mode;
  if (mode === "onlyoffice") return STANDARD_INCLUSIONS_EDITOR_MODES.ONLYOFFICE_PPTX;
  if (mode === "document-page-builder" || mode === "editable-canvas-pages") return STANDARD_INCLUSIONS_EDITOR_MODES.DOCUMENT_ENGINE;
  return "";
}
...
export function createDefaultStandardInclusions(builderId = "local-builder", options = {}) {
  const documentBuilder = options.documentBuilder || createPremierInclusionsWorkingCopy({ builderId });
  return {
    ...
    documentBuilder,
    source: documentBuilder?.metadata?.documentSource || "native-master-template",
    activeDocumentSource: documentBuilder?.metadata?.documentSource || "native-master-template",
  };
}

// Legacy PDF imports could auto-generate a "text" object per extracted PDF
// text item (named "Extracted text N"), scattering dozens of editable blocks
// over an imported page. That extraction code path has been removed, but
// schedules imported before the fix can still carry those stale objects in
// stored data. Strip them here — on every load of a "pdf-import" document —
// so they never render, and so the next save persists the cleaned page.
// Only "pdf-import" documents are touched; PPTX imports and manually created
// blocks are left untouched.
const LEGACY_PDF_EXTRACTED_TEXT_NAME = /^Extracted text \d+$/;

function stripLegacyPdfExtractedTextBlocks(documentBuilder) {
  if (!documentBuilder || documentBuilder.metadata?.documentSource !== "pdf-import") return documentBuilder;
  if (!Array.isArray(documentBuilder.pages) || !documentBuilder.pages.length) return documentBuilder;
  let changed = false;
  const pages = documentBuilder.pages.map((page) => {
    if (!Array.isArray(page?.objects) || !page.objects.length) return page;
    const objects = page.objects.filter((object) => !(object?.type === "text" && LEGACY_PDF_EXTRACTED_TEXT_NAME.test(object?.name || "")));
    if (objects.length === page.objects.length) return page;
    changed = true;
    return { ...page, objects };
  });
  return changed ? { ...documentBuilder, pages } : documentBuilder;
}
...
export function normaliseStandardInclusions(value = {}, builderId = "local-builder") {
  ...
  const editorMode = normaliseStandardInclusionsEditorMode(value.editorMode || value.pdfEditorMode || "");
  const hasDeletedMarker = Boolean(value.scheduleDeleted || value.isDeleted);
  ...
  const documentBuilder = stripLegacyPdfExtractedTextBlocks(
    hasDeletedMarker ? value.documentBuilder || null : value.documentBuilder || defaults.documentBuilder || null
  );
  const hasDocumentPages = Array.isArray(documentBuilder?.pages) && documentBuilder.pages.length > 0;
  const onlyOfficeDocumentId = value.onlyOfficeDocumentId || value.onlyofficeDocumentId || "";
  const hasOnlyOfficeDocument = Boolean(onlyOfficeDocumentId);
  const deleted = hasDocumentPages || hasOnlyOfficeDocument ? false : hasDeletedMarker;
  return {
    selectedPackageId,
    ...
    editorMode,
    pdfEditorMode: editorMode || value.pdfEditorMode || "",
    documentBuilder,
    onlyOfficeDocumentId,
    onlyOfficeVersion: Number(value.onlyOfficeVersion || 0),
    onlyOfficePptxAssetId: value.onlyOfficePptxAssetId || "",
    onlyOfficeOfficeAssetId: value.onlyOfficeOfficeAssetId || value.onlyOfficePptxAssetId || "",
    onlyOfficeFileType: value.onlyOfficeFileType || (editorMode === STANDARD_INCLUSIONS_EDITOR_MODES.ONLYOFFICE_DOCX ? "docx" : editorMode === STANDARD_INCLUSIONS_EDITOR_MODES.ONLYOFFICE_PPTX ? "pptx" : ""),
    onlyOfficeExportedPdfAssetId: value.onlyOfficeExportedPdfAssetId || "",
    canvaDocumentId: value.canvaDocumentId || "",
    canvaTemplateId: value.canvaTemplateId || "",
    canvaDesignId: value.canvaDesignId || "",
    canvaEditUrl: value.canvaEditUrl || value.canva?.editUrl || "",
    canvaThumbnailUrl: value.canvaThumbnailUrl || value.canva?.thumbnailUrl || "",
    canvaExportedPdfUrl: value.canvaExportedPdfUrl || "",
    canvaExportedPdfStorageKey: value.canvaExportedPdfStorageKey || "",
    canvaExportedPdfPageCount: Number(value.canvaExportedPdfPageCount || 0),
    finishedPdfDocumentId: value.finishedPdfDocumentId || "",
    finishedPdfStorageKey: value.finishedPdfStorageKey || "",
    finishedPdfUrl: value.finishedPdfUrl || "",
    finishedPdfPageCount: Number(value.finishedPdfPageCount || 0),
    source: value.source || value.activeDocumentSource || documentBuilder?.metadata?.documentSource || "",
    scheduleDeleted: deleted,
    isDeleted: deleted,
    deletedAt: deleted ? value.deletedAt || null : null,
    activeDocumentId: value.activeDocumentId || onlyOfficeDocumentId || documentBuilder?.id || "",
    activeDocumentName: value.activeDocumentName || documentBuilder?.name || "",
    activeDocumentSource: value.activeDocumentSource || value.source || documentBuilder?.metadata?.documentSource || "",
    activeDocumentLastSavedAt: value.activeDocumentLastSavedAt || documentBuilder?.metadata?.lastSavedAt || "",
    revisionHistory: Array.isArray(value.revisionHistory) ? value.revisionHistory : [],
