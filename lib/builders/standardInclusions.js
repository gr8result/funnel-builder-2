...
export function createDefaultStandardInclusions(builderId = "local-builder") {
  return {
    ...
    documentBuilder: createPremierInclusionsWorkingCopy({ builderId }),
    source: "native-master-template",
    activeDocumentSource: "native-master-template",
  };
}

const GENERATED_PDF_TEXT_NAME = /^Extracted text \d+$/;

function repairImportedPdfDetectionRegions(documentBuilder) {
  if (!documentBuilder || documentBuilder.metadata?.documentSource !== "pdf-import") return documentBuilder;
  if (!Array.isArray(documentBuilder.pages)) return documentBuilder;
  let changed = false;
  const pages = documentBuilder.pages.map((page) => {
    if (!Array.isArray(page?.objects)) return page;
    const detectedRegions = Array.isArray(page.data?.detectedRegions) ? [...page.data.detectedRegions] : [];
    const objects = page.objects.map((object) => {
      const generatedText = object?.type === "text" && GENERATED_PDF_TEXT_NAME.test(object.name || "");
      const activation = object?.data?.overlayMode === "pdf-text-activation" || object?.data?.detectedRegion === true;
      if (!generatedText && !activation) return object;
      const regionId = object.data?.regionId || object.id;
      if (!detectedRegions.some((region) => region.id === regionId)) {
        detectedRegions.push({
          ...
          detectedText: object.data?.text || "",
          confidence: Number(object.data?.confidence || 0),
        });
      }
      if (object.data?.edited || object.data?.acceptedEdit) return object;
      changed = true;
      return {
        ...object,
        data: {
...
export function normaliseStandardInclusions(value = {}, builderId = "local-builder") {
  ...
  const documentBuilder = repairImportedPdfDetectionRegions(hasDeletedMarker ? value.documentBuilder || null : value.documentBuilder || defaults.documentBuilder || null);
  ...
  return {
    ...
    pdfMaster: value.pdfMaster && typeof value.pdfMaster === "object" ? value.pdfMaster : null,
    pdfMasterHistory: Array.isArray(value.pdfMasterHistory) ? value.pdfMasterHistory : [],
