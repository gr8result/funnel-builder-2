...
export const NO_ACTIVE_STANDARD_INCLUSIONS_MASTER_CODE = "NO_ACTIVE_STANDARD_INCLUSIONS_MASTER";
...
export async function assignCurrentStandardInclusionsToProject({ workspaceId, projectId, userId, estimateId = "", force = false }) {
  ...
  if (!master) {
    const error = new Error("No active Standard Inclusions master PDF is available.");
    error.code = NO_ACTIVE_STANDARD_INCLUSIONS_MASTER_CODE;
