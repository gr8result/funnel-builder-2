...
const REMOVED_QUOTE_SECTION_NAMES = new Set(["upper level framing", "lock up materials", "quick render estimate", "project management", "entry doors", "entry doors - complete", "standard 820 entrace door", "plasterer", "fixout", "specials", "internal door complete", "internal cavity sliding door complete", "internal doors"]);
...
function appliancePackageQuoteSections() {
  ...
  appliancePackageRows.forEach((row) => {
    if (REMOVED_IMPORTED_QUOTE_SOURCE_ROWS.has(Number(row.sourceRow || 0))) return;
...
function insertFitOffCostSections(entries = []) {
  ...
  const withoutExisting = entries.filter(([sectionName]) => !sectionNames.has(normalizeQuoteSectionName(sectionName)));
  const sections = [
    plumbersFitOffSection(),
    generatedSingleRowSection(ELECTRICIANS_FIT_OFF_SECTION, 30077, "Electrician's fit off costs"),
  ];
  const section136Index = withoutExisting.findIndex(([, section]) => quoteFirstSourceRow(section?.rows || []) === 136);
  if (section136Index < 0) return [...withoutExisting, ...sections];
  return [
    ...withoutExisting.slice(0, section136Index + 1),
    ...sections,
    ...withoutExisting.slice(section136Index + 1),
  ];
}

function plumbersFitOffSection() {
  return [PLUMBERS_FIT_OFF_SECTION, {
    collapsed: true,
    columns: ["Item", "Qty", "Unit", "Rate", "Cost", "Source", "Notes"],
    rows: [
      manualQuoteRow({ sourceRow: 30080, item: "Plumber's Quote", unit: "QUOTE", rate: "", section: PLUMBERS_FIT_OFF_SECTION }),
      manualQuoteRow({ sourceRow: 30076, item: "Plumber's fit off costs", unit: "ITEM", rate: "", section: PLUMBERS_FIT_OFF_SECTION }),
    ],
  }];
}

function generatedSingleRowSection(sectionName, sourceRow, item) {
  return [sectionName, {
    collapsed: true,
    columns: ["Item", "Qty", "Unit", "Rate", "Cost", "Source", "Notes"],
    rows: [manualQuoteRow({ sourceRow, item, unit: "ITEM", rate: "", section: sectionName })],
  }];
}

function quoteFirstSourceRow(rows = []) {
  const row = rows.find((item) => item?.excelRow || item?.sourceRow || item?.values?.sourceRow);
  return row ? Number(row.excelRow || row.sourceRow || row.values?.sourceRow || 0) : 0;
}
