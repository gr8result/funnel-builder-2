...
import { getEffectiveApplianceCatalogue, getEffectiveCabinetryCatalogue } from "../product-library/catalogueService.js";
...
function cabinetMakerRow(existingRows, sectionName, row) {
  return manualReplacementQuoteRow(existingRows, sectionName, { ...row, ...cabinetryQuoteMappingForRow(row), quantity: "", unit: row.heading || row.total ? "" : row.unit, rate: row.heading || row.total ? "" : row.rate, forceItem: true, cabinetMakerTotalRow: Boolean(row.total) });
}

function cabinetryQuoteMappingForRow(row = {}) {
  if (row.heading || row.total) return { canonicalMappingStatus: "not_catalogue_record" };
  const item = normalizeQuoteSectionName(row.item);
  const product = cabinetryQuoteProductForText(item);
  if (!product) {
    return {
      canonicalMappingStatus: "review_required",
      productLibraryFamilyKey: /stone|benchtop|tops/.test(item) ? "stone-benchtops" : "cabinetry",
      sourceOfRate: "workbook",
    };
  }
  return {
    canonicalProductId: product.productCode || product.productId,
    ...
    productImageUrl: productLibraryImageReference(product),
    thumbnailUrl: product.thumbnailUrl || product.primaryImageUrl || "",
    productLibraryFamilyKey: product.familyKey,
    productLibraryCatalogueOwner: product.isCustom ? "builder-private" : "product-library",
    productLibrarySnapshot: productLibraryQuoteSnapshot(product),
...
function cabinetryQuoteProductForText(item = "") {
  const products = cabinetryQuoteProducts();
  const byCode = (code) => products.find((product) => product.productCode === code);
  if (/soft close/.test(item)) return byCode("CABINETRY-HARDWARE-BLUM-SOFT-CLOSE");
  if (/bulkhead|craftwood/.test(item)) return byCode("CABINETRY-BULKHEAD-RAW-MDF");
  if (/hanging rail/.test(item)) return byCode("CABINETRY-ROBE-HANGING-RAIL") || byCode("CABINETRY-SHELVING-CLEATED");
  if (/open shelves|extra shelf|bank of shelves|shelf/.test(item)) return byCode("CABINETRY-SHELVING-CLEATED") || byCode("CABINETRY-SHELVING-OPEN");
  if (/dishwasher/.test(item)) return byCode("CABINETRY-UNIT-DISHWASHER-CABINET") || byCode("CABINETRY-APPLIANCE-PANEL-DISHWASHER");
  if (/fridge/.test(item)) return byCode("CABINETRY-APPLIANCE-PANEL-FRIDGE");
  if (/rangehood/.test(item)) return byCode("CABINETRY-UNIT-RANGEHOOD-CABINET");
  if (/microwave/.test(item)) return byCode("CABINETRY-UNIT-MICROWAVE-CABINET");
  if (/oven/.test(item)) return byCode("CABINETRY-UNIT-UNDERBENCH-OVEN-CABINET");
  if (/sink|tub cupboard/.test(item)) return byCode("CABINETRY-UNIT-SINK-CUPBOARD");
  if (/corner/.test(item)) return byCode("CABINETRY-UNIT-CORNER-UNIT");
  if (/overhead/.test(item)) return byCode("CABINETRY-UNIT-OVERHEAD");
  if (/pantry/.test(item)) return byCode("CABINETRY-UNIT-TALL-PANTRY");
  if (/pot draw|pot drawer/.test(item)) return byCode("CABINETRY-UNIT-TWO-BANK-POT-DRAWERS") || byCode("CABINETRY-UNIT-THREE-BANK-POT-DRAWERS-ONE-SMALL-AND-TWO-LARGE");
  if (/drawer|drawers/.test(item)) return byCode("CABINETRY-UNIT-FOUR-BANK-DRAWERS") || byCode("CABINETRY-UNIT-FIVE-BANK-DRAWERS");
  if (/vanity/.test(item)) return byCode("CABINETRY-UNIT-BATH-FLOOR-TWO-DOOR");
  if (/washing|dryer|washer/.test(item)) return byCode("CABINETRY-LAUNDRY-WASHING-MACHINE-PROVISION");
  if (/robe|wir|linen/.test(item)) return byCode("CABINETRY-ROBE-HANGING-RAIL") || byCode("CABINETRY-SHELVING-CLEATED");
  if (/island|base|cabinet|cupboard|misc cabinetry|misc extras/.test(item)) return byCode("CABINETRY-UNIT-STANDARD-BASE");
  return null;
}

let cabinetryQuoteProductCache = null;

function cabinetryQuoteProducts() {
  if (!cabinetryQuoteProductCache) {
    cabinetryQuoteProductCache = getEffectiveCabinetryCatalogue({ includeDisabled: true }).products;
  }
  return cabinetryQuoteProductCache;
}
