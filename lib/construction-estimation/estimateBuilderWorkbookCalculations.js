...
export function calculateEstimateBuilderWorkbook(workbook) {
  ...
  const isRenderedBrickVeneer = lowerSystem === "Rendered Brick Veneer";
  const isFaceBrickVeneer = lowerSystem === "Face Brick Brick Veneer";
  ...
  quantities.quoteFrameInstallWindows = quantities.windowDoorCount;
  ...
  const quotationResult = calculateQuotation(workbook.quotation, quantities, { frameMethod, windowsDoors: wd });
...
function calculateQuotation(quotation, quantities, options = {}) {
  ...
  const calculatedQuotation = Object.fromEntries(Object.entries(quotationWithWindowDoors).map(([sectionName, section]) => {
    ...
    const rows = removeRemovedImportedQuoteRows(removeRoofingMaterialsRemovedRows(sectionName, section.rows)).map((row) => {
      ...
      const notes = quoteNotesWithImportedData(sourceNotes, Boolean(quoteRowSourceNumber(qtyRow) !== 116 && !manualQty && (formulaQty || linkedQty || floorSystemLinkedQty) && qty));
...
function isBlankQtyQuoteSectionName(sectionName) {
  const name = normalizedSectionName(sectionName);
  return ["demolition works", "base brickwork", "face brickwork", "bricklayers labour", "entry doors", "double entry doors", "windows", "couplings", "misc", "materials", "roofing materials", "renderers labour", "misc rendering"].includes(name) || name.startsWith("roof cover");
}
...
function isBrick(system) { return ["Brick Veneer", "Rendered Brick Veneer", "Face Brick Brick Veneer", "Blockwork"].includes(system); }
