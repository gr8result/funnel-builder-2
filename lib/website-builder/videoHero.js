...
export const VIDEO_HERO_UPLOAD_FIELD_KEYS = [
  "__video_hero_src__",
  "videoUrl",
  "video",
  "videoSrc",
...
export function isVideoHeroMediaFieldKey(value = "") {
  return VIDEO_HERO_UPLOAD_FIELD_KEYS.includes(String(value || ""));
}
...
export function hasVideoHeroBlock(blocks = []) {
  return Array.isArray(blocks) && blocks.some((block) => String(block?.type || "") === "video-hero");
}

export function syncVideoHeroChaiDataBlocks(chaiData, blocks = []) {
  if (!hasVideoHeroBlock(blocks)) return chaiData;
  const normalizedBlocks = normalizeVideoHeroBlocksForPersistence(blocks);
  return {
    ...(chaiData && typeof chaiData === "object" && !Array.isArray(chaiData) ? chaiData : {}),
