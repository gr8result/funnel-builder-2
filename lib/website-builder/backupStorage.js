...
// Supabase (website_builder_sites / website_builder_pages / website_builder_page_versions) is the
// durable store. These filesystem snapshots are a local development recovery aid only -- production
// must never depend on, or write customer content into, the repo filesystem.
const IS_PRODUCTION_RUNTIME = process.env.NODE_ENV === "production" || process.env.VERCEL === "1";
const BACKUP_RETENTION_COUNT = Math.max(1, Number(process.env.WEBSITE_BUILDER_BACKUP_RETENTION) || 5);
...
export async function createWebsiteBuilderBackup(accountId, projectId, options = {}) {
  if (IS_PRODUCTION_RUNTIME) {
    return { id: null, path: null, metadata: null, skipped: true, reason: "production runtime: filesystem backups are disabled, Supabase is authoritative" };
  }
  ...
  await pruneOldBackups(safeAccountId, safeProjectId, BACKUP_RETENTION_COUNT).catch((error) => {
    console.warn("[website-builder backups] retention prune failed", { accountId: safeAccountId, projectId: safeProjectId, error: error?.message || error });
  });
...
// Keeps only the newest `retain` snapshots for a project, oldest-first deletion.
// Never removes the single newest snapshot, even if retain is set to 0.
export async function pruneOldBackups(accountId, projectId, retain = BACKUP_RETENTION_COUNT) {
  const keep = Math.max(1, Number(retain) || 1);
  const backups = await listWebsiteBuilderBackups(accountId, projectId);
  const toRemove = backups.slice(keep);
  for (const backup of toRemove) {
    await fs.rm(backup.path, { recursive: true, force: true });
  }
  return { kept: Math.min(keep, backups.length), removed: toRemove.length, removedPaths: toRemove.map((b) => b.path) };
}
