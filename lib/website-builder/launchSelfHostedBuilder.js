...
import { getTemplatePageDefinitions } from "./templates";

function normalizePages(pages, buildType = "website", mode = "blank", templateSlug = "") {
  if (Array.isArray(pages) && pages.length) return pages;

  const templatePages = getTemplatePageDefinitions(templateSlug);
  if (templatePages.length) return templatePages;
...
export async function openSelfHostedBuilder(options = {}) {
  ...
  const normalizedMode = String(options.mode || "blank");
  const initialStatus = options.status || (normalizedMode === "ai" || normalizedMode === "import" ? "saved" : "unsaved");
  ...
  const project = createWebsiteProject({
    ...
    brief: options.brief || {},
    pages: normalizePages(options.pages, options.buildType, options.mode, options.templateSlug || options.theme || ""),
    pagesContent: options.pagesContent || {},
    pageBlocks: options.pageBlocks || {},
    globalNavBlock: options.globalNavBlock || null,
    globalFooterBlock: options.globalFooterBlock || null,
    brandAssets: options.brandAssets || { logo: null, images: [] },
