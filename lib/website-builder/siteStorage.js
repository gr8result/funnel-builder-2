...
export async function saveSplitWebsiteProject(accountId, project, options = {}) {
  ...
  if (existingSite && selectedPageName) {
    ...
    if (existingPage) {
      ...
      if (shouldPreserveExistingPageBlocks(existingPageDoc?.blocks, incomingBlocks, backupSource)) {
        project = {
          ...project,
          pageBlocks: {
            ...(project?.pageBlocks || {}),
            [existingPage.name]: existingPageDoc.blocks,
          },
          pagesContent: {
            ...(project?.pagesContent || {}),
            [existingPage.name]: existingPageDoc.html || "",
          },
          chaiData: existingPageDoc.chaiData
            ? {
                ...(project?.chaiData || {}),
                [existingPage.name]: existingPageDoc.chaiData,
              }
            : project?.chaiData,
        };
      }
