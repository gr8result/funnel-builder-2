...
function buildPageDocument(project, page, existingPageDoc = null) {
  ...
  const rawBlocks = hasIncomingPageBlocks
    ? (Array.isArray(project.pageBlocks[name]) ? project.pageBlocks[name] : [])
    : !hasPageBlocksMap && Array.isArray(chaiData?.blocks)
      ? chaiData.blocks
  ...
  const restoredChaiData = hasIncomingChaiData
    ? restorePreservedDataUrls(chaiData, existingPageDoc?.chaiData)
    : chaiData;
  const nextChaiData = {
    ...(restoredChaiData && typeof restoredChaiData === "object" && !Array.isArray(restoredChaiData) ? restoredChaiData : {}),
    blocks,
  };
...
export async function saveSplitWebsiteProject(accountId, project, options = {}) {
  ...
  const currentRevision = readProjectRevision(existingSite || {});
  ...
  const currentPageRevisions = existingSite?.pageRevisions && typeof existingSite.pageRevisions === "object" ? existingSite.pageRevisions : {};
  const nextPageRevisions = { ...currentPageRevisions };
  const selectedPageNameForRevision = pageNameFromValue(options?.pageName || "");
  const pagesForRevision = normalizePages({ ...(existingSite || {}), ...(project || {}), id: siteId });
  ...
  for (const page of revisionPages) {
    nextPageRevisions[page.name] = revisionNumber(currentPageRevisions[page.name], 0) + 1;
  }
  project = {
    ...(project || {}),
  ...
  let site = buildSiteDocument({ ...(existingSite || {}), ...(project || {}), id: siteId }, accountId);
  ...
  project = normalizeSharedPrimaryNavigation(project || {});
  site = buildSiteDocument({ ...(existingSite || {}), ...(project || {}), id: siteId }, accountId);
  site.updatedAt = now;
