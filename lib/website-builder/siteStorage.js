  ...
  const rawBlocks = hasIncomingPageBlocks
    ...
    : !hasPageBlocksMap && Array.isArray(chaiData?.blocks)
      ...
      : Array.isArray(existingPageDoc?.blocks)
        ...
        : [];
  const restoredBlocks = restorePreservedDataUrls(rawBlocks, existingPageDoc?.blocks);
  const blocks = preserveExistingTestimonialAvatarUrls(restoredBlocks, existingPageDoc?.blocks || []);
  const preservedChaiData = hasIncomingChaiData
    ? preserveExistingTestimonialChaiAvatarUrls(
        restorePreservedDataUrls(chaiData, existingPageDoc?.chaiData),
        blocks,
        existingPageDoc?.chaiData || null
      )
    : chaiData;
  const nextChaiData = {
...
function hydrateProject(site, pageDoc = null) {
  ...
  return repairProjectTestimonialAvatarUrls(normalizeSharedBlockTemplateProject(normalizeSharedPrimaryNavigation(project)));
}
...
export async function loadFullSplitWebsiteProject(accountId, siteId) {
  ...
  return repairProjectTestimonialAvatarUrls(project);
}
