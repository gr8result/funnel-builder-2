...
function normalizeNavPath(value = "") {
  ...
  if (!raw) return "";
  if (raw === "/") return "/";
  if (/^(mailto:|tel:|https?:\/\/)/i.test(raw) || raw.startsWith("#")) return raw;
  const clean = raw.replace(/^\/+/, "").split(/[?#]/)[0];
  return clean ? `/${slugifyNavValue(clean)}` : "";
}
...
export function normalizeSharedNavigationLinks(links = [], project = {}) {
  ...
  const shouldRestorePageLinks = Number(project?.globalNavBlock?.props?.globalHeaderSchemaVersion || 0) >= GLOBAL_HEADER_SCHEMA_VERSION
    || project?.globalNavBlock?.props?.syncPagesToNavigation === true;
  const seen = new Set();
  ...
  const normalizeItem = (item, index, childIndex = null) => {
    ...
    const key = canonicalNavKey(item, pageRef);
    ...
    const href = pageRef?.href || normalizeNavPath(item.href || item.url || item.path || "");
    const children = (Array.isArray(item.children) ? item.children : [])
      .map((child, idx) => normalizeItem(child, index, idx))
      .filter(Boolean);

    if (pageId) seenPageIds.add(String(pageId));
    if (slug) seenPageSlugs.add(String(slug));
  ...
  const visiblePageLinks = (Array.isArray(project?.pages) ? project.pages : [])
    .map((page, index) => ({ page, index, ref: buildPageNavRef(page, index) }))
    .filter(({ page, ref }) => ref && pageIsVisibleInNavigation(page))
    .sort((a, b) => a.ref.order - b.ref.order || a.index - b.index);

  if (!shouldRestorePageLinks) return normalized;

  visiblePageLinks.forEach(({ ref }) => {
    if (seenPageIds.has(ref.id) || seenPageSlugs.has(ref.slug)) return;
    const hrefKey = `href:${ref.href.toLowerCase()}`;
    const slugKey = `slug:${ref.slug}`;
    const pageKey = `page:${ref.id}`;
    if (seen.has(hrefKey) || seen.has(slugKey) || seen.has(pageKey)) return;
    seen.add(pageKey);
    seen.add(slugKey);
    seen.add(hrefKey);
    seenPageIds.add(ref.id);
    seenPageSlugs.add(ref.slug);
    normalized.push({
      id: `nav-page-${ref.slug}`,
      label: ref.label,
      href: ref.href,
      linkType: "page",
      pageId: ref.id,
...
function selectCanonicalNavigation(project = {}) {
  const candidates = [
    ...(isPrimaryNavigationBlock(project.globalNavBlock) ? [{ block: project.globalNavBlock, global: true, pageIndex: -1, blockIndex: -1 }] : []),
