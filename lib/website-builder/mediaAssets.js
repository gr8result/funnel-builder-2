  ...
  return Array.isArray(items)
    ? items
        ...
        .map((asset, index) => {
          ...
          return {
            id: asset.id || storagePath || `legacy-${fallbackName.toLowerCase()}-${index}`,
            name: asset.name || asset.description || `${fallbackName} ${index + 1}`,
            type: asset.type || asset.mimeType || asset.mime_type || inferMimeType(asset.name || src || "") || fallbackType,
            src,
            ...(storagePath ? { storagePath, storage_path: storagePath } : {}),
            ...(asset.size || asset.fileSize ? { size: asset.size || asset.fileSize } : {}),
            ...(asset.createdAt || asset.created_at ? { createdAt: asset.createdAt || asset.created_at } : {}),
          };
        })
...
export function mergeWebsiteBuilderAssetSources(...sources) {
  ...
  normalizedSources.forEach((source) => {
    (source?.images || []).forEach((image) => {
      ...
      images.push({
        id: image.id || storagePath || makeAssetId(image.name || "image"),
    ...
    (source?.videos || []).forEach((video) => {
      ...
      videos.push({
        id: video.id || storagePath || makeAssetId(video.name || "video"),
...
export function normalizeSelectedAsset(asset) {
  ...
  return {
    ...asset,
    id: asset.id || storagePath || asset.url || asset.name || "",
    src,
    ...(storagePath ? { storagePath, storage_path: storagePath } : {}),
  };
}
