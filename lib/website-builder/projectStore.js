...
function normalizeIncomingWebsiteProject(project) {
  ...
  const baseProject = {
    ...
    brief: project?.brief || {},
    pages: Array.isArray(project?.pages) && project.pages.length
      ? project.pages
      : getTemplatePageDefinitions(project?.templateSlug || ""),
...
function createBlock(type, props = {}) {
  return {
    id: createId(),
    type,
    props: type === BlockTypes.HERO
      ? stripLegacyHeroOverlayProps({
...
function buildHomeBlocks(project, page, templateBlocks = []) {
  ...
  return autoBalanceComposition([
    ...
    createBlock(BlockTypes.STATS, {
      title: "The visual-first launch stack",
      subtitle: `Built to give ${audience} a stronger first impression and a cleaner path to yes.`,
      stats: [
        { number: "Bold", label: "Colour-led positioning", detail: "A more expressive surface so the page feels designed, not assembled." },
        { number: "Visual", label: "Multiple image moments", detail: "Hero, layered visuals, gallery, and proof sections all work together." },
        { number: "Animated", label: "Movement with purpose", detail: "Entrance motion highlights hierarchy without turning into noise." },
      ],
  ...
  switch (block?.type) {
    ...
    case BlockTypes.PARALLAX:
      return `<section style="position:relative;overflow:hidden;padding:88px 20px;min-height:${escapeHtml(props.minHeight || "480px")};background:${props.backgroundImage ? `linear-gradient(rgba(15,23,42,0.45), rgba(15,23,42,0.22)), url('${escapeHtml(props.backgroundImage)}') center/cover no-repeat` : (props.backgroundColor || style.heroBg || "#1d4ed8")};color:${props.headlineColor || "#ffffff"};text-align:${escapeHtml(props.headlineAlignment || "center")};">
        ...
        <div style="position:relative;z-index:1;max-width:${escapeHtml(String(props.contentWidth || 940))}px;margin:0 auto;overflow:hidden;">
          <h1 style="margin:0 0 16px;font-size:${escapeHtml(String(props.headlineFontSize || 52))}px;line-height:1.08;word-break:break-word;overflow-wrap:anywhere;">${inlineRichTextToHtml(props.headline || "Your headline")}</h1>
          <div style="font-size:${escapeHtml(String(props.subheadlineFontSize || 20))}px;line-height:1.7;color:${props.textColor || "#dbeafe"};word-break:break-word;overflow-wrap:anywhere;">${richTextToHtml(props.subheadline || "Describe what this page offers.")}</div>
    ...
    case BlockTypes.TEXT:
      return `<section style="background:${props.backgroundColor || style.textBg || "#ffffff"};color:${props.textColor || style.textColor || "#0f172a"};">
        <div style="${sectionBase}">${richTextToHtml(props.text || "Add your content here.")}</div>
      </section>`;
...
function prepareProjectContent(project) {
  ...
  const pageBlocks = Object.fromEntries(
    Object.entries(pageBlocksSource).map(([pageName, blocks]) => {
      ...
      return [pageName, isPlatformPricingPageName(pageName) ? syncPlatformPricingBlocks(normalizedBlocks) : normalizedBlocks];
    })
  );
