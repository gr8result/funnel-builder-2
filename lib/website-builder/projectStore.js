...
function sanitizeValueForStorage(value, options = {}) {
  ...
  if (Array.isArray(value)) {
    return value
      ...
      .map((entry) => sanitizeValueForStorage(entry, options));
  }
  ...
  for (const [key, entry] of Object.entries(value)) {
    if (typeof entry === "string" && /^data:image\//i.test(entry)) {
      ...
      next[key] = hasAssetRef || compact ? "" : entry;
      continue;
    }

    next[key] = sanitizeValueForStorage(entry, options);
  }
...
export function ensureWebsiteProjectPageBlocks(project) {
  ...
  if (templateSlug) {
    try {
      templateSections = asArray(getTemplate(templateSlug)?.build?.("modern-blue")?.pages?.[0]?.sections);
    } catch {
      templateSections = [];
    }
  }

  const homeTemplateBlocks = templateSections.flatMap((section) =>
...
function prepareProjectContent(project) {
  ...
  const pageBlocks = ensuredBlocks || (project?.pageBlocks && typeof project.pageBlocks === "object" ? project.pageBlocks : {});
  ...
  return { pagesContent, pageBlocks };
}
...
export function saveWebsiteBuilderAssets(assets) {
  ...
  for (const limit of [MAX_STORED_ASSETS, 12, 8, 6, 4, 2, 1, 0]) {
...
export function getWebsiteProject(id) {
  ...
  if (JSON.stringify(project.pagesContent || {}) !== JSON.stringify(migrated.pagesContent || {}) || JSON.stringify(project.pageBlocks || {}) !== JSON.stringify(migrated.pageBlocks || {})) {
