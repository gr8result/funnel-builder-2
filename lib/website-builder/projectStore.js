...
function normalizeNavigationLinks(links, { removeDuplicates = true, pages = [] } = {}) {
  ...
  const normalizeItem = (item, index, childIndex = null) => {
    ...
    const label = matchedPage?.label || String(item?.label || "").trim();
    const pageId = matchedPage?.id || String(item?.pageId || item?.page_id || "").trim();
    const pageSlug = matchedPage?.slug || slugify(item?.slug || item?.pageSlug || label || item?.href || "");
    const href = matchedPage?.href || (linkType === "none" ? "" : normalizeNavHref(item?.href, label, pageSlug));
