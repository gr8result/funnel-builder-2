...
export function ensureWebsiteProjectPageBlocks(project) {
  ...
  pages.forEach((page, index) => {
    ...
    if (Array.isArray(nextPageBlocks[page.name]) && nextPageBlocks[page.name].length) {
      ...
      if (shouldRefreshAiStarterBlocks({ ...project, templateSlug }, page, nextPageBlocks[page.name])) {
        nextPageBlocks[page.name] = index === 0
...
export function listWebsiteProjects(options = {}) {
  ...
  const parsed = safeParse(raw || "[]");
  const projects = importLegacyWebsiteProjects(Array.isArray(parsed) ? parsed : []);
...
export function cacheWebsiteProjects(projects, options = {}) {
  ...
  const includeUnsaved = options?.includeUnsaved !== false;
  const onlyIfNewer = options?.onlyIfNewer !== false;
  // When replaceAll is true the server list is authoritative — stale local drafts
  // that were deleted on the server will not be re-merged from localStorage.
