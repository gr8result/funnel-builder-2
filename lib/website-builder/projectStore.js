  ...
  switch (block?.type) {
    ...
    case BlockTypes.FEATURE_LIST: {
      const featureItems = asArray(props.items).map((item, idx) => {
        ...
        return `<article style="display:flex;flex-direction:column;gap:14px;padding:18px;border-radius:22px;background:${escapeHtml(props.itemBackgroundColor || "#eff6ff")};border:1px solid ${escapeHtml(props.borderColor || "#dbeafe")};box-shadow:0 18px 42px rgba(15,23,42,0.16);min-height:100%;box-sizing:border-box;">
      ...
      const featureColumns = props.layout === "columns" ? "repeat(auto-fit,minmax(240px,1fr))" : "1fr";
      return `<section style="background:${escapeHtml(props.backgroundColor || style.textBg || "#ffffff")};color:${escapeHtml(props.textColor || style.textColor || "#0f172a")};${spacingCss(props)}"><div style="${sectionBase}"><h2 style="margin:0 0 24px;font-size:34px;text-align:${escapeHtml(props.headingAlign || props.headlineAlignment || props.alignment || "left")};">${richTextToHtml(props.title || "Features")}</h2><div style="display:grid;grid-template-columns:${featureColumns};gap:18px;">${featureItems}</div></div></section>`;
