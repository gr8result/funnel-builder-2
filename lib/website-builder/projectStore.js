...
import { applyGr8AustralianFooterPanel, buildFooterLinksFromPages, buildFooterNavigationContext, footerBlockToGlobalFooter, globalFooterToFooterBlock, normalizeFooterNavigationBlock } from "./footerNavigation";
import { normalizeSharedPrimaryNavigation } from "./sharedNavigation";
import { normalizeSharedBlockTemplateProject, resolveCtaOpenInNewTab, resolveSharedBlockInstance } from "./sharedBlockTemplates";
import { resolveAccordionPanelImageUrl } from "./accordionPanels";
import { resolveBlockImageUrl } from "./blockImageResolver";
import { hasGlobalPageWidthMode, normalizePageLayoutProject, resolveGlobalPageWidthMode, withPageLayoutDefaults } from "./pageLayout";
import { repairProjectTestimonialAvatarUrls } from "./testimonialImages";
...
const GR8_RESULT_WEBSITE_ID = "2208a52a-8175-477e-823c-fc6de7fe4afe";
...
function sanitizeProjectForStorage(project, options = {}) {
  ...
  if (project.globalFooter) {
    safeProject.globalFooter = sanitizeValueForStorage(project.globalFooter, options);
  }
...
function normalizeIncomingWebsiteProject(project) {
  ...
  const baseProject = normalizePageLayoutProject({
    ...project,
    ...
    brief: project?.brief || {},
    ...(hasGlobalPageWidthMode(project) ? {
      pageWidthMode: resolveGlobalPageWidthMode(project),
      globalPageWidthMode: resolveGlobalPageWidthMode(project),
    } : {}),
    ...
    sharedBlockTemplates: project?.sharedBlockTemplates || project?.sharedTemplates || {},
    globalNavBlock: project?.globalNavBlock || null,
    globalFooterBlock: project?.globalFooterBlock || globalFooterToFooterBlock(project?.globalFooter, null) || null,
    globalFooter: project?.globalFooter || footerBlockToGlobalFooter(project?.globalFooterBlock, buildFooterNavigationContext({ pages: project?.pages || [] })) || null,
  });

  const prepared = prepareProjectContent(baseProject);
  return normalizePageLayoutProject({
    ...baseProject,
    ...
    globalFooter: footerBlockToGlobalFooter(
      "globalFooterBlock" in prepared ? prepared.globalFooterBlock : baseProject.globalFooterBlock,
      buildFooterNavigationContext({ pages: baseProject.pages || [] })
    ) || baseProject.globalFooter || null,
  });
}
...
function normalizeStoredBlock(block, options = {}) {
  ...
  if (type === BlockTypes.NAV_BAR) {
    ...
    const normalizedLinks = normalizeNavigationLinks(currentLinks, { pages: options?.project?.pages });
    const manualNavigation = currentProps.navigationManual !== false;
    if (JSON.stringify(currentLinks) !== JSON.stringify(normalizedLinks)) {
      nextProps.links = normalizedLinks;
    }
    nextProps.navigationManual = manualNavigation;
  }

  if (type === BlockTypes.FOOTER) {
    const currentNavLinks = Array.isArray(currentProps.navigationLinks)
      ? currentProps.navigationLinks
      : Array.isArray(currentProps.navLinks)
        ? currentProps.navLinks
        : [];
    const footerContext = buildFooterNavigationContext({ pages: options?.project?.pages, logInvalid: true });
    const normalizedFooter = normalizeFooterNavigationBlock({ ...block, props: currentProps }, footerContext);
    Object.assign(nextProps, normalizedFooter.props || {});
    if (currentNavLinks.length === 0 && !currentProps.footerNavManual) {
      const pageLinks = buildFooterLinksFromPages(options?.project?.pages);
      if (pageLinks.length) nextProps.navigationLinks = pageLinks;
    }
  }
...
function normalizeProjectPages(pages, templateSlug = "") {
  ...
  return (normalized.length ? normalized : [{ name: "Home", slug: "home", order: 0 }]).map(withPageLayoutDefaults);
}

// Pages created before the preview/route slug field existed have no `page.slug`.
// Preview URLs must resolve against a saved, stable slug rather than deriving one
// from the display name on every click (see resolvePreviewPageSlug in
// PageBuilderCanvas.js and resolveProjectPageEntry in the preview route), so any
// page missing a slug gets one backfilled here — once, deterministically, and
// persisted by the caller — instead of failing preview forever.
function ensurePageSlugs(pages) {
  const list = Array.isArray(pages) ? pages : [];
  const seen = new Set(
    list.map((page) => String(page?.slug || "").trim()).filter(Boolean)
  );
  let changed = false;
  const next = list.map((page) => {
    if (page && typeof page === "object" && String(page.slug || "").trim()) return page;
    const base = slugify(pageNameFromValue(page) || "page") || "page";
    let candidate = base;
    let suffix = 2;
    ...
    seen.add(candidate);
    changed = true;
    return { ...(page && typeof page === "object" ? page : {}), slug: candidate };
  });
...
function findPageRefForNavigationLink(item, pageContext) {
  const pageMap = pageContext?.pageMap;
  if (!pageMap?.size || !item || typeof item !== "object") return null;
  const keys = [
    item.pageId,
    item.page_id,
    item.internalPageId,
  ...
  for (const value of keys) {
    const key = slugify(String(value || "").replace(/^\/+/, "").split(/[?#]/)[0]) || String(value || "").trim();
    if (key && pageMap.has(key)) return pageMap.get(key);
  }
  return null;
}

function normalizeNavigationLinks(links, { removeDuplicates = true, pages = [] } = {}) {
  const pageContext = buildFooterNavigationContext({ pages });
  const seenIds = new Set();
  const seenKeys = new Set();

  const normalizeItem = (item, index, childIndex = null) => {
    const matchedPage = findPageRefForNavigationLink(item, pageContext);
    const explicitLinkType = String(item?.linkType || "").trim().toLowerCase();
    const linkType = matchedPage ? "page" : explicitLinkType || "";
    const explicitLabel = String(item?.label || "").trim();
    const label = explicitLabel || matchedPage?.label || "";
    const pageId = String(item?.pageId || item?.page_id || matchedPage?.id || "").trim();
    const pageSlug = String(item?.slug || item?.pageSlug || matchedPage?.slug || slugify(label || item?.href || "")).trim();
    const explicitHref = normalizeNavHref(item?.href, explicitLabel, pageSlug);
    const href = linkType === "none" ? "" : (explicitHref || matchedPage?.href || "");
    ...
    return {
      ...
      label: label || "Link",
      href: href || "",
      linkType: linkType || (pageId ? "page" : href?.startsWith("#") ? "anchor" : /^(https?:|mailto:|tel:)/i.test(href) ? "external" : "custom"),
      ...(pageId ? { pageId } : {}),
...
function normalizeGlobalBlocks(project) {
  ...
  const normalizedProject = normalizeSharedPrimaryNavigation(project);
  ...
  if (normalizedProject.globalNavBlock && normalizedProject.globalNavBlock.type !== BlockTypes.NAV_BAR) {
    patch.globalNavBlock = null;
  } else if (normalizedProject.globalNavBlock?.type === BlockTypes.NAV_BAR) {
    const currentLinks = Array.isArray(normalizedProject.globalNavBlock?.props?.links)
      ? normalizedProject.globalNavBlock.props.links
      : [];
    const nextLinks = normalizeNavigationLinks(currentLinks, { pages });
    if (JSON.stringify(project.globalNavBlock || null) !== JSON.stringify(normalizedProject.globalNavBlock || null) || JSON.stringify(currentLinks) !== JSON.stringify(nextLinks)) {
      patch.globalNavBlock = {
        ...normalizedProject.globalNavBlock,
        props: {
          ...(normalizedProject.globalNavBlock.props || {}),
          links: nextLinks,
        },
      };
    }
  ...
  if (project.globalFooterBlock && project.globalFooterBlock.type !== BlockTypes.FOOTER) {
    ...
    const footerContext = buildFooterNavigationContext({ pages, logInvalid: true });
    let nextFooterBlock = normalizeFooterNavigationBlock(project.globalFooterBlock, footerContext);
    if (String(project.id || "").replace(/^draft:/, "") === GR8_RESULT_WEBSITE_ID) {
      nextFooterBlock = applyGr8AustralianFooterPanel(nextFooterBlock, footerContext);
    }
    if (JSON.stringify(project.globalFooterBlock) !== JSON.stringify(nextFooterBlock)) {
      patch.globalFooterBlock = {
        ...nextFooterBlock,
        props: { ...(nextFooterBlock.props || {}), footerNavManual: true },
      };
    }
  }
...
export function updateWebsiteTemplateOverride(templateSlug, patch = {}) {
  ...
  const next = sanitizeTemplateOverrideForStorage(key, {
    ...
    globalFooter: Object.prototype.hasOwnProperty.call(patch, "globalFooter")
      ? patch.globalFooter
      : footerBlockToGlobalFooter(
          Object.prototype.hasOwnProperty.call(patch, "globalFooterBlock") ? patch.globalFooterBlock : current.globalFooterBlock,
          buildFooterNavigationContext({ pages: patch.pages || current.pages || [] })
        ) || current.globalFooter || null,
...
function withScaffolding(blocks, project) {
  ...
  const businessName = project?.brief?.businessName || project?.name || "Your Business";
  const style = getStyleTokens(project);
  const hasCanonicalGlobalNav = !!(project?.globalNavBlock || project?.globalHeader?.enabled || project?.globalHeader?.id);
  if (hasCanonicalGlobalNav) return autoBalanceComposition(list);
...
export function ensureWebsiteProjectPageBlocks(project) {
  ...
  pages.forEach((page, index) => {
    ...
    if (Array.isArray(nextPageBlocks[page.name]) && nextPageBlocks[page.name].length) {
      ...
      const upgraded = applyDeletedBlockTombstones(
        withScaffolding(nextPageBlocks[page.name], project),
        normalizeDeletedBlockTombstones(project),
        page.name
      );
...
function normalizeStaticAccordionImageFit(value = "contain") {
  const fit = String(value || "contain").toLowerCase().trim();
  if (fit === "contain" || fit === "cover" || fit === "fill" || fit === "none" || fit === "scale-down") return fit;
  return "contain";
}

function normalizeStaticAccordionObjectPosition(value = "center center") {
...
function clampStaticImagePositionPercent(value, fallback) {
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) return fallback;
  return Math.max(0, Math.min(100, Math.round(numeric)));
}

function resolveStaticAccordionObjectPosition(source = {}) {
  ...
  if (x !== undefined && x !== null && x !== "" && y !== undefined && y !== null && y !== "") {
    return `${clampStaticImagePositionPercent(x, 50)}% ${clampStaticImagePositionPercent(y, 50)}%`;
  }
  return normalizeStaticAccordionObjectPosition(source.imageObjectPosition || source.objectPosition || "center center");
}

function resolveStaticAccordionAlign(value = "top") {
  const align = String(value || "top").toLowerCase();
  if (align === "bottom" || align === "end") return { grid: "end", flex: "flex-end" };
  if (align === "center" || align === "centre") return { grid: "center", flex: "center" };
  return { grid: "start", flex: "flex-start" };
}

function staticObjectPositionFlexAlign(objectPosition = "center center") {
...
function staticAccordionImageSetting(props, panel, key, fallback) {
  return panel?.useBlockImageSettings === false && panel?.[key] !== "" && panel?.[key] != null
    ? panel[key]
    : props?.[key] ?? fallback;
}

function renderStaticAccordionBlock(block, props = {}, project) {
  const panels = asArray(props.panels?.length ? props.panels : props.items).filter(Boolean);
  if (!panels.length) return "";
  const style = getStyleTokens(project);
  const isFeature = String(block?.type || "") === "feature-accordion";
  const scope = `wb-static-accordion-${String(block?.id || block?.uid || block?.type || "block").replace(/[^a-zA-Z0-9_-]/g, "")}`;
  const sectionBg = props.backgroundColor || panels[0]?.backgroundColor || style.heroBg || "#07111f";
  const sectionMaxWidth = Math.max(320, Number(props.contentMaxWidth || props.baseLayoutWidth || 1440) || 1440);
  const cardRadius = Math.max(0, Number(props.cardRadius ?? (isFeature ? 18 : 20)) || 0);
  const closedHeight = Math.max(48, Number(props.closedRowHeight || props.accordionHeaderHeight || 64) || 64);
  const openMinHeight = Math.max(0, Number(props.openCardMinHeight || props.cardMinHeight || props.cardHeight || 560) || 560);
  const contentVertical = props.contentVerticalAlign || props.textVerticalAlign || "top";
  const openAlign = resolveStaticAccordionAlign(contentVertical);
  const activeHeight = Number(props.activeCardHeight || props.cardHeight || 560) || 560;
  const imageHeight = Number(props.imageHeight || props.panelImageFixedHeight || 420) || 420;
  const imageMaxWidth = Number(props.imageMaxWidth || 620) || 620;
  const sectionCss = [
    `.${scope}{width:100%;background:${sectionBg};padding:${Number(props.paddingTop ?? 60)}px ${Number(props.paddingRight ?? 48)}px ${Number(props.paddingBottom ?? 60)}px ${Number(props.paddingLeft ?? 48)}px;box-sizing:border-box;}`,
    `.${scope}__inner{width:100%;max-width:${sectionMaxWidth}px;margin:0 auto;}`,
    `.${scope}__row{width:100%;overflow:hidden;border:1px solid rgba(255,255,255,0.10);margin:0;background:#07111f;color:#fff;}`,
    `.${scope}__row:first-of-type{border-top-left-radius:18px;border-top-right-radius:18px;}`,
    `.${scope}__row:last-of-type{border-bottom-left-radius:18px;border-bottom-right-radius:18px;}`,
    `.${scope}__row[open]{border-radius:${cardRadius}px;box-shadow:0 18px 46px rgba(0,0,0,0.22);}`,
    `.${scope}__header{min-height:${closedHeight}px;padding:${props.headerPadding || "20px 32px"};display:flex;align-items:center;gap:12px;cursor:pointer;font-weight:800;list-style:none;}`,
    `.${scope}__header::-webkit-details-marker{display:none;}`,
    `.${scope}__body{min-height:${openMinHeight}px;height:min(${activeHeight}px,70vh);display:grid;grid-template-columns:minmax(0,45%) minmax(0,55%);grid-template-areas:"text image";align-items:${openAlign.grid};align-content:${openAlign.grid};gap:${Number(props.cardGap || 40)}px;padding:${props.contentPadding || `${Number(props.cardPadding ?? 48)}px`};box-sizing:border-box;overflow:visible;}`,
    `.${scope}__text{grid-area:text;display:flex;flex-direction:column;justify-content:${openAlign.flex};align-self:${openAlign.grid};gap:20px;min-width:0;max-width:100%;}`,
    `.${scope}__imageCol{grid-area:image;display:flex;align-items:${openAlign.flex};justify-content:center;align-self:${openAlign.grid};height:${imageHeight}px;min-width:0;max-width:100%;overflow:visible;}`,
    `.${scope}__imageWrap{width:100%;height:100%;max-width:${imageMaxWidth}px;min-width:0;overflow:hidden;display:flex;justify-content:center;margin:0 auto;}`,
    `.${scope}__image{display:block;width:100%;max-width:100%;height:100%;min-width:0;}`,
    `@media(max-width:1024px){.${scope}{padding:48px 24px;}.${scope}__body{height:auto;min-height:700px;grid-template-columns:minmax(0,1fr);grid-template-areas:"text" "image";padding:32px;}.${scope}__imageCol{height:auto;}.${scope}__image{height:auto;}}`,
    `@media(max-width:640px){.${scope}{padding:32px 0;}.${scope}__header{min-height:56px;padding:16px;}.${scope}__body{height:auto;min-height:0;grid-template-columns:minmax(0,1fr);grid-template-areas:"text" "image";gap:24px;padding:24px 16px;}.${scope}__imageCol{height:auto;}.${scope}__image{height:auto;}}`,
  ].join("");

  const rows = panels.map((panel, idx) => {
    const panelBg = panel.backgroundColor || sectionBg;
    const textColor = panel.textColor || props.textColor || "#ffffff";
    const accent = panel.accentColor || props.accentColor || "#0ea5e9";
    const imageSrc = resolveAccordionPanelImageUrl(panel);
    const imageRight = panel.imagePosition !== "left";
    const fit = normalizeStaticAccordionImageFit(staticAccordionImageSetting(props, panel, "imageFit", "contain"));
    const objectPosition = resolveStaticAccordionObjectPosition({
      imageObjectPosition: staticAccordionImageSetting(props, panel, "imageObjectPosition", "center center"),
      imagePositionX: staticAccordionImageSetting(props, panel, "imagePositionX", ""),
      imagePositionY: staticAccordionImageSetting(props, panel, "imagePositionY", ""),
    });
    const flexAlign = staticObjectPositionFlexAlign(objectPosition);
    const imgHeight = fit === "contain" || fit === "scale-down" || fit === "none" ? "auto" : "100%";
    const imgMaxHeight = fit === "contain" || fit === "scale-down" || fit === "none" ? `${imageHeight}px` : "100%";
    const gridAreas = imageRight ? "\"text image\"" : "\"image text\"";
    const header = escapeHtml(htmlToPlainText(panel.eyebrow || panel.heading || `Panel ${idx + 1}`));
    return `<details class="${scope}__row" data-wb-static-accordion-panel="true" style="background:${panelBg};color:${textColor};" ${idx === 0 ? "open" : ""}>
      <summary class="${scope}__header"><span style="width:9px;height:9px;border-radius:50%;background:${accent};flex-shrink:0;"></span><span>${header}</span></summary>
      <div class="${scope}__body" style="grid-template-areas:${gridAreas};">
        <div class="${scope}__text">
          ${panel.eyebrow ? `<div style="color:${accent};font-size:13px;font-weight:900;letter-spacing:.12em;text-transform:uppercase;">${inlineRichTextToHtml(panel.eyebrow)}</div>` : ""}
          <h2 style="margin:0;max-width:${Number(panel.headingMaxWidth || props.headingMaxWidth || 560)}px;font-size:${Number(panel.headingSize || props.headingFontSize || 46)}px;line-height:1.08;color:${textColor};">${inlineRichTextToHtml(panel.heading || "Your headline")}</h2>
          ${panel.body ? `<div style="max-width:${Number(panel.bodyMaxWidth || props.bodyMaxWidth || 620)}px;font-size:${Number(panel.bodySize || 17)}px;line-height:1.75;color:${textColor};opacity:.78;">${richTextToHtml(panel.body)}</div>` : ""}
          ${panel.ctaText ? `<a href="${escapeHtml(panel.ctaUrl || "#")}" style="display:inline-flex;align-items:center;justify-content:center;align-self:flex-start;background:${accent};color:#fff;text-decoration:none;border-radius:999px;padding:13px 28px;font-weight:800;">${inlineRichTextToHtml(panel.ctaText)}</a>` : ""}
        </div>
        <div class="${scope}__imageCol" data-wb-static-accordion-image-column="true">
          <div class="${scope}__imageWrap" data-wb-static-accordion-image-wrapper="true" style="align-items:${flexAlign};">
            ${imageSrc ? `<img class="${scope}__image" data-wb-controlled-image="${escapeHtml(String(block?.type || "accordion"))}" src="${escapeHtml(imageSrc)}" alt="${escapeHtml(panel.imageAlt || htmlToPlainText(panel.heading || header))}" style="object-fit:${fit};object-position:${objectPosition};height:${imgHeight};max-height:${imgMaxHeight};" />` : ""}
          </div>
        </div>
      </div>
    </details>`;
  ...
  return `<section class="${scope}" data-wb-static-accordion-block="${escapeHtml(String(block?.type || "accordion"))}"><style>${sectionCss}</style><div class="${scope}__inner">${rows}</div><script>(function(){var root=document.currentScript&&document.currentScript.previousElementSibling; if(!root)return; root.querySelectorAll('details').forEach(function(row){row.addEventListener('toggle',function(){if(!row.open)return; root.querySelectorAll('details[open]').forEach(function(other){if(other!==row)other.open=false;});});});})();</script></section>`;
}

function buildHtmlFromBlock(block, project) {
  block = resolveSharedBlockInstance(block, project);
  ...
  switch (block?.type) {
    ...
    case "feature-accordion":
      return renderStaticAccordionBlock(block, props, project);
    ...
    case BlockTypes.HERO: {
      ...
      const backgroundImage = resolveBlockImageUrl({ type: BlockTypes.HERO, props });
      ...
      const overlayColor = colorWithAlpha(props.overlayColor || props.backgroundOverlayColor || "#020617", overlayOpacity);
      const overlayLayer = !overlayDisabled && props.overlayGradientEnabled
        ? `linear-gradient(${escapeHtml(props.overlayGradientDirection || "135deg")}, ${escapeHtml(props.overlayGradientStart || overlayColor)}, ${escapeHtml(props.overlayGradientEnd || colorWithAlpha(props.overlayColor || props.backgroundOverlayColor || "#020617", 0))})`
        : overlayOpacity > 0
          ? `linear-gradient(${overlayColor}, ${overlayColor})`
          : "";
      const backgroundSize = props.imageFit || props.backgroundSize || "cover";
      const backgroundPosition = props.imagePosition || props.backgroundPosition || "center center";
      const backgroundCss = backgroundImage
        ? `${overlayLayer ? `${overlayLayer}, ` : ""}url('${escapeHtml(backgroundImage)}') ${escapeHtml(backgroundPosition)}/${escapeHtml(backgroundSize)} no-repeat`
        : (props.backgroundColor || style.heroBg || "#1d4ed8");
      const primaryCta = props.cta && typeof props.cta === "object" ? props.cta : {};
      const secondaryCta = props.secondaryCta && typeof props.secondaryCta === "object" ? props.secondaryCta : {};
      const primaryText = primaryCta.text || props.ctaText || "";
      const secondaryText = secondaryCta.text || props.secondaryCtaText || "";
      const primaryHref = primaryCta.linkType === "none" ? "" : (primaryCta.href || props.ctaLink || "#");
      const secondaryHref = secondaryCta.linkType === "none" ? "" : (secondaryCta.href || props.secondaryCtaLink || "#");
      const primaryTarget = resolveCtaOpenInNewTab({
        ...(props.ctaNewTab !== undefined ? { targetBlank: props.ctaNewTab } : {}),
        ...primaryCta,
      }) ? ` target="_blank" rel="noopener noreferrer"` : "";
      const secondaryTarget = resolveCtaOpenInNewTab({
        ...(props.secondaryCtaNewTab !== undefined ? { targetBlank: props.secondaryCtaNewTab } : {}),
        ...secondaryCta,
      }) ? ` target="_blank" rel="noopener noreferrer"` : "";
      return `<section style="position:relative;overflow:hidden;padding:88px 20px;min-height:${escapeHtml(props.minHeight || "480px")};background:${backgroundCss};color:${escapeHtml(headlineBlock.color || props.headlineColor || "#ffffff")};text-align:${escapeHtml(sectionAlign)};${spacingCss(props)}">
        <div style="position:relative;z-index:1;max-width:${Math.max(contentWidth + (props.floatingImage ? 480 : 0), contentWidth)}px;margin:0 auto;display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:42px;align-items:center;overflow:hidden;">
          <div style="max-width:${contentWidth}px;">
            ...
            ${(primaryText || secondaryText) ? `<div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:24px;">${primaryText ? `<a href="${escapeHtml(primaryHref || "#")}"${primaryTarget} style="display:inline-flex;align-items:center;justify-content:center;min-height:48px;background:${props.buttonColor || "#ffffff"};color:${props.buttonTextColor || style.heroBg || "#1d4ed8"};padding:12px 18px;border-radius:999px;text-decoration:none;font-weight:800;">${inlineRichTextToHtml(primaryText)}</a>` : ""}${secondaryText ? `<a href="${escapeHtml(secondaryHref || "#")}"${secondaryTarget} style="display:inline-flex;align-items:center;justify-content:center;min-height:48px;background:rgba(255,255,255,0.08);color:${escapeHtml(headlineBlock.color || props.headlineColor || "#ffffff")};border:1px solid rgba(255,255,255,0.22);padding:12px 18px;border-radius:999px;text-decoration:none;font-weight:800;">${inlineRichTextToHtml(secondaryText)}</a>` : ""}</div>` : ""}
          </div>
    ...
    case BlockTypes.PRICING_TABLE: {
      ...
      const plans = asArray(props.plans).map((plan) => {
        ...
        return `<div data-pricing-card="true" style="min-width:0;overflow:hidden;flex:1 1 260px;border:${resolvedTheme.border};border-radius:28px;padding:24px;background:${resolvedTheme.background};box-shadow:${resolvedTheme.shadow};transform:${plan?.highlighted ? "scale(1.04)" : "none"};position:relative;display:grid;gap:16px;box-sizing:border-box;">
          <div style="min-width:0;padding-right:84px;">
            <h3 style="margin:0;font-size:24px;line-height:1.2;color:${resolvedTheme.text};overflow-wrap:anywhere;">${richTextToHtml(plan?.name || "Plan")}</h3>
            <div style="margin:4px 0 0;font-size:38px;font-weight:900;letter-spacing:-0.03em;color:${resolvedTheme.text};overflow-wrap:anywhere;">${richTextToHtml(plan?.price || "$0")}</div>
            <div style="margin:4px 0 0;color:${resolvedTheme.subtle};line-height:1.6;overflow-wrap:anywhere;">${richTextToHtml(plan?.description || "")}</div>
          </div>
          ...
          <div style="display:grid;gap:10px;">
            ${includedFeatures.map((item) => `<div data-pricing-feature-row="true" style="min-width:0;overflow:hidden;display:grid;grid-template-columns:34px minmax(0,1fr);gap:10px;align-items:start;padding:${plan?.highlighted ? "10px 12px" : "9px 10px"};border-radius:16px;background:${resolvedTheme.featureBg};border:${resolvedTheme.featureBorder};"><span style="display:inline-grid;place-items:center;width:30px;height:30px;border-radius:999px;background:${resolvedTheme.iconBg};color:${resolvedTheme.iconColor};font-weight:900;">${icon}</span><div style="min-width:0;line-height:1.55;color:${resolvedTheme.text};white-space:normal;overflow-wrap:anywhere;word-break:normal;">${richTextToHtml(item)}</div></div>`).join("")}
          </div>
          <div style="display:grid;gap:10px;padding:${plan?.highlighted ? "16px 18px" : "14px 16px"};border-radius:20px;background:${resolvedTheme.extrasBg};border:${resolvedTheme.extrasBorder};">
            ...
            <div style="display:grid;gap:8px;min-width:0;">${extras.length ? extras.map((item) => `<div data-pricing-feature-row="true" style="min-width:0;color:${resolvedTheme.text};line-height:1.55;white-space:normal;overflow-wrap:anywhere;">${richTextToHtml(item)}</div>`).join("") : `<div style="color:${resolvedTheme.subtle};font-style:italic;">No extras listed yet.</div>`}</div>
          </div>
...
function prepareProjectContent(project) {
  project = normalizeSharedBlockTemplateProject(normalizeSharedPrimaryNavigation(project));
  ...
  const pageBlocks = Object.fromEntries(
    Object.entries(pageBlocksSource).map(([pageName, blocks]) => {
      ...
      return [pageName, applyDeletedBlockTombstones(normalizedBlocks, deletedBlockIds, pageName)];
    })
  );
  ...
  const globalFooter = footerBlockToGlobalFooter(globalFooterBlock, buildFooterNavigationContext({ pages: baseProject.pages || [] })) || baseProject.globalFooter || null;
  ...
  return { pagesContent, pageBlocks, ...globalBlockPatch, globalFooter };
}
...
export function saveWebsiteProjects(projects) {
  ...
  const repairedProjects = Array.isArray(projects) ? projects.map((project) => repairProjectTestimonialAvatarUrls(project)) : projects;
  const primary = normalizeProjectsForStorage(repairedProjects);
  ...
  try {
    ...
    try {
      const compact = normalizeProjectsForStorage(repairedProjects, { compact: true });
      writeWebsiteProjectsToStorage(compact);
      return true;
    } catch {
      try {
        const emergency = normalizeProjectsForStorage(repairedProjects, { compact: true }).slice(0, 3);
...
export function createWebsiteProject(input) {
  ...
  const baseProject = normalizePageLayoutProject({
    ...
    brief: input?.brief || {},
    ...(hasGlobalPageWidthMode(input) ? {
      pageWidthMode: resolveGlobalPageWidthMode(input),
      globalPageWidthMode: resolveGlobalPageWidthMode(input),
    } : {}),
    ...
    sharedBlockTemplates: input?.sharedBlockTemplates || input?.sharedTemplates || {},
    globalNavBlock: input?.globalNavBlock || null,
    globalFooterBlock: input?.globalFooterBlock || globalFooterToFooterBlock(input?.globalFooter, null) || null,
    globalFooter: input?.globalFooter || footerBlockToGlobalFooter(input?.globalFooterBlock, buildFooterNavigationContext({ pages: input?.pages || [] })) || null,
  });

  const prepared = prepareProjectContent(baseProject);
  const project = normalizePageLayoutProject({
    ...baseProject,
    pagesContent: prepared.pagesContent,
    pageBlocks: prepared.pageBlocks,
    ...("globalNavBlock" in prepared ? { globalNavBlock: prepared.globalNavBlock } : {}),
    ...("globalFooterBlock" in prepared ? { globalFooterBlock: prepared.globalFooterBlock } : {}),
    globalFooter: prepared.globalFooter || baseProject.globalFooter || null,
  });
...
export function getWebsiteProject(id) {
  ...
  const project = repairProjectTestimonialAvatarUrls(projects[idx]);
  const prepared = prepareProjectContent(project);
  const { pages: pagesWithSlugs, changed: pageSlugsBackfilled } = ensurePageSlugs(project.pages);
  const migrated = repairProjectTestimonialAvatarUrls({
    ...project,
    pagesContent: prepared.pagesContent,
    pageBlocks: prepared.pageBlocks,
    pages: pagesWithSlugs,
    ...("globalNavBlock" in prepared ? { globalNavBlock: prepared.globalNavBlock } : {}),
    ...("globalFooterBlock" in prepared ? { globalFooterBlock: prepared.globalFooterBlock } : {}),
  });
...
export function updateWebsiteProject(id, patch) {
  ...
  const normalizedNext = repairProjectTestimonialAvatarUrls({
    ...next,
    pagesContent: prepared.pagesContent,
    pageBlocks: prepared.pageBlocks,
    ...("globalNavBlock" in prepared ? { globalNavBlock: prepared.globalNavBlock } : {}),
