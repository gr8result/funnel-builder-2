...
export function ensureWebsiteProjectPageBlocks(project) {
  ...
  pages.forEach((page, index) => {
    ...
    nextPageBlocks[page.name] = templateBlocks.length
...
export function getWebsiteTemplatePreview(templateSlug, pageKey = "") {
  ...
  const blocks = asArray(activeTemplatePage?.sections).flatMap((section) =>
...
function prepareProjectContent(project) {
  ...
  const globalNavBlock = globalBlockPatch.globalNavBlock || project?.globalNavBlock || null;
  const globalFooterBlock = globalBlockPatch.globalFooterBlock || project?.globalFooterBlock || null;
