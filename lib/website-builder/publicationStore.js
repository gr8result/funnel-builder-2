...
function domainWithoutWww(value = "") {
  return normalizeDomain(value).replace(/^www\./, "");
}

function collectPublicationDomains(record = {}) {
  ...
  return [
    ...
    siteData.primaryDomain,
    siteData.primary_domain,
    publication.customDomain,
    publication.custom_domain,
    publication.primaryDomain,
    publication.primary_domain,
    publication.primaryWebsiteUrl,
    publication.liveUrl,
  ]
    .map(normalizeDomain)
    .filter(Boolean);
...
function domainMatchesHost(record = {}, host = "") {
  const normalizedHost = normalizeDomain(host);
  const baseHost = domainWithoutWww(normalizedHost);
  if (!normalizedHost || !baseHost) return false;
  return collectPublicationDomains(record).some((domain) => {
    const baseDomain = domainWithoutWww(domain);
    return domain === normalizedHost
      || baseDomain === baseHost
      || baseDomain.startsWith(baseHost);
  });
}
...
export async function getPublishedWebsiteByDomain(hostname) {
  ...
  if (!host) return null;
  const baseHost = domainWithoutWww(host);
  const wwwHost = baseHost ? `www.${baseHost}` : "";

  const { data, error } = await supabaseAdmin
    .from("published_websites")
    .select("id, user_id, project_id, name, slug, primary_domain, custom_domain, domain_status, site_data, published, published_at, updated_at")
    .or(`custom_domain.eq.${host},primary_domain.eq.${host},custom_domain.eq.${baseHost},primary_domain.eq.${baseHost},custom_domain.eq.${wwwHost},primary_domain.eq.${wwwHost}`)
    .eq("published", true)
    .order("published_at", { ascending: false })
    .order("updated_at", { ascending: false })
    .limit(10);
  ...
  if (!selected) return getPublishedWebsiteByDomainFallback(host);
  return normalizePublication(selected);
}

async function getPublishedWebsiteByDomainFallback(hostname) {
  const host = normalizeDomain(hostname);
  if (!host) return null;
  ...
  const selected = sortPublishedCandidates(data.filter((record) => domainMatchesHost(record, host)))[0] || null;
  if (!selected) return null;
  logPublicationResolution("domain fallback lookup", {
    host,
    id: selected?.id || null,
    slug: selected?.slug || null,
    customDomain: selected?.custom_domain || null,
