...
export async function getPublishedWebsiteByDomain(hostname) {
  ...
  const exactCustomDomain = rows.find((record) => normalizeDomain(record?.custom_domain) === host);
  const exactPrimaryDomain = rows.find((record) => normalizeDomain(record?.primary_domain) === host);
  const selected = exactCustomDomain || exactPrimaryDomain || rows[0] || null;
