...
function domainLookupAliases(hostname = "") {
  const host = normalizeDomain(hostname);
  if (!host) return [];
  const aliases = new Set([host]);
  if (host.startsWith("www.")) {
    aliases.add(host.slice(4));
  } else {
...
export async function getPublishedWebsiteByDomain(hostname) {
  const host = normalizeDomain(hostname);
  if (!host) return null;
  const aliases = domainLookupAliases(host);
  const domainFilter = aliases
    .flatMap((alias) => [`custom_domain.eq.${alias}`, `primary_domain.eq.${alias}`])
    .join(",");
  ...
  const aliasRows = rows.filter((record) => aliasSet.has(normalizeDomain(record?.custom_domain)) || aliasSet.has(normalizeDomain(record?.primary_domain)));
  const selected = sortPublishedCandidates(aliasRows)[0] || sortPublishedCandidates(rows)[0] || null;
