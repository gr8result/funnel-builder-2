...
function buildProjectFromRows(siteRow, pageRows = []) {
  ...
  return normalizeSharedBlockTemplateProject(normalizeSharedPrimaryNavigation(project));
}
  ...
  try {
    ...
    if (!options?.siteOnly) {
      ...
      for (const page of pagesToSave) {
        ...
        incomingProject = normalizeSharedBlockTemplateProject(normalizeSharedPrimaryNavigation(incomingProject || {}));
...
export async function assembleWebsiteForRendering(accountId, siteId, overrides = {}) {
  ...
  return normalizeSharedBlockTemplateProject(normalizeSharedPrimaryNavigation(withProjectPublicationIdentity({
    ...project,
...
export async function loadSplitWebsiteProjectSite(accountId, siteId) {
  const project = await loadFullSplitWebsiteProject(accountId, siteId);
  if (!project) return null;
  return normalizeSharedBlockTemplateProject(normalizeSharedPrimaryNavigation({ ...project, pageBlocks: {}, pagesContent: {}, chaiData: {} }));
}
