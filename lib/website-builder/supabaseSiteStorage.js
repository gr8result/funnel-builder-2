...
function buildPageDocument(project, page, existingPage = null) {
  ...
  const incomingBlocks = Array.isArray(project?.pageBlocks?.[name])
    ...
    : Array.isArray(project?.chaiData?.[name]?.blocks)
      ...
      : null;
  const blocks = normalizeVideoHeroBlocksForPersistence(incomingBlocks || (Array.isArray(existingPage?.blocks) ? existingPage.blocks : []));
  ...
  const chaiData = syncVideoHeroChaiDataBlocks(rawChaiData, blocks);
