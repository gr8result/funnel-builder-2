...
import {
  ...
  repairProjectTestimonialAvatarUrls,
...
import { assertWebsiteUnlockedForMutation, isProtectedWebsiteProject } from "./contentLock";
...
function buildProjectFromRows(siteRow, pageRows = []) {
  ...
  return repairProjectTestimonialAvatarUrls(normalizePageLayoutProject(normalizeSharedBlockTemplateProject(normalizeSharedPrimaryNavigation(project))));
}
...
function buildPageDocument(project, page, existingPage = null) {
  ...
  const normalizedBlocks = normalizeVideoHeroBlocksForPersistence(incomingBlocks !== null ? incomingBlocks : (Array.isArray(existingPage?.blocks) ? existingPage.blocks : []));
  const blocks = preserveExistingTestimonialAvatarUrls(normalizedBlocks, existingPage?.blocks || []);
  ...
  const preservedChaiData = preserveExistingTestimonialChaiAvatarUrls(rawChaiData, blocks, existingPage?.chai_data || null);
  const chaiData = {
    ...(preservedChaiData && typeof preservedChaiData === "object" && !Array.isArray(preservedChaiData) ? preservedChaiData : {}),
    blocks,
  };
...
function valuesMatchForStorage(left, right) {
  return stableJson(jsonRoundTrip(left)) === stableJson(jsonRoundTrip(right));
}

function diffValuesForStorage(left, right, basePath = "") {
  const diffs = [];
  ...
  visit(jsonRoundTrip(left), jsonRoundTrip(right), basePath);
  return diffs;
}
...
export async function saveSplitWebsiteProject(accountId, project, options = {}) {
  ...
  const lock = assertWebsiteUnlockedForMutation({
    ...
    action: options?.backupSource || options?.source || "save",
  });
  if (!lock.ok) {
    const error = new Error(lock.error);
    error.code = lock.code;
    error.status = lock.status;
    throw error;
  }
  ...
  try {
    ...
    if (!options?.siteOnly) {
      ...
      for (const page of pagesToSave) {
        ...
        incomingProject = normalizePageLayoutProject(normalizeSharedBlockTemplateProject(normalizeSharedPrimaryNavigation(incomingProject || {})));
        ...
        const pageResult = await supabaseAdmin.from(PAGES_TABLE).upsert({
          ...
          html: pageDoc.html || "",
          updated_at: now,
        }, { onConflict: "user_id,site_id,page_id" }).select("page_id, page_data, blocks, chai_data, html").maybeSingle();
        if (pageResult.error) throw pageResult.error;
        if (!Array.isArray(pageResult.data?.blocks)) throw new Error(`Website save verification failed for "${page.name}"`);
        const expectedStoredPage = {
          pageData: pageDoc,
          blocks: pageDoc.blocks,
          chaiData: pageDoc.chaiData,
          html: pageDoc.html || "",
        };
        const actualStoredPage = {
          pageData: pageResult.data?.page_data || null,
          blocks: pageResult.data?.blocks,
          chaiData: pageResult.data?.chai_data ?? null,
          html: pageResult.data?.html || "",
        };
        if (!preservingExistingBlocks && !valuesMatchForStorage(expectedStoredPage, actualStoredPage)) {
          const diffs = diffValuesForStorage(expectedStoredPage, actualStoredPage, "website_builder_pages");
          console.error("[website-builder storage] page row verification mismatch", {
            siteId,
            pageName: page.name,
            source: backupSource,
          ...
          throw new Error(`Website save verification failed for "${page.name}": stored page row did not match submitted page document.`);
        }
...
export async function loadFullSplitWebsiteProject(accountId, siteId) {
  try {
    ...
    if (isProtectedWebsiteProject(safeSiteId)) return { ...localProject, id: safeSiteId };
...
export async function assembleWebsiteForRendering(accountId, siteId, overrides = {}) {
  ...
  return normalizePageLayoutProject(normalizeSharedBlockTemplateProject(normalizeSharedPrimaryNavigation(withProjectPublicationIdentity({
    ...project,
...
export async function loadSplitWebsiteProjectSite(accountId, siteId) {
  const project = await loadFullSplitWebsiteProject(accountId, siteId);
  if (!project) return null;
  return normalizePageLayoutProject(normalizeSharedBlockTemplateProject(normalizeSharedPrimaryNavigation({ ...project, pageBlocks: {}, pagesContent: {}, chaiData: {} })));
}
...
export async function deleteSplitWebsiteProject(accountId, siteId, options = {}) {
  const lock = assertWebsiteUnlockedForMutation({
    projectId: siteId,
    userId: accountId,
    unlockToken: options?.unlockToken || "",
