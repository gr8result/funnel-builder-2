...
function isAppOwnedPublicAssetPath(pathname = "") {
  return /^\/(?:email-assets|imported)\//i.test(String(pathname || ""));
}
...
function normalizePublishedAssetUrlFields(value, key = "") {
  ...
  if (!value || typeof value !== "object") {
    return typeof value === "string" && PUBLISHED_ASSET_FIELD_NAMES.has(key)
      ? normalizeAppOwnedPublicAssetUrl(value)
      : value;
  }
  const next = {};
