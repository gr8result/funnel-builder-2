...
export function normalizePublishedWebsiteBlocks(pageBlocks = {}) {
  const normalizeBlock = (block) => normalizePublishedBlockAssets(block);
  if (Array.isArray(pageBlocks)) return pageBlocks.map(normalizeBlock);
  if (!pageBlocks || typeof pageBlocks !== "object") return {};
...
export function createPublicationPayload(project = {}) {
  ...
  const pageBlocks = normalizePublishedWebsiteBlocks(normalizeVideoHeroBlocks(project?.pageBlocks && typeof project.pageBlocks === "object" ? project.pageBlocks : {}));
  ...
  const normalizedGlobalFooterBlock = globalFooterBlock?.type === "video-hero" ? normalizeVideoHeroBlocks([globalFooterBlock])[0] : normalizePublishedBlockAssets(globalFooterBlock);
  const siteData = {
