import { isUnsafeAccordionPanelImageUrl, resolveAccordionPanelImageUrl } from "./accordionPanels";
import { buildFooterNavigationContext, normalizeFooterNavigationBlock } from "./footerNavigation";
...
function normalizePublishedGridItemIcons(item = {}) {
  ...
  const inferredIconName = normalizeSocialPlatformName([item.title, item.label, item.text, item.eyebrow, item.platform, item.name].filter(Boolean).join(" "))
    ...
    || iconNameFromSocialIconUrl(item.iconImage)
    || iconNameFromSocialIconUrl(item.iconUrl);
  const nextIconName = iconName || inferredIconName;
  const shouldClearIconImage = inferredIconName || isUnsafePublishedIconUrl(item.iconImage);
  return {
    ...item,
    ...(nextIconName ? { iconName: nextIconName } : {}),
    ...(shouldClearIconImage ? { iconImage: "", iconAssetId: "" } : {}),
  };
}

function normalizePublishedBlockAssets(block) {
...
export function normalizeVideoHeroBlocks(pageBlocks = {}) {
  const normalizeBlock = (block) => {
    ...
    block = normalizeVideoHeroForPersistence(block);
    const props = block.props || {};
...
export function normalizePublishedWebsiteBlocks(pageBlocks = {}, footerContext = {}) {
  const normalizeBlock = (block) => normalizePublishedBlockAssets(normalizeFooterNavigationBlock(block, footerContext));
  if (Array.isArray(pageBlocks)) return pageBlocks.map(normalizeBlock);
  if (!pageBlocks || typeof pageBlocks !== "object") return {};
...
export function createPublicationPayload(project = {}) {
  ...
  const pageBlocks = normalizePublishedWebsiteBlocks(normalizeVideoHeroBlocks(project?.pageBlocks && typeof project.pageBlocks === "object" ? project.pageBlocks : {}), footerContext);
  const globalNavBlock = project?.globalNavBlock || null;
  const globalFooterBlock = project?.globalFooterBlock || null;
  const normalizedGlobalNavBlock = globalNavBlock?.type === "video-hero" ? normalizeVideoHeroBlocks([globalNavBlock])[0] : normalizePublishedBlockAssets(globalNavBlock);
  const normalizedGlobalFooterBlock = globalFooterBlock?.type === "video-hero"
    ? normalizeVideoHeroBlocks([globalFooterBlock])[0]
    : normalizePublishedBlockAssets(normalizeFooterNavigationBlock(globalFooterBlock, footerContext));
  const siteData = {
