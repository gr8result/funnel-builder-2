...
import { DEFAULT_FOOTER_COMPANY_LINKS, GR8_RESULT_FOOTER_NAVIGATION_LINKS, applyGr8AustralianFooterPanel, buildFooterNavigationContext, footerBlockToGlobalFooter, globalFooterToFooterBlock, normalizeFooterNavigationBlock } from "./footerNavigation";
import { getAssetFromLibrary, normalizeWebsiteBuilderAssets } from "./mediaAssets";
import { normalizePageLayoutProject, withPageLayoutDefaults } from "./pageLayout";
import { normalizeSharedPrimaryNavigation } from "./sharedNavigation";
import { normalizeSharedBlockTemplateProject } from "./sharedBlockTemplates";
...
function appendWebsitePagePath(url, page = "") {
  const base = safeTrim(url).replace(/\/$/, "");
  if (!base) return "";
  const pageSlug = slugifyWebsiteValue(page);
  if (!pageSlug || pageSlug === "home" || pageSlug === "index") return base;
  return `${base}/${pageSlug}`;
}

function isDisconnectedDomainStatus(value = "") {
  return /^(disabled|disconnected|failed|rejected|removed)$/i.test(safeTrim(value));
}

export function resolveWebsiteUrls(website = {}, options = {}) {
  const slug = resolveProjectSlug(website, options?.slug || website?.name || website?.id || "site");
  const publication = website?.publication && typeof website.publication === "object" ? website.publication : {};
  const customDomain = resolveProjectCustomDomain(website);
  const domainStatus = safeTrim(
    website?.domainStatus
    || website?.domain_status
    || publication?.domainStatus
    || publication?.domain_status
  );
  const hasActiveCustomDomain = !!customDomain && isLikelyCustomDomain(customDomain) && !isDisconnectedDomainStatus(domainStatus);
  const customDomainUrl = hasActiveCustomDomain ? buildWebsiteDomainUrl({ domain: customDomain }) : "";
  const internalPreviewUrl = buildHostedWebsiteUrl({ slug });
  const internalFallbackUrl = internalPreviewUrl;
  const pageName = options?.page || options?.pageName || options?.pageSlug || "";

  return {
    primaryPublicUrl: appendWebsitePagePath(customDomainUrl || internalPreviewUrl, pageName),
    customDomainUrl: appendWebsitePagePath(customDomainUrl, pageName),
    internalPreviewUrl: appendWebsitePagePath(internalPreviewUrl, pageName),
    internalFallbackUrl: appendWebsitePagePath(internalFallbackUrl, pageName),
  };
}

function normalizeWebsiteId(value = "") {
  return safeTrim(value).replace(/^draft:/, "");
}

export function resolveWebsitePublicationStatus(website = {}, options = {}) {
  if (options?.isPublishing) {
    return { state: "publishing", label: "Publishing", validPublished: false };
  }

  const publishError = options?.publishError || website?.publishError || website?.publication?.publishError;
  if (publishError) {
    return { state: "publish_failed", label: "Publish failed", validPublished: false, error: safeTrim(publishError) };
  }

  const publication = website?.publication && typeof website.publication === "object" ? website.publication : {};
  const publishedRevision = safeTrim(
    website?.publishedVersion
    || website?.published_revision
    || website?.publishedRevision
    || publication?.publishedVersion
    || publication?.published_revision
    || publication?.publishedRevision
  );
  const publishedAt = safeTrim(
    website?.publishedAt
    || website?.published_at
    || publication?.publishedAt
    || publication?.published_at
  );
  const draftRevision = safeTrim(
    website?.projectVersion
    || website?.project_version
    || website?.savedVersion
    || publication?.projectVersion
    || publication?.project_version
  );
  const expectedWebsiteId = normalizeWebsiteId(options?.expectedWebsiteId || website?.id || website?.project_id);
  const actualWebsiteId = normalizeWebsiteId(
    website?.id
    || website?.project_id
  ...
  const readbackVerified = options?.readbackVerified !== false;
  const routeVerified = options?.routeVerified !== false;
  const websiteMatches = !!actualWebsiteId && (!expectedWebsiteId || expectedWebsiteId === actualWebsiteId);
  const validPublished = !!publishedRevision
    && !!publishedAt
    && websiteMatches
    && pages.length > 0
    && readbackVerified
  ...
  if (draftRevision && publishedRevision && draftRevision !== publishedRevision) {
    return {
...
function normalizePublishedAccordionPanel(panel = {}) {
  ...
  return {
    ...
    imagePositionX: panel.imagePositionX === "" || panel.imagePositionX == null ? "" : panel.imagePositionX,
    imagePositionY: panel.imagePositionY === "" || panel.imagePositionY == null ? "" : panel.imagePositionY,
    imageScale: panel.imageScale === "" || panel.imageScale == null ? "" : panel.imageScale,
    imageMaxWidth: panel.imageMaxWidth === "" || panel.imageMaxWidth == null ? "" : panel.imageMaxWidth,
    imageMaxHeightMode: safeTrim(panel.imageMaxHeightMode) || "",
    imageMaxHeightCustom: panel.imageMaxHeightCustom === "" || panel.imageMaxHeightCustom == null ? "" : panel.imageMaxHeightCustom,
    imagePadding: panel.imagePadding === "" || panel.imagePadding == null ? "" : panel.imagePadding,
    panelImageHeightMode: safeTrim(panel.panelImageHeightMode) || "",
    panelImageFixedHeight: panel.panelImageFixedHeight === "" || panel.panelImageFixedHeight == null ? "" : panel.panelImageFixedHeight,
    contentVerticalAlign: safeTrim(panel.contentVerticalAlign) || "",
  };
}
...
function normalizePublishedAccordionImageSettings(props = {}) {
  return {
    ...
    imagePositionX: props.imagePositionX === "" || props.imagePositionX == null ? "" : props.imagePositionX,
    imagePositionY: props.imagePositionY === "" || props.imagePositionY == null ? "" : props.imagePositionY,
    imageScale: props.imageScale === "" || props.imageScale == null ? 100 : props.imageScale,
    imageMaxWidth: props.imageMaxWidth === "" || props.imageMaxWidth == null ? 620 : props.imageMaxWidth,
    ...
    panelImageHeightMode: safeTrim(props.panelImageHeightMode) || "fixed",
    panelImageFixedHeight: props.panelImageFixedHeight === "" || props.panelImageFixedHeight == null ? 420 : props.panelImageFixedHeight,
    contentVerticalAlign: safeTrim(props.contentVerticalAlign) || "top",
  };
}
  ...
  const visitBlock = (block, pageName = "", index = -1, location = "page") => {
    ...
    if (String(block.type || "") === "video-hero") {
      ...
      entries.push({
        ...
        videoStoragePath: safeTrim(props.videoStoragePath || props.storagePath || props.storage_path || ""),
        videoFileName: safeTrim(props.videoFileName || props.fileName || props.name || ""),
        videoMimeType: safeTrim(props.videoMimeType || props.mimeType || props.mime_type || props.type || ""),
...
function footerNavigationCount(block) {
  if (!block || block.type !== "footer") return 0;
  const props = block.props || {};
  const links = Array.isArray(props.navigationLinks)
    ? props.navigationLinks
    : Array.isArray(props.navLinks)
      ? props.navLinks
      : Array.isArray(props.footerLinks)
        ? props.footerLinks
...
export function resolveCanonicalGlobalFooterBlock(project = {}, footerContext = {}) {
  const candidates = [
  ...
  Object.values(pageBlocks).forEach((blocks) => {
    (Array.isArray(blocks) ? blocks : []).forEach((block) => {
      if (block?.type === "footer") candidates.push(block);
    });
  });

  const chaiData = project?.chaiData && typeof project.chaiData === "object" ? project.chaiData : {};
  Object.values(chaiData).forEach((pageData) => {
  ...
  candidates.forEach((candidate) => {
    if (candidate?.type !== "footer") return;
    const normalized = normalizeFooterNavigationBlock(candidate, footerContext);
    const score = footerNavigationCount(normalized);
...
export function normalizePublishedGlobalFooterBlock(globalFooterBlock = null, project = {}, footerContext = {}, assets = null) {
  const canonicalFooterBlock = resolveCanonicalGlobalFooterBlock(
    { ...project, globalFooterBlock: globalFooterBlock || project?.globalFooterBlock },
    footerContext
  ) || globalFooterBlock;
  let normalizedGlobalFooterBlock = canonicalFooterBlock?.type === "video-hero"
    ? normalizeVideoHeroBlocks([canonicalFooterBlock], assets)[0]
    : normalizePublishedBlockAssets(normalizeFooterNavigationBlock(canonicalFooterBlock, footerContext), assets);
...
export function collectWebsitePublishIntegrity(project = {}) {
  ...
  const globalFooterBlock = resolveCanonicalGlobalFooterBlock(project, footerContext)
    || (project?.globalFooterBlock?.type === "footer" ? project.globalFooterBlock : globalFooterToFooterBlock(project?.globalFooter, null));
  ...
  return {
    pages: pages.map((page) => {
      ...
      return {
        ...
        pageWidthMode: withPageLayoutDefaults(page).pageWidthMode,
...
export function createPublicationPayload(project = {}) {
  project = normalizePageLayoutProject(normalizeSharedBlockTemplateProject(normalizeSharedPrimaryNavigation(project)));
  ...
  const siteData = {
    ...
    pages: Array.isArray(project.pages) ? project.pages.map(withPageLayoutDefaults) : [],
    pageWidthMode: project.pageWidthMode || project.globalPageWidthMode || "contained",
    globalPageWidthMode: project.globalPageWidthMode || project.pageWidthMode || "contained",
    pageBlocks,
    sharedBlockTemplates: project?.sharedBlockTemplates || {},
