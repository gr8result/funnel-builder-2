...
async function requestDraftProjects(auth, { method = "GET", projectId = "", project = null, pageName = "", siteOnly = false, saveSource = "", timeoutMs = 0 } = {}) {
  ...
  try {
    response = await fetch(`/api/website-builder/projects${query}`, {
      ...
      ...(project ? { body: JSON.stringify({
        ...
        baseRevision: project?.__saveBaseRevision ?? project?.baseRevision ?? project?.revision ?? project?.saveRevision ?? "",
        basePageRevision: project?.__saveBasePageRevision ?? project?.basePageRevision ?? (selectedPageName ? project?.pageRevisions?.[selectedPageName] : "") ?? "",
