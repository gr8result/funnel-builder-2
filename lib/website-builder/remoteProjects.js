...
function resolveVideoHeroUrlForDebug(props = {}) {
  const candidates = [
    props?.videoUrl,
    props?.video,
    props?.videoSrc,
...
function summarizeVideoHeroPayload(project = {}, pageName = "") {
  const pages = pageName
    ? { [pageName]: project?.pageBlocks?.[pageName] }
    : (project?.pageBlocks || {});
  return Object.entries(pages || {}).flatMap(([name, blocks]) => (
    ...
    return {
      ...
      index,
      id: block?.id || "",
      videoUrl: resolveVideoHeroUrlForDebug(block?.props || {}),
      fields: {
...
async function requestDraftProjects(auth, { method = "GET", projectId = "", project = null, pageName = "", siteOnly = false, saveSource = "", timeoutMs = 0, unlockToken = "" } = {}) {
  ...
  const scopedProject = project ? buildPageScopedProject(project, selectedPageName, siteOnly) : null;
  if (project && method !== "GET") {
    console.info("[website-builder video-hero] API payload", {
      method,
      projectId,
      pageName: selectedPageName,
      siteOnly,
      saveSource,
      videoHeroes: summarizeVideoHeroPayload(scopedProject, selectedPageName),
    });
  }
  ...
  const timeoutId = controller && resolvedTimeoutMs > 0
    ? setTimeout(() => controller.abort(new Error(`Website builder request timed out after ${resolvedTimeoutMs}ms`)), resolvedTimeoutMs)
    : null;

  let response;
  try {
    response = await fetch(`/api/website-builder/projects${query}`, {
      method,
      headers: {
    ...
    if (error?.name === "AbortError") {
      throw new Error(`Could not sync website drafts (request timeout after ${resolvedTimeoutMs}ms)`);
    }
    throw error;
  } finally {
...
export async function deleteWebsiteProjectFromServer(auth, projectId, options = {}) {
  if (!projectId) return true;
  const payload = await requestDraftProjects(auth, { method: "DELETE", projectId, unlockToken: options?.unlockToken || "" });
  return !!payload?.ok;
}

export async function renameWebsiteProjectOnServer(auth, projectId, name, options = {}) {
  const accessToken = getAccessToken(auth);
  ...
  const response = await fetch("/api/website-builder/projects", {
    method: "PATCH",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}`, ...(options?.unlockToken ? { "x-website-unlock-token": options.unlockToken } : {}) },
    body: JSON.stringify({ projectId, name, unlockToken: options?.unlockToken || "" }),
  });
