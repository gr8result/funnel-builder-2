...
export function blocksToHtml(blocks) {
  ...
  const containerStyle = "width:600px;max-width:600px;margin:0 auto;padding:18px 0;";
    ...
    .map((b) => {
      ...
      if (b.type === "social") {
        const mk = (name, url) => {
          ...
          const badge = SOCIAL_BADGES[key] || { label: String(name || "?").slice(0, 2).toUpperCase(), bg: "#475569", color: "#ffffff", fontSize: 12 };
          return `<a href="${escAttr(url)}" aria-label="${escAttr(name)}" style="display:inline-block;width:28px;height:28px;line-height:28px;margin:0 6px;text-align:center;text-decoration:none;border-radius:999px;background:${badge.bg};color:${badge.color};font-family:Arial,Helvetica,sans-serif;font-size:${badge.fontSize}px;font-weight:700;mso-line-height-rule:exactly;">${badge.label}</a>`;
        };
        return row(
          `<div style="text-align:center;">
