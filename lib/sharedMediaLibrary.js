...
const EMAIL_TEMPLATE_CACHE_TTL_MS = 15 * 60 * 1000;
...
/** Call after any image deletion to prevent the 15-min server cache from resurrecting it. */
export function clearMaterializationCaches() {
  materializedEmailImageCache.clear();
  materializedGenericTemplateCache.clear();
}
...
// Extract a 'bucket:path' key from a Supabase public storage URL so we can
// cross-check it against the live storage listing even when storage_path is null.
// e.g. https://xyz.supabase.co/storage/v1/object/public/assets/user-id/img.png
//   → 'assets:user-id/img.png'
function deriveStoragePathFromPublicUrl(url) {
  if (!url) return null;
...
function listGenericLibraryEntries() {
  ...
  const deduped = Array.from(new Map(
    assets
      .map((asset, index) => ({
        ...
        fallbackUrl: String(asset?.fallbackUrl || '').trim() || undefined,
      }))
  ...
  return deduped.map((asset, index) => ({
    id: `generic:${asset.id}`,
    url: asset.fallbackUrl || asset.src,
    name: asset.name,
...
export function canonicalSourceUrl(imageUrl = '') {
...
async function materializeUserEmailTemplateImage({ admin, userId, entry, deletedPathHashes }) {
  ...
  // Secondary check: if the user explicitly deleted this specific storage path,
  // suppress re-materialization even if the source URL marker is absent.
  if (deletedPathHashes instanceof Set) {
    const storagePathHash = String(hashText(storagePath));
    if (storagePathHash && deletedPathHashes.has(storagePathHash)) {
      materializedEmailImageCache.set(cacheKey, { createdAt: Date.now(), entry: null });
      return null;
    }
  }
...
export async function listMergedSharedMediaLibrary({ admin, userId, bucket = 'assets', limit = 500, includeEmailTemplateImages = true }) {
  ...
  const prefix = `${userId}/`;
  // Fetch user entries and raw generic entries in parallel — a single fetch each.
  // deletedGenericSourceHashes and storedGenericEntries are both derived from the
  // same rawGenericStorageEntries, eliminating a duplicate storage list() call.
  // deletedUserSourceHashes is derived from topLevelStorageEntries (already fetched),
  // eliminating the second duplicate list() call.
  // listWebsiteBuilderTemplateReferencedImages is only needed for template materialization
  // which runs on the slow (includeEmailTemplateImages) path.
  const [topLevelStorageEntries, rawGenericStorageEntries, emailTemplateImages, websiteTemplateImages] = await Promise.all([
    listAllStorageEntries({ admin, bucket, prefix, limit }),
    listAllStorageEntries({ admin, bucket, prefix: 'generic', limit }),
    includeEmailTemplateImages ? listEmailTemplateReferencedImages({ admin, userId }) : Promise.resolve([]),
    includeEmailTemplateImages ? listWebsiteBuilderTemplateReferencedImages() : Promise.resolve([]),
  ]);

  // Derive both storedGenericEntries and deletedGenericSourceHashes from the single fetch.
  const storedGenericEntries = (rawGenericStorageEntries || [])
    .filter((entry) => isImageName(entry?.name))
    .map((entry) => {
      const objectPath = `generic/${entry.name}`;
      const storagePath = `${bucket}:${objectPath}`;
  ...
  const deletedGenericSourceHashes = new Set(
    (rawGenericStorageEntries || [])
      .map((entry) => extractDeletedTemplateSourceHash(entry?.name))
      .filter(Boolean)
  );
  const deletedUserSourceHashes = new Set(
    (topLevelStorageEntries || [])
      .map((entry) => extractDeletedTemplateSourceHash(entry?.name))
      .filter(Boolean)
  );

  // Supabase Storage list() is not recursive — entries whose id===null are virtual folder
  // nodes (e.g. {userId}/website/, {userId}/funnels/). Scan one level deep so images
  // uploaded by the website builder or funnel builder in subdirectories are visible.
  const subfolderDirs = (topLevelStorageEntries || []).filter(
    (item) => item?.id === null && item?.name && !isImageName(item?.name)
  );
  const subfolderImageLists = subfolderDirs.length > 0
    ? await Promise.all(subfolderDirs.map(async (folder) => {
      try {
        const subs = await listAllStorageEntries({ admin, bucket, prefix: `${prefix}${folder.name}/`, limit });
        return (subs || []).filter((item) => isImageName(item?.name)).map((item) => ({
          ...item,
    ...
    : [];
  const storageEntries = [...(topLevelStorageEntries || []), ...subfolderImageLists.flat()];
  ...
  const normalizedEmailTemplateImages = includeEmailTemplateImages
    ? await Promise.all((emailTemplateImages || []).map(async (entry) => {
      ...
      if (sourceHash && genericSourceHashes.has(sourceHash)) return null;
      try {
        return await materializeUserEmailTemplateImage({ admin, userId, entry, deletedPathHashes: deletedUserSourceHashes });
      } catch {
        return isPlatformHostedImageUrl(entry?.url || '') ? entry : null;
      }
    }))
  ...
  // Track which storage-backed paths were actually found in storage, so we can
  // skip DB rows whose storage file has been deleted (avoids broken-URL thumbnails).
  const foundStoragePaths = new Set();
  // Map from storagePath → current public URL so DB rows with stale URLs can be
  // normalised to the live URL (prevents duplicate/broken thumbnails after a
  // Supabase project URL change or other URL drift).
  const storagePathToCurrentUrl = new Map();

  for (const entry of (storageEntries || []).filter((item) => isImageName(item?.name))) {
    const objectPath = `${prefix}${entry.name}`;
    const storagePath = `${bucket}:${objectPath}`;
    foundStoragePaths.add(storagePath);
    const { data: publicData } = admin.storage.from(bucket).getPublicUrl(objectPath);
    const publicUrl = publicData?.publicUrl || '';
    storagePathToCurrentUrl.set(storagePath, publicUrl);
  ...
  for (const row of (rows || []).sort(sortNewestFirst)) {
    if (!row?.url) continue;
    // Skip rows whose storage file was deleted — the storage listing loop above
    // only adds to merged if the file actually exists. If it's gone, the stored
    // URL will 404 and show a broken thumbnail. Skip it silently instead.
    const rowStoragePath = String(row?.storage_path || '');
    if (rowStoragePath.startsWith(`${bucket}:${prefix}`) && !foundStoragePaths.has(rowStoragePath)) continue;
    // Also handle rows that have no storage_path set (older rows): derive the
    // storage path from the public URL itself and apply the same dead-file check.
    if (!rowStoragePath.startsWith(`${bucket}:${prefix}`)) {
      const derivedPath = deriveStoragePathFromPublicUrl(row.url);
      if (derivedPath && derivedPath.startsWith(`${bucket}:${prefix}`) && !foundStoragePaths.has(derivedPath)) continue;
    }
    // Normalise the URL to the current live storage URL to prevent stale DB URLs
    // (e.g. after a Supabase project URL change) causing broken thumbnails or
    // appearing as duplicates alongside the storage-loop entry.
    const liveUrl = storagePathToCurrentUrl.get(rowStoragePath) || row.url;
    const canonical = canonicalUrl(liveUrl);
    ...
    merged.push({
      ...row,
      url: liveUrl,
      name: deriveDisplayName({ description: row.description, storagePath: row.storage_path, url: liveUrl }),
  ...
  for (const genericEntry of listGenericLibraryEntries()) {
    const sourceHash = String(hashText(canonicalSourceUrl(genericEntry.url || '')));
    if (sourceHash && deletedGenericSourceHashes.has(sourceHash)) continue;
