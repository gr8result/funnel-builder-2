...
# ── 1. Add PLATFORM_SVGS const after PLATFORM_META ───────────────────────────
svgs_block = r"""
const PLATFORM_SVGS = {
...
old_meta_end = """function pm(platform) {"""
hit1 = old_meta_end in src
src = src.replace(old_meta_end, svgs_block + "\nfunction pm(platform) {", 1)
print("SVGs insert:", "OK" if hit1 else "MISS")

# ── 2. Replace the chips rendering in the date strip ─────────────────────────
old_chips = """          {!loading && (
              ...
              {Object.keys(PLATFORM_META).map(plat => {
                ...
                return (
                  <button key={plat}
                    ...
                    style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, background: isActive ? meta.color : cnt > 0 ? `${meta.color}33` : 'rgba(255,255,255,0.05)', border: `2px solid ${isActive ? meta.color : cnt > 0 ? `${meta.color}88` : 'rgba(255,255,255,0.1)'}`, borderRadius: 14, padding: '10px 14px', cursor: 'pointer', outline: 'none', transition: 'all 0.15s', minWidth: 56, boxShadow: isActive ? `0 0 18px ${meta.color}88` : 'none', opacity: cnt === 0 ? 0.35 : 1 }}>
                    {SvgIcon ? SvgIcon(isActive ? '#fff' : cnt > 0 ? meta.color : '#6b7280') : <span style={{ fontSize: 28 }}>{meta.icon}</span>}
                    {cnt > 0 && (
                      <span style={{ fontSize: 15, fontWeight: 900, color: isActive ? '#fff' : meta.color, lineHeight: 1 }}>{cnt}</span>
                    )}
                  </button>
                );
              })}
...
hit2 = old_chips in src
src = src.replace(old_chips, new_chips, 1)
print("Chips replace:", "OK" if hit2 else "MISS")
