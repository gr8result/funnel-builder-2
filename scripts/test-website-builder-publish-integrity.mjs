...
function materialBrandAssets(value) {
  if (!value || typeof value !== "object") return null;
  const logo = value.logo || null;
  const images = Array.isArray(value.images) ? value.images : [];
  const videos = Array.isArray(value.videos) ? value.videos : [];
  return !logo && images.length === 0 && videos.length === 0 ? null : { ...value, logo, images, videos };
}
...
function materialPageState(project, pageName) {
  return {
    ...
    brandAssets: materialBrandAssets(project.brandAssets),
  };
}
