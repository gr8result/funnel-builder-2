...
assert(activeReturn.includes("<DocxImportReview"), "DOCX imports must go through the Word review screen before save");
assert(activeBody.includes("importDocxAsStandardDocumentPreview"), "StandardInclusionsSheet must use the DOCX importer");
...
assert(emptyStateBody.includes("Import Editable Word Document (.docx)"), "Empty state must make DOCX the recommended editable workflow");
assert(emptyStateBody.includes("Attach Finished PDF"), "Empty state must preserve the finished-PDF path");
assert(emptyStateBody.includes("Upload PowerPoint Template"), "Empty state must preserve the PowerPoint route as a secondary path");
...
assert(emptyStateBody.includes("Use Premier Base Template"), "Empty state must expose deliberate Premier Template loading");
...
assert(contextPanelBody.includes("Import Editable Word Document (.docx)"), "Replace dialog must offer DOCX first-class import");
