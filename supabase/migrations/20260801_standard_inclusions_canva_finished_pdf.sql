...
set organisation_id = coalesce(organisation_id, tenant_id),
    current_export_pdf_storage_key = coalesce(current_export_pdf_storage_key, current_exported_pdf_asset_id)
where organisation_id is null
   or current_export_pdf_storage_key is null;
...
create index if not exists standard_inclusions_documents_org_project_idx on public.standard_inclusions_documents (organisation_id, project_id);
create index if not exists standard_inclusions_documents_canva_design_idx on public.standard_inclusions_documents (canva_design_id);
create index if not exists standard_inclusions_versions_document_idx on public.standard_inclusions_versions (document_id, version_number desc);
...
alter table public.standard_inclusions_versions enable row level security;

drop policy if exists canva_connections_owner_select on public.canva_connections;
create policy canva_connections_owner_select on public.canva_connections
  for select using (auth.uid() = user_id);

drop policy if exists canva_templates_member_select on public.canva_templates;
...
drop policy if exists standard_inclusions_versions_member_select on public.standard_inclusions_versions;
