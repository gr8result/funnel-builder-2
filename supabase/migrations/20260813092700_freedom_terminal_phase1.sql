...
alter table companies
  ...
  add column if not exists colour_primary text,
  add column if not exists colour_secondary text,
  add column if not exists colour_accent text,
  add column if not exists employees integer;

update companies
set ticker = coalesce(ticker, symbol)
...
alter table live_prices
  ...
  add column if not exists close_previous numeric,
...
alter table valuation_models
  add column if not exists current_eps numeric,
  add column if not exists growth_rate numeric,
  add column if not exists required_return numeric,
  add column if not exists terminal_pe numeric,
  add column if not exists strong_buy numeric,
  add column if not exists buy numeric,
  add column if not exists hold numeric,
  add column if not exists expensive numeric,
...
update watchlists
set name = coalesce(name, watchlist_name)
where name is null;

create unique index if not exists watchlists_name_owner_uidx
  on watchlists(name, owner);
...
alter table portfolio_holdings
  add column if not exists portfolio_id uuid references portfolio(id) on delete cascade,
